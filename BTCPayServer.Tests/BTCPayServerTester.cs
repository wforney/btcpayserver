...
public class BTCPayServerTester : IDisposable
{
    ...
    private readonly ILog TestLogs;
    public BTCPayServerTester(ILog testLogs, ILoggerProvider loggerProvider, string scope)
    {
        LoggerProvider = loggerProvider;
        TestLogs = testLogs;
        _Directory = scope ?? throw new ArgumentNullException(nameof(scope));
    }
    ...
    public async Task StartAsync()
    {
        ...
        Microsoft.Extensions.Configuration.IConfiguration conf = new DefaultConfiguration() { Logger = LoggerProvider.CreateLogger("Console") }.CreateConfiguration(new[] { "--datadir", _Directory, "--conf", confPath, "--disable-registration", DisableRegistration ? "true" : "false" });
        ...
        ICollection<string> urls = _Host.ServerFeatures.Get<IServerAddressesFeature>().Addresses;
    ...
    private MockRateProvider coinAverageMock;
    private async Task WaitSiteIsOperational()
    {
        _ = HttpClient.GetAsync("/").ConfigureAwait(false);
        using var cts = new CancellationTokenSource(20_000);
        Task synching = WaitIsFullySynched(cts.Token);
    ...
    {
        NBXplorerDashboard dashBoard = GetService<NBXplorerDashboard>();
    ...
    {
        DirectoryInfo solutionDirectory = TestUtils.TryGetSolutionDirectoryInfo(Directory.GetCurrentDirectory());
    ...
    public T GetController<T>(string userId = null, string storeId = null, bool isAdmin = false) where T : Controller
    {
        ...
        IServiceProvider provider = scope.CreateScope().ServiceProvider;
        context.RequestServices = provider;

        IHttpContextAccessor httpAccessor = provider.GetRequiredService<IHttpContextAccessor>();
