...
public class FastTests : UnitTestBase
{
    ...
    public void CheckDockerComposeUpToDate()
    {
        ...
        List<DockerImage> img1 = GetImages(compose1);
        List<DockerImage> img2 = GetImages(compose2);
        IEnumerable<IGrouping<string, DockerImage>> groups = img1.Concat(img2).GroupBy(g => g.Name);
        foreach (IGrouping<string, DockerImage> g in groups)
        {
    ...
    private string GetAttributeValue(string nodeHtml, string attribute)
    {
        ...
        Match match = regex.Match(nodeHtml);
        if (match.Success)
        {
    ...
    private void CheckHtmlNodesForReferrer(string filePath, string html, string tagName, string attribute)
    {
        ...
        IEnumerable<Match> matches = aNodeRegex.Matches(html).OfType<Match>();
    ...
    public void CanParseTorrc()
    {
        ...
        Assert.True(Torrc.TryParse(input, out Torrc torrc));
...
#pragma warning restore CS0618
    ...
    public void CanParseLegacyLabels()
    {
        static void AssertContainsRawLabel(WalletTransactionInfo info)
        {
            foreach (var item in new[] { "blah", "lol", "hello" })
            {
                ...
                RawLabel rawLabel = Assert.IsType<RawLabel>(info.Labels[item]);
        ...
        {
            Labels = "blah,lol,hello,lol"
        };
        WalletTransactionInfo info = data.GetBlobInfo();
        ...
        info = data.GetBlobInfo();
        KeyValuePair<string, LabelData> label = Assert.Single(info.Labels);
        ...
        static void AssertContainsLabel(WalletTransactionInfo info)
        {
            ...
            ReferenceLabel invoiceLabel = Assert.IsType<ReferenceLabel>(info.Labels["invoice"]);
            ...
            ReferenceLabel appLabel = Assert.IsType<ReferenceLabel>(info.Labels["app"]);
        ...
        static void AssertPayoutLabel(WalletTransactionInfo info)
        {
            ...
            PayoutLabel l = Assert.IsType<PayoutLabel>(info.Labels["payout"]);
    ...
    public void DeterministicUTXOSorter()
    {
        ...
        Payments.PayJoin.PayJoinEndpointController.UTXODeterministicComparer comparer = Payments.PayJoin.PayJoinEndpointController.UTXODeterministicComparer.Instance;
        UTXO[] utxos = Enumerable.Range(0, 100).Select(_ => CreateRandomUTXO()).ToArray();
        Array.Sort(utxos, comparer);
        UTXO utxo53 = utxos[53];
        Array.Sort(utxos, comparer);
        Assert.Equal(utxo53, utxos[53]);
        UTXO utxo54 = utxos[54];
        UTXO utxo52 = utxos[52];
...
#pragma warning disable CS0618
        ...
        PaymentMethod paymentMethod = entity.GetPaymentMethods().TryGet("BTC", PaymentTypes.BTCLike);
        PaymentMethodAccounting accounting = paymentMethod.Calculate();
        Assert.Equal(Money.Coins(1.1m), accounting.Due);
        Assert.Equal(Money.Coins(1.1m), accounting.TotalDue);
    ...
    public void CanCalculatePeriod()
    {
        ...
        (DateTimeOffset Start, DateTimeOffset? End) period = data.GetPeriod(Date(1)).Value;
    ...
    {
        foreach ((decimal, string, string) test in new[]
        {
    ...
    public async Task CanEnumerateTorServices()
    {
        ...
        TorService btcpayS = Assert.Single(tor.Services.Where(t => t.ServiceType == TorServiceType.BTCPayServer));
        Assert.Null(btcpayS.Network);
        Assert.Equal("host.onion", btcpayS.OnionHost);
        Assert.Equal(80, btcpayS.VirtualPort);

        TorService p2p = Assert.Single(tor.Services.Where(t => t.ServiceType == TorServiceType.P2P));
        ...
        TorService rpc = Assert.Single(tor.Services.Where(t => t.ServiceType == TorServiceType.RPC));
        Assert.NotNull(p2p.Network);
        Assert.Equal("BTC", rpc.Network.CryptoCode);
        Assert.Equal("host3.onion", rpc.OnionHost);
        Assert.Equal(82, rpc.VirtualPort);

        TorService unknown = Assert.Single(tor.Services.Where(t => t.ServiceType == TorServiceType.Other));
    ...
    {
        BTCPayNetwork mainnet = new BTCPayNetworkProvider(ChainName.Mainnet).GetNetwork<BTCPayNetwork>("BTC");
        ExtKey root = new Mnemonic(
        ...
        Assert.True(DerivationSchemeSettings.TryParseFromWalletFile(
            ...
            mainnet, out DerivationSchemeSettings settings));
        ...
        Assert.Equal(settings.AccountKeySettings[0].RootFingerprint,
            HDFingerprint.TryParse("8bafd160", out HDFingerprint hd) ? hd : default);
        ...
        BTCPayNetwork testnet = new BTCPayNetworkProvider(ChainName.Testnet).GetNetwork<BTCPayNetwork>("BTC");
        ...
        Assert.True(DerivationSchemeSettings.TryParseFromWalletFile(
            ...
            mainnet, out DerivationSchemeSettings specter));
    ...
    public void CheckRatesProvider()
    {
        ...
        RateRules.TryParse("X_X = bittrex(X_X);", out RateRules rateRules);

        RateProviderFactory factory = CreateBTCPayRateFactory();
        factory.Providers.Clear();
        var fetcher = new RateFetcher(factory);
        factory.Providers.Clear();
        var fetch = new BackgroundFetcherRateProvider(spy)
        {
            DoNotAutoFetchIfExpired = true
        };
        factory.Providers.Add("bittrex", fetch);
        RateResult fetchedRate = fetcher.FetchRate(CurrencyPair.Parse("BTC_USD"), rateRules, default).GetAwaiter()
            .GetResult();
        spy.AssertHit();
    ...
    private class SpyRateProvider : IRateProvider
    {
        ...
        public Task<PairRate[]> GetRatesAsync(CancellationToken cancellationToken)
        {
            ...
            var rates = new List<PairRate>
                {
                    new PairRate(CurrencyPair.Parse("BTC_USD"), new BidAsk(5000))
                };
    ...
    public async Task CanExpandExternalConnectionString()
    {
        ...
        Assert.True(ExternalConnectionString.TryParse("server=/test", out ExternalConnectionString connStr, out var error));
        ExternalConnectionString expanded = await connStr.Expand(new Uri("https://toto.com"), ExternalServiceTypes.Charge,
            ChainName.Mainnet);
    ...
    public void CanCheckFileNameValid()
    {
        ...
        foreach ((string, bool) t in tests)
        {
    ...
    public void CanFixupWebhookEventPropertyName()
    {
        ...
        WebhookEvent obj = JsonConvert.DeserializeObject<WebhookEvent>(legacy, WebhookEvent.DefaultSerializerSettings);
    ...
    {
        Assert.True(SSH.SSHFingerprint.TryParse("4e343c6fc6cfbf9339c02d06a151e1dd", out SSH.SSHFingerprint unused));
        ...
        Assert.True(SSH.SSHFingerprint.TryParse("Wl7CdRgT4u5T7yPMsxSrlFP+HIJJWwidGkzphJ8di5w=", out SSH.SSHFingerprint f1));
        Assert.True(SSH.SSHFingerprint.TryParse("SHA256:Wl7CdRgT4u5T7yPMsxSrlFP+HIJJWwidGkzphJ8di5w", out SSH.SSHFingerprint f2));
    ...
    public void HasCurrencyDataForNetworks()
    {
        ...
        foreach (BTCPayNetworkBase network in btcPayNetworkProvider.GetAll())
        {
            CurrencyData cd = CurrencyNameTable.Instance.GetCurrencyData(network.CryptoCode, false);
            Assert.NotNull(cd);
    ...
    {
        Assert.True(CurrencyValue.TryParse("1.50USD", out CurrencyValue result));
    ...
    public async Task MultiProcessingQueueTests()
    {
        ...
        MultiProcessingQueueTest q10 = Enqueue(q, "q1");
        MultiProcessingQueueTest q11 = Enqueue(q, "q1");
        MultiProcessingQueueTest q20 = Enqueue(q, "q2");
        MultiProcessingQueueTest q30 = Enqueue(q, "q3");
        q10.AssertStarted();
        q11.AssertStopped();
        q20.AssertStarted();
        q30.AssertStarted();
    ...
    private class MultiProcessingQueueTest
    {
        public bool Started;
        public bool Aborted;
    ...
    public async Task CanScheduleBackgroundTasks()
    {
        ...
        Task processing = client.ProcessJobs(cts.Token);

        Assert.Equal(4, client.GetExecutingCount());

        Task waitJobsFinish = client.WaitAllRunning(default);
    ...
    public void SecondDuplicatedRuleIsIgnored()
    {
        ...
        RateRule rule = rules.GetRuleFor(new CurrencyPair("DOGE", "BTC"));
        rule.Reevaluate();
    ...
    public void CanSerializeExchangeRatesCache()
    {
        HostedServices.RatesHostedService.ExchangeRatesCache cache = new HostedServices.RatesHostedService.ExchangeRatesCache
        {
            ...
            States = new List<Services.Rates.BackgroundFetcherState>()
        };
        ...
        RatesHostedService.ExchangeRatesCache cache2 = JsonConvert.DeserializeObject<HostedServices.RatesHostedService.ExchangeRatesCache>(str);
    ...
    {
        PullPaymentBlob b = JsonConvert.DeserializeObject<PullPaymentBlob>("{}");
    ...
    public void CanParseRateRules()
    {
        ...
        Assert.False(RateRules.TryParse("DPW*&W&#hdi&#&3JJD", out RateRules rules));
        ...
        Assert.Equal(
            ...
            rules.ToString());
        (string Pair, string Expected)[] tests = new[]
        {
        ...
        foreach ((string Pair, string Expected) test in tests)
        {
        ...
        (string Pair, string Expected, string ExpectedExchangeRates)[] tests2 = new[]
        {
        ...
        foreach ((string Pair, string Expected, string ExpectedExchangeRates) test in tests2)
        {
            RateRule rule = rules.GetRuleFor(CurrencyPair.Parse(test.Pair));
        ...
        RateRule rule2 = rules.GetRuleFor(CurrencyPair.Parse("DOGE_CAD"));
    ...
    public void LoadSubChainsAlways()
    {
        ...
        BTCPayNetworkProvider networkProvider = config.ConfigureNetworkProvider(BTCPayLogs);
    ...
    public void CanParseDerivationScheme()
    {
        ...
        (DerivationStrategyBase, RootedKeyPath[]) parsedDescriptor = mainnetParser.ParseOutputDescriptor(
        ...
        MultisigDerivationStrategy strat = Assert.IsType<MultisigDerivationStrategy>(Assert.IsType<P2SHDerivationStrategy>(parsedDescriptor.Item1).Inner);
...
#pragma warning disable CS0618
        ...
        BTCPayNetworkBase networkBTC = networkProvider.GetNetwork("BTC");
        BTCPayNetworkBase networkLTC = networkProvider.GetNetwork("LTC");
        InvoiceEntity invoiceEntity = new InvoiceEntity
        {
            Networks = networkProvider,
            Payments = new System.Collections.Generic.List<PaymentEntity>(),
            Price = 100
        };
        PaymentMethodDictionary paymentMethods = new PaymentMethodDictionary
            {
                new PaymentMethod() { Network = networkBTC, CryptoCode = "BTC", Rate = 10513.44m, }
                ...
                new PaymentMethod() { Network = networkLTC, CryptoCode = "LTC", Rate = 216.79m }
                .SetPaymentMethodDetails(
                    new BTCPayServer.Payments.Bitcoin.BitcoinLikeOnChainPaymentMethod()
                    {
        ...
        PaymentMethod btc = invoiceEntity.GetPaymentMethod(new PaymentMethodId("BTC", PaymentTypes.BTCLike));
        PaymentMethodAccounting accounting = btc.Calculate();
        ...
        PaymentMethod ltc = invoiceEntity.GetPaymentMethod(new PaymentMethodId("LTC", PaymentTypes.BTCLike));
        ...
        PaymentMethod paymentMethod = InvoiceWatcher.GetNearestClearedPayment(paymentMethods, out PaymentMethodAccounting accounting2);
