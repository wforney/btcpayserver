...
public class PayJoinTests : UnitTestBase
{
    ...
    public async Task CanUseTheDelayedBroadcaster()
    {
        ...
        BTCPayNetwork network = tester.NetworkProvider.GetNetwork<BTCPayNetwork>("BTC");
        DelayedTransactionBroadcaster broadcaster = tester.PayTester.GetService<DelayedTransactionBroadcaster>();
        await broadcaster.Schedule(DateTimeOffset.UtcNow + TimeSpan.FromDays(500), RandomTransaction(network), network);
        Transaction tx = RandomTransaction(network);
    ...
    public async Task CanUsePayjoinRepository()
    {
        ...
        await tester.StartAsync();
        BTCPayNetwork network = tester.NetworkProvider.GetNetwork<BTCPayNetwork>("BTC");
        PayJoinRepository repo = tester.PayTester.GetService<PayJoinRepository>();
        ...
        OutPoint outpoint1 = RandomOutpoint();
        OutPoint outpoint2 = RandomOutpoint();
    ...
    public async Task ChooseBestUTXOsForPayjoin()
    {
        ...
        await tester.StartAsync();
        BTCPayNetwork network = tester.NetworkProvider.GetNetwork<BTCPayNetwork>("BTC");
        PayJoinEndpointController controller = tester.PayTester.GetService<PayJoinEndpointController>();

        //Only one utxo, so obvious result
        UTXO[] utxos = new[] { FakeUTXO(1.0m) };
        var paymentAmount = 0.5m;
        var otherOutputs = new[] { 0.5m };
        var inputs = new[] { 1m };
        (UTXO[] selectedUTXO, PayJoinEndpointController.PayjoinUtxoSelectionType selectionType) result = await controller.SelectUTXO(network, utxos, inputs, paymentAmount, otherOutputs);
    ...
    {
        Transaction tx = network.NBitcoinNetwork.CreateTransaction();
    ...
    public async Task CanOnlyUseCorrectAddressFormatsForPayjoin()
    {
        ...
        await tester.StartAsync();
        DelayedTransactionBroadcaster broadcaster = tester.PayTester.GetService<DelayedTransactionBroadcaster>();
        PayJoinRepository payjoinRepository = tester.PayTester.GetService<PayJoinRepository>();
        broadcaster.Disable();
        BTCPayNetwork network = tester.NetworkProvider.GetNetwork<BTCPayNetwork>("BTC");
        BTCPayWallet btcPayWallet = tester.PayTester.GetService<BTCPayWalletProvider>().GetWallet(network);
        NBitcoin.RPC.RPCClient cashCow = tester.ExplorerNode;
        cashCow.Generate(2); // get some money in case

        ScriptPubKeyType[] unsupportedFormats = new[] { ScriptPubKeyType.Legacy };
        ...
        foreach (ScriptPubKeyType senderAddressType in Enum.GetValues(typeof(ScriptPubKeyType)))
        {
            ...
            TestAccount senderUser = tester.NewAccount();
            ...
            foreach (ScriptPubKeyType receiverAddressType in Enum.GetValues(typeof(ScriptPubKeyType)))
            {
                ...
                Coin senderCoin = await senderUser.ReceiveUTXO(Money.Satoshis(100000), network);

                TestLogs.LogInformation($"Testing payjoin with sender: {senderAddressType} receiver: {receiverAddressType}");
                TestAccount receiverUser = tester.NewAccount();
                ...
                Coin receiverCoin = await receiverUser.ReceiveUTXO(Money.Satoshis(810), network);

                string errorCode = receiverAddressType == senderAddressType ? null : "unavailable|any UTXO available";
                Invoice invoice = receiverUser.BitPay.CreateInvoice(new Invoice() { Price = 50000, Currency = "sats", FullNotifications = true });
                ...
                TransactionBuilder txBuilder = network.NBitcoinNetwork.CreateTransactionBuilder();
                ...
                PSBT psbt = txBuilder.BuildPSBT(false);
                psbt = await senderUser.Sign(psbt);
                PSBT pj = await senderUser.SubmitPayjoin(invoice, psbt, errorCode, false);
            }
        }
    }
    ...
    {
        using SeleniumTester s = CreateSeleniumTester();
        await s.StartAsync();
        s.RegisterNewUser(true);
        (string storeName, string storeId) receiver = s.CreateNewStore();
        Mnemonic receiverSeed = s.GenerateWallet("BTC", "", true, true, ScriptPubKeyType.Segwit);
        var receiverWalletId = new WalletId(receiver.storeId, "BTC");

        (string storeName, string storeId) sender = s.CreateNewStore();
        Mnemonic senderSeed = s.GenerateWallet("BTC", "", true, true, ScriptPubKeyType.Segwit);
        ...
        InvoiceRepository invoiceRepository = s.Server.PayTester.GetService<InvoiceRepository>();
        await TestUtils.EventuallyAsync(async () =>
        {
            InvoiceEntity invoice = await invoiceRepository.GetInvoice(invoiceId);
    ...
    public async Task CanUsePayjoinViaUI()
    {
        ...
        foreach (ScriptPubKeyType format in new[] { ScriptPubKeyType.Segwit, ScriptPubKeyType.SegwitP2SH })
        {
            var cryptoCode = "BTC";
            (string storeName, string storeId) receiver = s.CreateNewStore();
            Mnemonic receiverSeed = s.GenerateWallet(cryptoCode, "", true, true, format);
            ...
            (string storeName, string storeId) sender = s.CreateNewStore();
            Mnemonic senderSeed = s.GenerateWallet(cryptoCode, "", true, true, format);
            ...
            await TestUtils.EventuallyAsync(async () =>
            {
                InvoiceEntity invoice = await s.Server.PayTester.GetService<InvoiceRepository>().GetInvoice(invoiceId);
                Assert.Equal(InvoiceStatusLegacy.Paid, invoice.Status);
            });

            s.SelectStoreContext(receiver.storeId);
            s.GoToInvoices();
            IWebElement paymentValueRowColumn = s.Driver.FindElement(By.Id($"invoice_details_{invoiceId}"))
            ...
            s.Driver.FindElement(By.Id("SignTransaction")).Click();
            NewOnChainTransactionEvent txId = await s.Server.WaitForEvent<NewOnChainTransactionEvent>(() =>
            {
                s.Driver.FindElement(By.CssSelector("button[value=payjoin]")).Click();
                return Task.CompletedTask;
            });
            ...
            {
                InvoiceEntity invoice = await invoiceRepository.GetInvoice(invoiceId);
                System.Collections.Generic.List<PaymentEntity> payments = invoice.GetPayments(false);
                Assert.Equal(2, payments.Count);
                PaymentEntity originalPayment = payments[0];
                PaymentEntity coinjoinPayment = payments[1];
                Assert.Equal(-1,
            ...
            {
                InvoiceEntity invoice = await s.Server.PayTester.GetService<InvoiceRepository>().GetInvoice(invoiceId);
                Models.InvoiceResponse dto = invoice.EntityToDTO();
                Assert.Equal(InvoiceStatusLegacy.Paid, invoice.Status);
            });
    ...
    public async Task CanUsePayjoin2()
    {
        using ServerTester tester = CreateServerTester();
        await tester.StartAsync();
        PayjoinClient pjClient = tester.PayTester.GetService<PayjoinClient>();
        NBXplorer.ExplorerClient nbx = tester.PayTester.GetService<ExplorerClientProvider>().GetExplorerClient("BTC");
        NBXplorer.WebsocketNotificationSession notifications = await nbx.CreateWebsocketNotificationSessionAsync();
        TestAccount alice = tester.NewAccount();
        ...
        BitcoinAddress paymentAddress = new Key().PubKey.GetAddress(ScriptPubKeyType.Legacy, Network.RegTest);
        BitcoinAddress otherAddress = new Key().PubKey.GetAddress(ScriptPubKeyType.Legacy, Network.RegTest);
        PSBT psbt = (await nbx.CreatePSBTAsync(alice.DerivationScheme, new CreatePSBTRequest()
        {
            Destinations =
                    {
        ...
        DerivationSchemeSettings derivationSchemeSettings = alice.GetController<UIWalletsController>().GetDerivationSchemeSettings(new WalletId(alice.StoreId, "BTC"));
        AccountKeySettings signingAccount = derivationSchemeSettings.GetSigningAccountKeySettings();
        ...
        Task<PSBT> requesting = pjClient.RequestPayjoin(bip21, new PayjoinWallet(derivationSchemeSettings), psbt, default);
        HttpContext request = await fakeServer.GetNextRequest();
        ...
        PSBT originalPSBT = await ParsePSBT(request);
        Transaction proposalTx = originalPSBT.GetGlobalTransaction();
        ...
        fakeServer.Done();
        PayjoinSenderException ex = await Assert.ThrowsAsync<PayjoinSenderException>(async () => await requesting);
        ...
        originalPSBT = await ParsePSBT(request);
        proposalTx = originalPSBT.GetGlobalTransaction();
        TxOut removedOutput = proposalTx.Outputs.First(o => o.ScriptPubKey == otherAddress.ScriptPubKey);
        ...
        TestAccount bob = tester.NewAccount();
        ...
        Invoice invoice = bob.BitPay.CreateInvoice(
        ...
        psbt.SignAll(derivationSchemeSettings.AccountDerivation, alice.GenerateWalletResponseV.AccountHDKey, signingAccount.GetRootedKeyPath());
        BitcoinUrlBuilder endpoint = TestAccount.GetPayjoinBitcoinUrl(invoice, Network.RegTest);
        pjClient.MaxFeeBumpContribution = Money.Satoshis(50);
        PSBT proposal = await pjClient.RequestPayjoin(endpoint, new PayjoinWallet(derivationSchemeSettings), psbt, default);
        Assert.True(proposal.TryGetFee(out Money newFee));
        ...
        PayjoinReceiverException ex2 = await Assert.ThrowsAsync<PayjoinReceiverException>(async () => await pjClient.RequestPayjoin(endpoint, new PayjoinWallet(derivationSchemeSettings), psbt, default));
    ...
    public async Task CanUsePayjoinFeeCornerCase()
    {
        using (ServerTester tester = CreateServerTester())
        {
            ...
            (Money SpentCoin, Money InvoiceAmount, Money Paid, Money Fee, bool InvoicePaid, string ExpectedError, bool OriginalTxBroadcasted) vector = (SpentCoin: Money.Satoshis(810), InvoiceAmount: Money.Satoshis(700), Paid: Money.Satoshis(700), Fee: Money.Satoshis(110), InvoicePaid: true, ExpectedError: "not-enough-money", OriginalTxBroadcasted: true);
            async Task<PSBT> RunVector(bool skipLockedCheck = false)
            {
                Coin coin = await senderUser.ReceiveUTXO(vector.SpentCoin, network);
                Invoice invoice = receiverUser.BitPay.CreateInvoice(new Invoice() { Price = vector.InvoiceAmount.ToDecimal(MoneyUnit.BTC), Currency = "BTC", FullNotifications = true });
                ...
                PSBT psbt = txBuilder.BuildPSBT(false);
                psbt = await senderUser.Sign(psbt);
                PSBT pj = await senderUser.SubmitPayjoin(invoice, psbt, vector.ExpectedError);
                if (vector.ExpectedError is null)
                {
                    Assert.Contains(pj.Inputs, o => o.PrevOut == receiverCoin.Outpoint);
                    foreach (TxIn input in pj.GetGlobalTransaction().Inputs)
                    {
                ...
                psbt.Finalize();
                BroadcastResult broadcasted = await tester.PayTester.GetService<ExplorerClientProvider>().GetExplorerClient("BTC").BroadcastAsync(psbt.ExtractTransaction(), true);
            ...
            {
                ReceivedCoin[] coins = await btcPayWallet.GetUnspentCoins(receiverUser.DerivationScheme);
                foreach (ReceivedCoin coin in coins)
                {
            ...
            TestAccount originalSenderUser = senderUser;
retry:
...
// So total paid by sender should be 86 + 510 + 200 so we should get 1090 - (86 + 510 + 200) == 294 back)
            ...
            NBXplorer.ExplorerClient explorerClient = tester.PayTester.GetService<ExplorerClientProvider>().GetExplorerClient(proposedPSBT.Network.NetworkSet.CryptoCode);
            BroadcastResult result = await explorerClient.BroadcastAsync(proposedPSBT.ExtractTransaction());
            ...
            await TestUtils.EventuallyAsync(async () =>
            {
                InvoiceEntity invoice = await tester.PayTester.GetService<InvoiceRepository>().GetInvoice(lastInvoiceId);
                Assert.Equal(InvoiceStatusLegacy.Paid, invoice.Status);
    ...
    public async Task CanUsePayjoin()
    {
        using (ServerTester tester = CreateServerTester())
        {
            ...
            BTCPayNetwork btcPayNetwork = tester.NetworkProvider.GetNetwork<BTCPayNetwork>("BTC");
            BTCPayWallet btcPayWallet = tester.PayTester.GetService<BTCPayWalletProvider>().GetWallet(btcPayNetwork);
            NBitcoin.RPC.RPCClient cashCow = tester.ExplorerNode;
            cashCow.Generate(2); // get some money in case
            ...
            Invoice invoice = senderUser.BitPay.CreateInvoice(
            ...
            BitcoinAddress senderChange = await senderUser.GetNewAddress(btcPayNetwork);

            //Let's start the harassment
            invoice = receiverUser.BitPay.CreateInvoice(
                new Invoice() { Price = 0.02m, Currency = "BTC", FullNotifications = true });
            // Bad version should throw incorrect version
            BitcoinUrlBuilder bip21 = TestAccount.GetPayjoinBitcoinUrl(invoice, btcPayNetwork.NBitcoinNetwork);
            bip21.TryGetPayjoinEndpoint(out Uri endpoint);
            HttpResponseMessage response = await tester.PayTester.HttpClient.PostAsync(endpoint.AbsoluteUri + "?v=2",
            ...
            Invoice invoice2 = receiverUser.BitPay.CreateInvoice(
                new Invoice() { Price = 0.02m, Currency = "BTC", FullNotifications = true });
            var secondInvoiceParsedBip21 = new BitcoinUrlBuilder(invoice2.CryptoInfo.First().PaymentUrls.BIP21,
                tester.ExplorerClient.Network.NBitcoinNetwork);

            Data.StoreData senderStore = await tester.PayTester.StoreRepository.FindStore(senderUser.StoreId);
            var paymentMethodId = new PaymentMethodId("BTC", PaymentTypes.BTCLike);
            DerivationSchemeSettings derivationSchemeSettings = senderStore.GetSupportedPaymentMethods(tester.NetworkProvider)
            ...
            BitcoinExtKey extKey =
                senderUser.GenerateWalletResponseV.MasterHDKey.Derive(signingKeySettings.GetRootedKeyPath()
                    .KeyPath);


            Network n = tester.ExplorerClient.Network.NBitcoinNetwork;
            Transaction Invoice1Coin1 = tester.ExplorerClient.Network.NBitcoinNetwork.CreateTransactionBuilder()
            ...
            Transaction Invoice1Coin2 = tester.ExplorerClient.Network.NBitcoinNetwork.CreateTransactionBuilder()
                .SetChange(senderChange)
                .Send(parsedBip21.Address, parsedBip21.Amount)
                .AddCoins(coin2.Coin)
            ...
            Transaction Invoice2Coin1 = tester.ExplorerClient.Network.NBitcoinNetwork.CreateTransactionBuilder()
                .SetChange(senderChange)
            ...
            Transaction Invoice2Coin2 = tester.ExplorerClient.Network.NBitcoinNetwork.CreateTransactionBuilder()
                .SetChange(senderChange)
                .Send(secondInvoiceParsedBip21.Address, secondInvoiceParsedBip21.Amount)
                .AddCoins(coin2.Coin)
                .AddKeys(extKey.Derive(coin2.KeyPath))
            ...
            Transaction Invoice1Coin1ResponseTx = await senderUser.SubmitPayjoin(invoice, Invoice1Coin1, btcPayNetwork);

            await senderUser.SubmitPayjoin(invoice, Invoice1Coin1, btcPayNetwork, "already-paid");
            System.Collections.Generic.IEnumerable<TxIn> contributedInputsInvoice1Coin1ResponseTx =
            ...
            Transaction Invoice2Coin2ResponseTx = await senderUser.SubmitPayjoin(invoice2, Invoice2Coin2, btcPayNetwork);

            System.Collections.Generic.IEnumerable<TxIn> contributedInputsInvoice2Coin2ResponseTx =
            ...
            Invoice invoice3 = receiverUser.BitPay.CreateInvoice(
            ...
            Invoice invoice4 = receiverUser.BitPay.CreateInvoice(
                new Invoice() { Price = 0.01m, Currency = "BTC", FullNotifications = true });
            var invoice4ParsedBip21 = new BitcoinUrlBuilder(invoice4.CryptoInfo.First().PaymentUrls.BIP21,
                tester.ExplorerClient.Network.NBitcoinNetwork);


            Transaction Invoice3AndInvoice4Coin3 = tester.ExplorerClient.Network.NBitcoinNetwork.CreateTransactionBuilder()
                .SetChange(senderChange)
            ...
            Invoice invoice5 = receiverUser.BitPay.CreateInvoice(
                new Invoice() { Price = 0.01m, Currency = "BTC", FullNotifications = true });
            var invoice5ParsedBip21 = new BitcoinUrlBuilder(invoice5.CryptoInfo.First().PaymentUrls.BIP21,
                tester.ExplorerClient.Network.NBitcoinNetwork);

            TransactionBuilder Invoice5Coin4TxBuilder = tester.ExplorerClient.Network.NBitcoinNetwork.CreateTransactionBuilder()
                .SetChange(senderChange)
            ...
            Transaction Invoice5Coin4 = Invoice5Coin4TxBuilder.BuildTransaction(true);
            Transaction Invoice5Coin4ResponseTx = await senderUser.SubmitPayjoin(invoice5, Invoice5Coin4, btcPayNetwork);
            ...
            Invoice invoice6 = receiverUser.BitPay.CreateInvoice(
                new Invoice() { Price = 0.01m, Currency = "BTC", FullNotifications = true });
            var invoice6ParsedBip21 = new BitcoinUrlBuilder(invoice6.CryptoInfo.First().PaymentUrls.BIP21,
                tester.ExplorerClient.Network.NBitcoinNetwork);

            TransactionBuilder invoice6Coin5TxBuilder = tester.ExplorerClient.Network.NBitcoinNetwork.CreateTransactionBuilder()
                .SetChange(senderChange)
            ...
            Transaction invoice6Coin5 = invoice6Coin5TxBuilder
                .BuildTransaction(true);

            Transaction Invoice6Coin5Response1Tx = await senderUser.SubmitPayjoin(invoice6, invoice6Coin5, btcPayNetwork);
            Transaction Invoice6Coin5Response1TxSigned = invoice6Coin5TxBuilder.SignTransaction(Invoice6Coin5Response1Tx);
            ...
            Invoice invoice7 = receiverUser.BitPay.CreateInvoice(
                new Invoice() { Price = 0.01m, Currency = "BTC", FullNotifications = true });
            var invoice7ParsedBip21 = new BitcoinUrlBuilder(invoice7.CryptoInfo.First().PaymentUrls.BIP21,
                tester.ExplorerClient.Network.NBitcoinNetwork);

            TransactionBuilder txBuilder = tester.ExplorerClient.Network.NBitcoinNetwork.CreateTransactionBuilder();
            txBuilder.OptInRBF = true;
            TransactionBuilder invoice7Coin6TxBuilder = txBuilder
                .SetChange(senderChange)
            ...
            Transaction invoice7Coin6Tx = invoice7Coin6TxBuilder
                .BuildTransaction(true);

            Transaction invoice7Coin6Response1Tx = await senderUser.SubmitPayjoin(invoice7, invoice7Coin6Tx, btcPayNetwork);
            Transaction Invoice7Coin6Response1TxSigned = invoice7Coin6TxBuilder.SignTransaction(invoice7Coin6Response1Tx);
            TxIn contributedInputsInvoice7Coin6Response1TxSigned =
            ...
            BroadcastResult res = (await tester.ExplorerClient.BroadcastAsync(Invoice7Coin6Response1TxSigned));
            Assert.True(res.Success);

            // Paid with coinjoin
            await TestUtils.EventuallyAsync(async () =>
            {
                InvoiceEntity invoiceEntity = await tester.PayTester.GetService<InvoiceRepository>().GetInvoice(invoice7.Id);
            ...
            Transaction invoice7Coin6Tx2 = tester.ExplorerClient.Network.NBitcoinNetwork.CreateTransactionBuilder()
                .SetChange(senderChange)
                .AddCoins(coin6.Coin)
            ...
            uint256 blockhash = tester.ExplorerNode.Generate(1)[0];
