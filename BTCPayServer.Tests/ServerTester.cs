...
public class ServerTester : IDisposable
{
    ...
    {
        Network btc = NetworkProvider.GetNetwork<BTCPayNetwork>("BTC").NBitcoinNetwork;
    ...
    public async Task<T> WaitForEvent<T>(Func<Task> action, Func<T, bool> correctEvent = null)
    {
        ...
        IEventAggregatorSubscription sub = PayTester.GetService<EventAggregator>().Subscribe<T>(evt =>
        {
    ...
    public void Dispose()
    {
        foreach (IDisposable r in Resources)
        {
            r.Dispose();
        }
