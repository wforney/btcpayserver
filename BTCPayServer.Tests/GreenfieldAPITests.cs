...
public class GreenfieldAPITests : UnitTestBase
{
    ...
    {
        using ServerTester tester = CreateServerTester();
        await tester.StartAsync();
        TestAccount user = tester.NewAccount();
        await user.GrantAccessAsync();
        await user.MakeAdmin();
        IBTCPayServerClientFactory factory = tester.PayTester.GetService<IBTCPayServerClientFactory>();
        Assert.NotNull(factory);
        BTCPayServerClient client = await factory.Create(user.UserId);
        ApplicationUserData u = await client.GetCurrentUser();
        IEnumerable<StoreData> s = await client.GetStores();
    }
    ...
    public async Task MissingPermissionTest()
    {
        ...
        TestAccount user = tester.NewAccount();
        user.GrantAccess();
        BTCPayServerClient clientWithWrongPermissions = await user.CreateClient(Policies.CanViewProfile);
        GreenfieldAPIException e = await AssertAPIError("missing-permission", () => clientWithWrongPermissions.CreateStore(new CreateStoreRequest() { Name = "mystore" }));
    ...
    public async Task ApiKeysControllerTests()
    {
        ...
        user.GrantAccess();
        await user.MakeAdmin();
        BTCPayServerClient client = await user.CreateClient(Policies.CanViewProfile);
        BTCPayServerClient clientBasic = await user.CreateClient();
        //Get current api key 
        ApiKeyData apiKeyData = await client.GetCurrentAPIKeyInfo();
    ...
    public async Task CanUseMiscAPIs()
    {
        ...
        {
            await tester.StartAsync();
            TestAccount acc = tester.NewAccount();
            await acc.GrantAccessAsync();
            BTCPayServerClient unrestricted = await acc.CreateClient();
            Client.Models.Language[] langs = await unrestricted.GetAvailableLanguages();
            Assert.NotEmpty(langs);
            Assert.NotNull(langs[0].Code);
            Assert.NotNull(langs[0].DisplayName);

            PermissionMetadata[] perms = await unrestricted.GetPermissionMetadata();
            Assert.NotEmpty(perms);
            PermissionMetadata p = perms.First(p => p.PermissionName == "unrestricted");
    ...
    public async Task SpecificCanModifyStoreCantCreateNewStore()
    {
        ...
        await acc.GrantAccessAsync();
        BTCPayServerClient unrestricted = await acc.CreateClient();
        StoreData response = await unrestricted.CreateStore(new CreateStoreRequest() { Name = "mystore" });
        ...
        await AssertHttpError(403, async () => await restricted.CreateStore(new CreateStoreRequest() { Name = "store2" }));
    ...
    public async Task CanCreateAndDeleteAPIKeyViaAPI()
    {
        ...
        await acc.GrantAccessAsync();
        BTCPayServerClient unrestricted = await acc.CreateClient();
        ApiKeyData apiKey = await unrestricted.CreateAPIKey(new CreateApiKeyRequest()
        {
            Label = "Hello world",
            Permissions = new Permission[] { Permission.Create(Policies.CanViewProfile) }
        });
        Assert.Equal("Hello world", apiKey.Label);
        Permission p = Assert.Single(apiKey.Permissions);
        Assert.Equal(Policies.CanViewProfile, p.Policy);

        BTCPayServerClient restricted = acc.CreateClientFromAPIKey(apiKey.ApiKey);
    ...
    {
        using ServerTester tester = CreateServerTester(newDb: true);
        await tester.StartAsync();
        ...
        await user.GrantAccessAsync();
        await user.MakeAdmin();
        BTCPayServerClient adminClient = await user.CreateClient(Policies.Unrestricted);
        ...
        await user.GrantAccessAsync();
        BTCPayServerClient badClient = await user.CreateClient(Policies.CanCreateInvoice);

        await AssertHttpError(403,
            async () => await badClient.DeleteCurrentUser());

        BTCPayServerClient goodClient = await user.CreateClient(Policies.CanDeleteUser, Policies.CanViewProfile);
    ...
    public async Task CanViewUsersViaApi()
    {
        ...
        TestAccount adminUser = tester.NewAccount();
        await adminUser.GrantAccessAsync();
        await adminUser.MakeAdmin();
        BTCPayServerClient adminClient = await adminUser.CreateClient(Policies.Unrestricted);
        ...
        TestAccount goodUser = tester.NewAccount();
        await goodUser.GrantAccessAsync();
        await goodUser.MakeAdmin();
        BTCPayServerClient goodClient = await goodUser.CreateClient(Policies.CanViewUsers);
        ...
        TestAccount badUser = tester.NewAccount();
        await badUser.GrantAccessAsync();
        await badUser.MakeAdmin();

        // Bad user has a permission, but it's the wrong one.
        BTCPayServerClient badClient = await goodUser.CreateClient(Policies.CanCreateInvoice);
    ...
    public async Task CanCreateUsersViaAPI()
    {
        ...
        ApplicationUserData user1 = await unauthClient.CreateUser(
            new CreateApplicationUserRequest() { Email = "test@gmail.com", Password = "abceudhqw" });
        Assert.Empty(user1.Roles);

        // We have no admin, so it should work
        ApplicationUserData user2 = await unauthClient.CreateUser(
        ...
        // Let's make an admin
        ApplicationUserData admin = await unauthClient.CreateUser(new CreateApplicationUserRequest()
        {
        ...
        GreenfieldAPIException ex = await AssertAPIError("unauthenticated",
            async () => await unauthClient.CreateUser(
        ...
        SettingsRepository settings = tester.PayTester.GetService<SettingsRepository>();
        ...
        TestAccount adminAcc = tester.NewAccount();
        adminAcc.UserId = admin.Id;
        adminAcc.IsAdmin = true;
        BTCPayServerClient adminClient = await adminAcc.CreateClient(Policies.CanModifyProfile);
        ...
        TestAccount user1Acc = tester.NewAccount();
        user1Acc.UserId = user1.Id;
        user1Acc.IsAdmin = false;
        BTCPayServerClient user1Client = await user1Acc.CreateClient(Policies.CanModifyServerSettings);
    ...
    public async Task CanUsePullPaymentViaAPI()
    {
        ...
        BTCPayServerClient client = await acc.CreateClient();
        PullPaymentData result = await client.CreatePullPayment(storeId, new Client.Models.CreatePullPaymentRequest()
        {
            Name = "Test",
        ...
        PullPaymentData[] pullPayments = await client.GetPullPayments(storeId);
        result = Assert.Single(pullPayments);
        VerifyResult();

        PullPaymentData test2 = await client.CreatePullPayment(storeId, new Client.Models.CreatePullPaymentRequest()
        {
            Name = "Test 2",
            Amount = 12.3m,
            Currency = "BTC",
        ...
        GreenfieldAPIException ex = await AssertAPIError("missing-permission", async () => await client.ArchivePullPayment("lol", result.Id));
        ...
        PullPaymentData[] pps = await client.GetPullPayments(storeId);
        ...
        PayoutData[] payouts = await unauthenticated.GetPayouts(pps[0].Id);
        Assert.Empty(payouts);

        var destination = (await tester.ExplorerNode.GetNewAddressAsync()).ToString();
        await AssertAPIError("overdraft", async () => await unauthenticated.CreatePayout(pps[0].Id, new CreatePayoutRequest()
        {
        ...
        await AssertAPIError("archived", async () => await unauthenticated.CreatePayout(pps[1].Id, new CreatePayoutRequest()
        {
            Destination = destination,
            PaymentMethod = "BTC"
        }));

        PayoutData payout = await unauthenticated.CreatePayout(pps[0].Id, new CreatePayoutRequest()
        {
            Destination = destination,
            PaymentMethod = "BTC"
        });
        ...
        PayoutData payout2 = Assert.Single(payouts);
        ...
        await AssertAPIError("amount-too-low", async () => await unauthenticated.CreatePayout(pps[0].Id, new CreatePayoutRequest()
        {
            Destination = destination2,
            PaymentMethod = "BTC"
        }));
        ...
        DateTimeOffset start = RoundSeconds(DateTimeOffset.Now + TimeSpan.FromDays(7.0));
        PullPaymentData inFuture = await client.CreatePullPayment(storeId, new Client.Models.CreatePullPaymentRequest()
        {
        ...
        await AssertAPIError("not-started", async () => await unauthenticated.CreatePayout(inFuture.Id, new CreatePayoutRequest()
        {
            Amount = 1.0m,
            Destination = destination,
            PaymentMethod = "BTC"
        ...
        DateTimeOffset expires = RoundSeconds(DateTimeOffset.Now - TimeSpan.FromDays(7.0));
        PullPaymentData inPast = await client.CreatePullPayment(storeId, new Client.Models.CreatePullPaymentRequest()
        {
            Name = "Will expires",
            Amount = 12.3m,
            ExpiresAt = expires,
        ...
        await AssertAPIError("expired", async () => await unauthenticated.CreatePayout(inPast.Id, new CreatePayoutRequest()
        {
            Amount = 1.0m,
            Destination = destination,
            PaymentMethod = "BTC"
        ...
        await AssertValidationError(new[] { "ExpiresAt" }, async () => await client.CreatePullPayment(storeId, new Client.Models.CreatePullPaymentRequest()
        {
            Name = "Test 2",
            Amount = 12.3m,
        ...
        PullPaymentData pp = await client.CreatePullPayment(storeId, new Client.Models.CreatePullPaymentRequest()
        {
            Name = "Test USD",
            Amount = 5000m,
            Currency = "USD",
        ...
        await AssertAPIError("old-revision", async () => await client.ApprovePayout(storeId, payout.Id, new ApprovePayoutRequest()
        {
            Revision = -1
        }));
        await AssertAPIError("rate-unavailable", async () => await client.ApprovePayout(storeId, payout.Id, new ApprovePayoutRequest()
        {
        ...
        await AssertAPIError("invalid-state", async () => await client.ApprovePayout(storeId, payout.Id, new ApprovePayoutRequest()
        {
            Revision = payout.Revision
        }));

        // Create one pull payment with an amount of 9 decimals
        PullPaymentData test3 = await client.CreatePullPayment(storeId, new Client.Models.CreatePullPaymentRequest()
        {
            Name = "Test 2",
    ...
    {
        GreenfieldAPIException err = await Assert.ThrowsAsync<GreenfieldAPIException>(async () => await act());
    ...
    {
        GreenfieldAPIException err = await Assert.ThrowsAsync<GreenfieldAPIException>(async () => await act());
        GreenfieldPermissionAPIError err2 = Assert.IsType<GreenfieldPermissionAPIError>(err.APIError);
    ...
    public async Task StoresControllerTests()
    {
        ...
        user.GrantAccess();
        await user.MakeAdmin();
        BTCPayServerClient client = await user.CreateClient(Policies.Unrestricted);

        //create store
        StoreData newStore = await client.CreateStore(new CreateStoreRequest() { Name = "A" });

        //update store
        StoreData updatedStore = await client.UpdateStore(newStore.Id, new UpdateStoreRequest() { Name = "B" });
        Assert.Equal("B", updatedStore.Name);
        Assert.Equal("B", (await client.GetStore(newStore.Id)).Name);

        //list stores
        IEnumerable<StoreData> stores = await client.GetStores();
        IEnumerable<string> storeIds = stores.Select(data => data.Id);
        IEnumerable<string> storeNames = stores.Select(data => data.Name);
        ...
        //get store
        StoreData store = await client.GetStore(user.StoreId);
        ...
        BTCPayServerClient scopedClient =
        ...
        BTCPayServerClient noauth = await user.CreateClient(Array.Empty<string>());
        await AssertAPIError("missing-permission", () => noauth.GetStores());

        // We strip the user's Owner right, so the key should not work
        using Data.ApplicationDbContext ctx = tester.PayTester.GetService<Data.ApplicationDbContextFactory>().CreateContext();
        Data.UserStore storeEntity = await ctx.UserStore.SingleAsync(u => u.ApplicationUserId == user.UserId && u.StoreDataId == newStore.Id);
    ...
    private async Task<GreenfieldValidationException> AssertValidationError(string[] fields, Func<Task> act)
    {
        ...
        GreenfieldValidationException ex = await Assert.ThrowsAsync<GreenfieldValidationException>(act);
    ...
    {
        GreenfieldAPIException ex = await Assert.ThrowsAsync<GreenfieldAPIException>(act);
    ...
    public async Task UsersControllerTests()
    {
        ...
        user.GrantAccess();
        await user.MakeAdmin();
        BTCPayServerClient clientProfile = await user.CreateClient(Policies.CanModifyProfile);
        BTCPayServerClient clientServer = await user.CreateClient(Policies.CanCreateUser, Policies.CanViewProfile);
        BTCPayServerClient clientInsufficient = await user.CreateClient(Policies.CanModifyStoreSettings);
        BTCPayServerClient clientBasic = await user.CreateClient();


        ApplicationUserData apiKeyProfileUserData = await clientProfile.GetCurrentUser();
        ...
        ApplicationUserData newUser = await clientServer.CreateUser(new CreateApplicationUserRequest()
        {
            Email = $"{Guid.NewGuid()}@g.com",
            Password = Guid.NewGuid().ToString()
        });
        ...
        ApplicationUserData newUser2 = await clientBasic.CreateUser(new CreateApplicationUserRequest()
        {
            Email = $"{Guid.NewGuid()}@g.com",
            Password = Guid.NewGuid().ToString()
        });
    ...
    public async Task CanUseWebhooks()
    {
        ...
        BTCPayServerClient clientProfile = await user.CreateClient(Policies.CanModifyStoreWebhooks, Policies.CanCreateInvoice);
        StoreWebhookData hook = await clientProfile.CreateWebhook(user.StoreId, new CreateStoreWebhookRequest()
        {
        ...
        AssertHook(fakeServer, hook);
        StoreWebhookData[] hooks = await clientProfile.GetWebhooks(user.StoreId);
        hook = Assert.Single(hooks);
        AssertHook(fakeServer, hook);
        await clientProfile.CreateInvoice(user.StoreId,
                    new CreateInvoiceRequest() { Currency = "USD", Amount = 100 });
        Microsoft.AspNetCore.Http.HttpContext req = await fakeServer.GetNextRequest();
        ...
        {
            WebhookDeliveryData[] deliveries = await clientProfile.GetWebhookDeliveries(user.StoreId, hook.Id);
        ...
        await TestUtils.EventuallyAsync(async () =>
        {
            ...
            fakeServer.Done();
            WebhookDeliveryData newDelivery = await clientProfile.GetWebhookDelivery(user.StoreId, hook.Id, newDeliveryId);
            Assert.NotNull(newDelivery);
            Assert.Equal(404, newDelivery.HttpCode);
            WebhookEvent req = await clientProfile.GetWebhookDeliveryRequest(user.StoreId, hook.Id, newDeliveryId);
        ...
        WebhookEvent jObj = await clientProfile.GetWebhookDeliveryRequest(user.StoreId, hook.Id, newDeliveryId);
        Assert.NotNull(jObj);

        TestLogs.LogInformation("Should not be able to access webhook without proper auth");
        BTCPayServerClient unauthorized = await user.CreateClient(Policies.CanCreateInvoice);
        await AssertHttpError(403, async () =>
        {
    ...
    public async Task HealthControllerTests()
    {
        ...
        ApiHealthData apiHealthData = await unauthClient.GetHealth();
    ...
    public async Task ServerInfoControllerTests()
    {
        ...
        user.GrantAccess();
        BTCPayServerClient clientBasic = await user.CreateClient();
        ServerInfoData serverInfoData = await clientBasic.GetServerInfo();
    ...
    public async Task PaymentControllerTests()
    {
        ...
        await user.MakeAdmin();
        BTCPayServerClient client = await user.CreateClient(Policies.Unrestricted);
        BTCPayServerClient viewOnly = await user.CreateClient(Policies.CanViewPaymentRequests);
        ...
        PaymentRequestData newPaymentRequest = await client.CreatePaymentRequest(user.StoreId,
            new CreatePaymentRequestRequest() { Title = "A", Currency = "USD", Amount = 1 });

        //list payment request
        IEnumerable<PaymentRequestData> paymentRequests = await viewOnly.GetPaymentRequests(user.StoreId);
        ...
        //get payment request
        PaymentRequestData paymentRequest = await viewOnly.GetPaymentRequest(user.StoreId, newPaymentRequest.Id);
        Assert.Equal(newPaymentRequest.Title, paymentRequest.Title);
        Assert.Equal(newPaymentRequest.StoreId, user.StoreId);

        //update payment request
        UpdatePaymentRequestRequest updateRequest = JObject.FromObject(paymentRequest).ToObject<UpdatePaymentRequestRequest>();
        ...
        PaymentRequestData paymentTestPaymentRequest = await client.CreatePaymentRequest(user.StoreId,
        ...
        Invoice invoice = user.BitPay.GetInvoice(invoiceId);
    ...
    public async Task InvoiceLegacyTests()
    {
        using (ServerTester tester = CreateServerTester())
        {
            ...
            user.RegisterDerivationScheme("BTC");
            BTCPayServerClient client = await user.CreateClient(Policies.Unrestricted);
            Bitpay oldBitpay = user.BitPay;

            TestLogs.LogInformation("Let's create an invoice with bitpay API");
            Invoice oldInvoice = await oldBitpay.CreateInvoiceAsync(new Invoice()
            {
                Currency = "BTC",
            ...
            async Task<Client.Models.InvoiceData> AssertInvoiceMetadata()
            {
                ...
                Client.Models.InvoiceData newInvoice = await client.GetInvoice(user.StoreId, oldInvoice.Id);
            ...
            Data.ApplicationDbContextFactory db = tester.PayTester.GetService<Data.ApplicationDbContextFactory>();
            using Data.ApplicationDbContext ctx = db.CreateContext();
            Data.InvoiceData dbInvoice = await ctx.Invoices.FindAsync(oldInvoice.Id);
            dbInvoice.Blob = ZipUtils.Zip(invoiceV1);
            await ctx.SaveChangesAsync();
            Client.Models.InvoiceData newInvoice = await AssertInvoiceMetadata();
    ...
    public async Task CanOverpayInvoice()
    {
        ...
        TestAccount user = tester.NewAccount();
        await user.RegisterDerivationSchemeAsync("BTC");
        BTCPayServerClient client = await user.CreateClient();
        Client.Models.InvoiceData invoice = await client.CreateInvoice(user.StoreId, new CreateInvoiceRequest() { Amount = 5000.0m, Currency = "USD" });
        InvoicePaymentMethodDataModel[] methods = await client.GetInvoicePaymentMethods(user.StoreId, invoice.Id);
        InvoicePaymentMethodDataModel method = methods.First();
        var amount = method.Amount;
        Assert.Equal(amount, method.Due);
#pragma warning disable CS0618 // Type or member is obsolete
        Network btc = tester.NetworkProvider.BTC.NBitcoinNetwork;
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    public async Task InvoiceTests()
    {
        ...
        BTCPayServerClient client = await user.CreateClient(Policies.Unrestricted);
        BTCPayServerClient viewOnly = await user.CreateClient(Policies.CanViewInvoices);
        ...
        await user.RegisterDerivationSchemeAsync("BTC");
        Client.Models.InvoiceData newInvoice = await client.CreateInvoice(user.StoreId,
        ...
        //list 
        IEnumerable<Client.Models.InvoiceData> invoices = await viewOnly.GetInvoices(user.StoreId);
        ...
        //list Filtered
        IEnumerable<Client.Models.InvoiceData> invoicesFiltered = await viewOnly.GetInvoices(user.StoreId,
        ...
        //list Yesterday
        IEnumerable<Client.Models.InvoiceData> invoicesYesterday = await viewOnly.GetInvoices(user.StoreId,
        ...
        IEnumerable<Client.Models.InvoiceData> invoicesExistingOrderId =
        ...
        IEnumerable<Client.Models.InvoiceData> invoicesNonExistingOrderId =
        ...
        IEnumerable<Client.Models.InvoiceData> invoicesExistingStatus =
        ...
        IEnumerable<Client.Models.InvoiceData> invoicesNonExistingStatus = await viewOnly.GetInvoices(user.StoreId,
        ...
        //get
        Client.Models.InvoiceData invoice = await viewOnly.GetInvoice(user.StoreId, newInvoice.Id);
        Assert.Equal(newInvoice.Metadata, invoice.Metadata);
        InvoicePaymentMethodDataModel[] paymentMethods = await viewOnly.GetInvoicePaymentMethods(user.StoreId, newInvoice.Id);
        Assert.Single(paymentMethods);
        InvoicePaymentMethodDataModel paymentMethod = paymentMethods.First();
        ...
        foreach (InvoiceStatus marked in new[] { InvoiceStatus.Settled, InvoiceStatus.Invalid })
        {
            Client.Models.InvoiceData inv = await client.CreateInvoice(user.StoreId,
            new CreateInvoiceRequest() { Currency = "USD", Amount = 100 });
            ...
            Client.Models.InvoiceData result = await client.GetInvoice(user.StoreId, inv.Id);
            ...
            if (marked == InvoiceStatus.Invalid)
            {
                ...
                WebhookInvoiceInvalidEvent evt = user.AssertHasWebhookEvent<WebhookInvoiceInvalidEvent>(WebhookEventType.InvoiceInvalid,
                    o =>
                    {
                        Assert.Equal(inv.Id, o.InvoiceId);
                        Assert.True(o.ManuallyMarked);
        ...
        UIInvoiceController controller = tester.PayTester.GetController<UIInvoiceController>(user.UserId, user.StoreId);
        ...
        LanguageService langs = tester.PayTester.GetService<LanguageService>();
        ...
        Client.Models.InvoiceData invoiceWithdefaultPaymentMethodLN = await client.CreateInvoice(user.StoreId,
            new CreateInvoiceRequest()
            {
                Currency = "USD",
                Amount = 100,
        ...
        Client.Models.InvoiceData invoiceWithdefaultPaymentMethodOnChain = await client.CreateInvoice(user.StoreId,
            new CreateInvoiceRequest()
            {
                Currency = "USD",
                Amount = 100,
        ...
        InvoicePaymentMethodDataModel pm = Assert.Single(await client.GetInvoicePaymentMethods(user.StoreId, invoice.Id));
    ...
    public async Task CanUseLightningAPI()
    {
        ...
        TestAccount merchant = tester.NewAccount();
        merchant.GrantAccess(true);
        merchant.RegisterLightningNode("BTC", LightningConnectionType.LndREST);
        BTCPayServerClient merchantClient = await merchant.CreateClient($"{Policies.CanUseLightningNodeInStore}:{merchant.StoreId}");
        LightningInvoiceData merchantInvoice = await merchantClient.CreateLightningInvoice(merchant.StoreId, "BTC", new CreateLightningInvoiceRequest(LightMoney.Satoshis(1_000), "hey", TimeSpan.FromSeconds(60)));
        // The default client is using charge, so we should not be able to query channels
        BTCPayServerClient client = await user.CreateClient(Policies.CanUseInternalLightningNode);

        LightningNodeInformationData info = await client.GetLightningNodeInfo("BTC");
        ...
        LightningInvoiceData invoiceData = await client.CreateLightningInvoice("BTC", new CreateLightningInvoiceRequest()
        {
        ...
        LightningInvoiceData chargeInvoice = invoiceData;
        ...
        IEnumerable<LightningChannelData> data = await client.GetLightningNodeChannels(user.StoreId, "BTC");
        ...
        GreenfieldValidationException validationErr = await Assert.ThrowsAsync<GreenfieldValidationException>(async () => await client.CreateLightningInvoice(user.StoreId, "BTC", new CreateLightningInvoiceRequest()
        {
            Amount = -1,
        ...
        LightningInvoiceData invoice = await merchantClient.GetLightningInvoice(merchant.StoreId, "BTC", merchantInvoice.Id);
        ...
        TestAccount guest = tester.NewAccount();
    ...
    public async Task NotificationAPITests()
    {
        ...
        BTCPayServerClient client = await user.CreateClient(Policies.CanManageNotificationsForUser);
        BTCPayServerClient viewOnlyClient = await user.CreateClient(Policies.CanViewNotificationsForUser);
        ...
        NotificationData notification = (await client.GetNotifications()).First();
    ...
    public async Task OnChainPaymentMethodAPITests()
    {
        ...
        await tester.StartAsync();
        TestAccount user = tester.NewAccount();
        TestAccount user2 = tester.NewAccount();
        await user.GrantAccessAsync(true);
        await user2.GrantAccessAsync(false);

        BTCPayServerClient client = await user.CreateClient(Policies.CanModifyStoreSettings);
        BTCPayServerClient client2 = await user2.CreateClient(Policies.CanModifyStoreSettings);
        BTCPayServerClient viewOnlyClient = await user.CreateClient(Policies.CanViewStoreSettings);

        StoreData store = await client.CreateStore(new CreateStoreRequest() { Name = "test store" });
        ...
        ExtKey xpriv = new Mnemonic("all all all all all all all all all all all all").DeriveExtKey()
        ...
        OnChainPaymentMethodData method = await client.UpdateStoreOnChainPaymentMethod(store.Id, "BTC",
        ...
        await client.RemoveStoreOnChainPaymentMethod(store.Id, "BTC");
        OnChainPaymentMethodDataWithSensitiveData generateResponse = await client.GenerateOnChainWallet(store.Id, "BTC",
    ...
    public async Task LightningNetworkPaymentMethodAPITests()
    {
        ...
        await tester.StartAsync();
        await tester.EnsureChannelsSetup();
        TestAccount admin = tester.NewAccount();
        await admin.GrantAccessAsync(true);
        TestAccount admin2 = tester.NewAccount();
        await admin2.GrantAccessAsync(true);
        BTCPayServerClient adminClient = await admin.CreateClient(Policies.CanModifyStoreSettings);
        BTCPayServerClient admin2Client = await admin2.CreateClient(Policies.CanModifyStoreSettings, Policies.CanModifyServerSettings);
        BTCPayServerClient viewOnlyClient = await admin.CreateClient(Policies.CanViewStoreSettings);
        StoreData store = await adminClient.GetStore(admin.StoreId);
        ...
        LightningNetworkPaymentMethodData method = await adminClient.GetStoreLightningNetworkPaymentMethod(store.Id, "BTC");
        await AssertHttpError(403, async () =>
        {
            await viewOnlyClient.RemoveStoreOnChainPaymentMethod(store.Id, "BTC");
        });
        ...
        {
            GreenfieldValidationException ex = await AssertValidationError(new[] { "ConnectionString" }, async () =>
            {
        ...
        PoliciesSettings settings = (await tester.PayTester.GetService<SettingsRepository>().GetSettingAsync<PoliciesSettings>()) ?? new PoliciesSettings();
        ...
        TestAccount nonAdminUser = tester.NewAccount();
        await nonAdminUser.GrantAccessAsync(false);
        BTCPayServerClient nonAdminUserClient = await nonAdminUser.CreateClient(Policies.CanModifyStoreSettings);
        ...
        LightningNetworkPaymentMethodData data = await adminClient.UpdateStoreLightningNetworkPaymentMethod(nonAdminUser.StoreId, "BTC", new UpdateLightningNetworkPaymentMethodRequest()
        {
            Enabled = method.Enabled,
            ConnectionString = "Internal Node"
        });
    ...
    public async Task WalletAPITests()
    {
        ...
        BTCPayServerClient client = await user.CreateClient(Policies.CanModifyStoreSettings, Policies.CanModifyServerSettings);
        BTCPayServerClient viewOnlyClient = await user.CreateClient(Policies.CanViewStoreSettings);
        WalletId walletId = await user.RegisterDerivationSchemeAsync("BTC", ScriptPubKeyType.Segwit, true);
        ...
        OnChainWalletOverviewData overview = await client.ShowOnChainWalletOverview(walletId.StoreId, walletId.CryptoCode);
        Assert.Equal(0m, overview.Balance);


        OnChainWalletFeeRateData fee = await client.GetOnChainFeeRate(walletId.StoreId, walletId.CryptoCode);
        ...
        OnChainWalletAddressData address = await client.GetOnChainWalletReceiveAddress(walletId.StoreId, walletId.CryptoCode);
        OnChainWalletAddressData address2 = await client.GetOnChainWalletReceiveAddress(walletId.StoreId, walletId.CryptoCode);
        OnChainWalletAddressData address3 = await client.GetOnChainWalletReceiveAddress(walletId.StoreId, walletId.CryptoCode, true);
        ...
        OnChainWalletAddressData address4 = await client.GetOnChainWalletReceiveAddress(walletId.StoreId, walletId.CryptoCode, false);
        ...
        OnChainWalletAddressData address5 = await client.GetOnChainWalletReceiveAddress(walletId.StoreId, walletId.CryptoCode, true);
        Assert.Equal(address5.Address, address4.Address);


        OnChainWalletUTXOData utxo = Assert.Single(await client.GetOnChainWalletUTXOs(walletId.StoreId, walletId.CryptoCode));
        ...
        BitcoinAddress nodeAddress = await tester.ExplorerNode.GetNewAddressAsync();
        ...
        OnChainWalletTransactionData txdata =
            await client.CreateOnChainTransaction(walletId.StoreId, walletId.CryptoCode,
                createTxRequest);
    ...
    public async Task StorePaymentMethodsAPITests()
    {
        ...
        TestAccount admin = tester.NewAccount();
        await admin.GrantAccessAsync(true);
        BTCPayServerClient adminClient = await admin.CreateClient(Policies.Unrestricted);
        BTCPayServerClient viewerOnlyClient = await admin.CreateClient(Policies.CanViewStoreSettings);
        StoreData store = await adminClient.GetStore(admin.StoreId);
        ...
        {
            Assert.True(dictionary.TryGetValue(new PaymentMethodId("BTC", PaymentTypes.LightningLike).ToStringNormalized(), out GenericPaymentMethodData item));
            LightningNetworkPaymentMethodBaseData lightningNetworkPaymentMethodBaseData = Assert.IsType<JObject>(item.Data).ToObject<LightningNetworkPaymentMethodBaseData>();
            Assert.Equal("Internal Node", lightningNetworkPaymentMethodBaseData.ConnectionString);
        }

        Dictionary<string, GenericPaymentMethodData> methods = await adminClient.GetStorePaymentMethods(store.Id);
        ...
        {
            Assert.True(dictionary.TryGetValue(new PaymentMethodId("BTC", PaymentTypes.BTCLike).ToStringNormalized(), out GenericPaymentMethodData item));
            OnChainPaymentMethodBaseData paymentMethodBaseData = Assert.IsType<JObject>(item.Data).ToObject<OnChainPaymentMethodBaseData>();
        ...
        Assert.True(methods.TryGetValue(new PaymentMethodId("BTC", PaymentTypes.LightningLike).ToStringNormalized(), out GenericPaymentMethodData item));
        LightningNetworkPaymentMethodBaseData lightningNetworkPaymentMethodBaseData = Assert.IsType<JObject>(item.Data).ToObject<LightningNetworkPaymentMethodBaseData>();
    ...
    public async Task StoreUsersAPITest()
    {
        ...
        IEnumerable<StoreUserData> users = await client.GetStoreUsers(user.StoreId);
        StoreUserData storeuser = Assert.Single(users);
        ...
        BTCPayServerClient user2Client = await user2.CreateClient(Policies.CanModifyStoreSettings);
