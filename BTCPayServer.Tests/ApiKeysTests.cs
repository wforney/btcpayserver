...
public class ApiKeysTests : UnitTestBase
{
    ...
    public async Task CanCreateApiKeys()
    {
        ...
        IWebElement dropdown = s.Driver.FindElement(By.Name(getPermissionValueIndex));
        IWebElement option = dropdown.FindElement(By.TagName("option"));
        ...
        APIKeyRepository apiKeyRepo = s.Server.PayTester.GetService<APIKeyRepository>();
        ...
        foreach (IWebElement checkbox in s.Driver.FindElements(By.ClassName("form-check-input")))
        {
        ...
        ApiKeyData apikeydata = await TestApiAgainstAccessToken<ApiKeyData>(allAPIKey, "api/v1/api-keys/current", tester.PayTester.HttpClient);
        Assert.Equal(checkedPermissionCount, apikeydata.Permissions.Length);
    }

    private async Task TestApiAgainstAccessToken(string accessToken, ServerTester tester, TestAccount testAccount,
        params string[] expectedPermissionsArr)
    {
        Permission[] expectedPermissions = Permission.ToPermissions(expectedPermissionsArr).ToArray();
        expectedPermissions ??= new Permission[0];
        ApiKeyData apikeydata = await TestApiAgainstAccessToken<ApiKeyData>(accessToken, $"api/v1/api-keys/current", tester.PayTester.HttpClient);
        Permission[] permissions = apikeydata.Permissions;
        Assert.Equal(expectedPermissions.Length, permissions.Length);
        foreach (Permission expectPermission in expectedPermissions)
        {
        ...
        TestAccount secondUser = tester.NewAccount();
        ...
        System.Collections.Generic.IEnumerable<Permission> selectiveStorePermissions = permissions.Where(p => p.Scope != null && p.Policy == Policies.CanModifyStoreSettings);
        if (permissions.Contains(canModifyAllStores) || selectiveStorePermissions.Any())
        {
            StoreData[] resultStores =
                await TestApiAgainstAccessToken<StoreData[]>(accessToken, $"{TestApiPath}/me/stores",
                    tester.PayTester.HttpClient);

            foreach (Permission selectiveStorePermission in selectiveStorePermissions)
            {
