...
public class SeleniumTester : IDisposable
{
    ...
    public async Task StartAsync()
    {
        ...
        (int Width, int Height) windowSize = (Width: 1200, Height: 1000);
    ...
    internal IWebElement FindAlertMessage(params StatusMessageModel.StatusSeverity[] severity)
    {
        ...
        try
        {
            System.Collections.ObjectModel.ReadOnlyCollection<IWebElement> elements = Driver.FindElements(By.CssSelector(className));
    ...
    public void AddLightningNode(string cryptoCode = null, LightningConnectionType? connectionType = null, bool test = true)
    {
        ...
        IWebElement enabled = Driver.FindElement(By.Id($"{cryptoCode}LightningEnabled"));
    ...
    )
    {
        ...
        IWebElement currencyEl = Driver.FindElement(By.Id("Currency"));
        currencyEl.Clear();
        ...
        IWebElement statusElement = FindAlertMessage(expectedSeverity);
    ...
    public async Task<string> FundStoreWallet(WalletId walletId = null, int coins = 1, decimal denomination = 1m)
    {
        ...
        var addressStr = Driver.FindElement(By.Id("address")).GetDomProperty("value");
