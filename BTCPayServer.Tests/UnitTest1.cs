...
public class UnitTest1 : UnitTestBase
{
    ...
    private class DockerImage
    {
        ...
        public static DockerImage Parse(string str)
        {
            ...
            Match variableMatch = Regex.Match(str, @"\$\{[^-]+-([^\}]+)\}");
    ...
    public async Task EnsureSwaggerPermissionsDocumented()
    {
        ...
        IEnumerable<KeyValuePair<string, (string Title, string Description)>> storePolicies =
        ...
        IEnumerable<KeyValuePair<string, (string Title, string Description)>> serverPolicies =
            UIManageController.AddApiKeyViewModel.PermissionValueItem.PermissionDescriptions.Where(pair =>
                Policies.IsServerPolicy(pair.Key));
        IEnumerable<KeyValuePair<string, (string Title, string Description)>> otherPolicies =
            UIManageController.AddApiKeyViewModel.PermissionValueItem.PermissionDescriptions.Where(pair =>
    ...
    public async Task CanAcceptInvoiceWithTolerance2()
    {
        ...
        // Set tolerance to 50%
        UIStoresController stores = user.GetController<UIStoresController>();
        IActionResult response = stores.GeneralSettings();
        GeneralSettingsViewModel vm = Assert.IsType<GeneralSettingsViewModel>(Assert.IsType<ViewResult>(response).Model);
        Assert.Equal(0.0, vm.PaymentTolerance);
        vm.PaymentTolerance = 50.0;
        Assert.IsType<RedirectToActionResult>(stores.GeneralSettings(vm).Result);

        Invoice invoice = await user.BitPay.CreateInvoiceAsync(
            new Invoice()
            {
        ...
        {
            Invoice localInvoice = user.BitPay.GetInvoice(invoice.Id, Facade.Merchant);
    ...
    public async Task CanThrowBitpay404Error()
    {
        ...
        Invoice invoice = user.BitPay.CreateInvoice(
            new Invoice()
            {
                Buyer = new Buyer() { email = "test@fwf.com" },
                Price = 5000.0m,
        ...
        try
        {
            Invoice throwsBitpay404Error = user.BitPay.GetInvoice(invoice.Id + "123");
        }
        ...
        HttpResponseMessage result = await tester.PayTester.HttpClient.SendAsync(req);
    ...
    public async Task EnsureNewLightningInvoiceOnPartialPayment()
    {
        ...
        Invoice invoice = await user.BitPay.CreateInvoiceAsync(new Invoice(0.0001m, "BTC"));
        ...
        InvoiceDataChangedEvent evt = await tester.WaitForEvent<InvoiceDataChangedEvent>(async () =>
        {
            await tester.SendLightningPaymentAsync(newInvoice);
        }, evt => evt.InvoiceId == invoice.Id);

        InvoiceEntity fetchedInvoice = await tester.PayTester.InvoiceRepository.GetInvoice(evt.InvoiceId);
    ...
    public async Task CanSetLightningServer()
    {
        ...
        TestAccount user = tester.NewAccount();
        await user.GrantAccessAsync(true);
        UIStoresController storeController = user.GetController<UIStoresController>();
        IActionResult storeResponse = storeController.GeneralSettings();
        ...
        LightningSettingsViewModel storeVm =
    ...
    private async Task ProcessLightningPayment(LightningConnectionType type)
    {
    ...
    private async Task CanSendLightningPaymentCore(ServerTester tester, TestAccount user)
    {
        Invoice invoice = await user.BitPay.CreateInvoiceAsync(new Invoice()
        {
            Price = 0.01m,
            Currency = "USD",
            PosData = "posData",
        ...
        await TestUtils.EventuallyAsync(async () =>
        {
            Invoice localInvoice = await user.BitPay.GetInvoiceAsync(invoice.Id);
    ...
    public async Task CanUseServerInitiatedPairingCode()
    {
        ...
        UIStoresController controller = acc.GetController<UIStoresController>();
    ...
    public async Task CanSendIPN()
    {
        ...
        Invoice invoice = acc.BitPay.CreateInvoice(new Invoice
        {
            Price = 5.0m,
            Currency = "USD",
            PosData = "posData",
...
#pragma warning disable CS0618
        ...
        {
            JObject request = await callbackServer.GetNextRequest();
        ...
        Invoice invoice2 = acc.BitPay.GetInvoice(invoice.Id);
    ...
    public async Task CantPairTwiceWithSamePubkey()
    {
        ...
        acc.Register();
        acc.CreateStore();
        UIStoresController store = acc.GetController<UIStoresController>();
        PairingCode pairingCode = acc.BitPay.RequestClientAuthorization("test", Facade.Merchant);
        Assert.IsType<RedirectToActionResult>(store.Pair(pairingCode.ToString(), acc.StoreId).GetAwaiter()
            .GetResult());

        pairingCode = acc.BitPay.RequestClientAuthorization("test1", Facade.Merchant);
        acc.CreateStore();
        UIStoresController store2 = acc.GetController<UIStoresController>();
    ...
    public async Task CanUseTorClient()
    {
        using ServerTester tester = CreateServerTester();
        await tester.StartAsync();
        IHttpClientFactory httpFactory = tester.PayTester.GetService<IHttpClientFactory>();
        HttpClient client = httpFactory.CreateClient(PayjoinServerCommunicator.PayjoinOnionNamedClient);
        Assert.NotNull(client);

        TestLogs.LogInformation("Querying an clearnet site over tor");
        HttpResponseMessage response = await client.GetAsync("https://check.torproject.org/");
    ...
    public async Task CanRescanWallet()
    {
        ...
        DerivationStrategyBase btcDerivationScheme = acc.DerivationScheme;

        UIWalletsController walletController = acc.GetController<UIWalletsController>();

        var walletId = new WalletId(acc.StoreId, "BTC");
        acc.IsAdmin = true;
        walletController = acc.GetController<UIWalletsController>();

        RescanWalletModel rescan =
        ...
        // Sending a coin
        uint256 txId = tester.ExplorerNode.SendToAddress(
            btcDerivationScheme.GetDerivation(new KeyPath("0/90")).ScriptPubKey, Money.Coins(1.0m));
        tester.ExplorerNode.Generate(1);
        ListTransactionsViewModel transactions = Assert.IsType<ListTransactionsViewModel>(Assert
        ...
        transactions = Assert.IsType<ListTransactionsViewModel>(Assert
            .IsType<ViewResult>(walletController.WalletTransactions(walletId).Result).Model);
        ListTransactionsViewModel.TransactionViewModel tx = Assert.Single(transactions.Transactions);
        ...
        WalletBlobInfo walletInfo = await tester.PayTester.GetService<WalletRepository>().GetWalletInfo(walletId);
    ...
    public async Task CanListInvoices()
    {
        ...
        Invoice invoice = await acc.BitPay.CreateInvoiceAsync(
            new Invoice
            {
                Price = 500,
                Currency = "USD",
        ...
        NBitcoin.RPC.RPCClient cashCow = tester.ExplorerNode;
        var invoiceAddress = BitcoinAddress.Create(invoice.CryptoInfo[0].Address, cashCow.Network);
        Money firstPayment = invoice.CryptoInfo[0].TotalDue - Money.Satoshis(10);
        ...
        DateTimeOffset time = invoice.InvoiceTime;
    ...
    public async Task CanListNotifications()
    {
        ...
        UINotificationsController ctrl = acc.GetController<UINotificationsController>();
        IActionResult resp = await ctrl.Generate(newVersion);

        Models.NotificationViewModels.IndexViewModel vm = Assert.IsType<Models.NotificationViewModels.IndexViewModel>(
        ...
        NotificationViewModel fn = vm.Items.First();
    ...
    public async Task CanGetRates()
    {
        ...
        BitpayRateController rateController = acc.GetController<BitpayRateController>();
        DataWrapper<Rate[]> GetBaseCurrencyRatesResult = JObject.Parse(((JsonResult)rateController
        ...
        Rate rate = Assert.Single(GetBaseCurrencyRatesResult.Data);
        Assert.Equal("BTC", rate.Code);

        DataWrapper<Rate[]> GetRatesResult = JObject.Parse(((JsonResult)rateController.GetRates(null, default)
            .GetAwaiter().GetResult()).Value.ToJson()).ToObject<DataWrapper<Rate[]>>();
        ...
        DataWrapper<Rate> GetCurrencyPairRateResult = JObject.Parse(((JsonResult)rateController
        ...
        Rates rates = acc.BitPay.GetRates();
    ...
    public async Task CanRBFPayment()
    {
        ...
        Money payment1 = invoice.BtcDue + Money.Coins(0.0001m);
        Money payment2 = invoice.BtcDue;
        ...
        Transaction tx = tester.ExplorerNode.GetRawTransaction(new uint256(tx1));
        foreach (TxIn input in tx.Inputs)
        {
            input.ScriptSig = Script.Empty; //Strip signatures
        }

        TxOut output = tx.Outputs.First(o => o.Value == payment1);
        output.Value = payment2;
        output.ScriptPubKey = invoiceAddress.ScriptPubKey;

        using (var cts = new CancellationTokenSource(10000))
        using (WebsocketNotificationSession listener = tester.ExplorerClient.CreateWebsocketNotificationSession())
        {
            listener.ListenAllDerivationSchemes();
            Transaction replaced = tester.ExplorerNode.SignRawTransaction(tx);
            Thread.Sleep(1000); // Make sure the replacement has a different timestamp
            uint256 tx2 = tester.ExplorerNode.SendRawTransaction(replaced);
            TestLogs.LogInformation(
        ...
        Transaction invoice2Tx2 = invoice2Tx1.Clone();
        foreach (TxIn input in invoice2Tx2.Inputs)
        {
            input.ScriptSig = Script.Empty; //Strip signatures
        ...
        await TestUtils.EventuallyAsync(async () =>
        {
            InvoiceEntity i = await tester.PayTester.InvoiceRepository.GetInvoice(invoice2.Id);
        ...
        await TestUtils.EventuallyAsync(async () =>
            {
                InvoiceEntity invoiceEntity = await tester.PayTester.InvoiceRepository.GetInvoice(invoice.Id);
                BitcoinLikePaymentData[] btcPayments = invoiceEntity.GetAllBitcoinPaymentData(false).ToArray();
                PaymentEntity[] payments = invoiceEntity.GetPayments(false).ToArray();
    ...
    public async Task CanSaveKeyPathForOnChainPayments()
    {
        ...
        Invoice invoice = await user.BitPay.CreateInvoiceAsync(new Invoice(0.01m, "BTC"));
        await tester.WaitForEvent<InvoiceEvent>(async () =>
        {
            uint256 tx = await tester.ExplorerNode.SendToAddressAsync(
        ...
        List<PaymentEntity> payments = Assert.IsType<InvoiceDetailsModel>(
    ...
    public async void CheckCORSSetOnBitpayAPI()
    {
        using ServerTester tester = CreateServerTester();
        await tester.StartAsync();
        foreach (Task req in new[] { "invoices/", "invoices", "rates", "tokens" }.Select(async path =>
          {
              ...
              response.EnsureSuccessStatusCode();
              Assert.True(response.Headers.TryGetValues("Access-Control-Allow-Origin", out IEnumerable<string> val));
        ...
        Assert.True(response2.Headers.TryGetValues("Access-Control-Allow-Origin", out IEnumerable<string> val2));
    ...
    public async Task TestAccessBitpayAPI()
    {
        ...
        TokenRepository repo = tester.PayTester.GetService<TokenRepository>();
    ...
    public async Task CanUseAnyoneCanCreateInvoice()
    {
        ...
        HttpResponseMessage response = await tester.PayTester.HttpClient.SendAsync(
    ...
    public async Task CanTweakRate()
    {
        ...
        user.RegisterDerivationScheme("BTC");
        // First we try payment with a merchant having only BTC
        Invoice invoice1 = user.BitPay.CreateInvoice(
            new Invoice()
            {
                Price = 5000.0m,
                Currency = "USD",
        ...
        Invoice invoice2 = user.BitPay.CreateInvoice(
            new Invoice()
            {
                Price = 5000.0m,
                Currency = "USD",
    ...
    public async Task CanCreateTopupInvoices()
    {
        ...
        foreach (NetworkFeeMode networkFeeMode in Enum.GetValues(typeof(NetworkFeeMode)).Cast<NetworkFeeMode>())
        {
    ...
    private static async Task AssertTopUpBtcPrice(ServerTester tester, TestAccount user, Money btcSent, decimal expectedPriceWithoutNetworkFee, NetworkFeeMode networkFeeMode)
    {
        ...
        Client.Models.InvoiceData invoice = await client.CreateInvoice(user.StoreId, new CreateInvoiceRequest()
        {
            Amount = null,
            Currency = "USD"
        });
        ...
        InvoicePaymentMethodDataModel btcmethod = (await client.GetInvoicePaymentMethods(user.StoreId, invoice.Id))[0];
        Money paid = btcSent;
        ...
        await TestUtils.EventuallyAsync(async () =>
        {
            try
            {
                Invoice bitpayinvoice = await user.BitPay.GetInvoiceAsync(invoice.Id);
                ...
                IEnumerable<Client.Models.InvoiceData> invoices = await client.GetInvoices(user.StoreId, textSearch: $"{bitpayinvoice.Price.ToString(CultureInfo.InvariantCulture)}");
    ...
    public async Task CanModifyRates()
    {
        ...
        UIStoresController store = user.GetController<UIStoresController>();
        RatesViewModel rateVm = Assert.IsType<RatesViewModel>(Assert.IsType<ViewResult>(store.Rates()).Model);
    ...
    public async Task CanUseDefaultCurrency()
    {
        ...
        // with bitpay api
        Invoice invoice2 = await user.BitPay.CreateInvoiceAsync(new Invoice());
        Assert.Equal("EUR", invoice2.Currency);

        // via UI
        UIInvoiceController controller = user.GetController<UIInvoiceController>();
        IActionResult model = await controller.CreateInvoice();
        ...
        UIWalletsController walletController = user.GetController<UIWalletsController>();
        WalletSendModel walletSend = await walletController.WalletSend(new WalletId(user.StoreId, "BTC")).AssertViewModelAsync<WalletSendModel>();
    ...
    public async Task CanSetPaymentMethodLimits()
    {
        ...
        CheckoutAppearanceViewModel vm = Assert.IsType<CheckoutAppearanceViewModel>(Assert
        ...
        PaymentMethodCriteriaViewModel criteria = Assert.Single(vm.PaymentMethodCriteria.Where(m => m.PaymentMethod == btcMethod.ToString()));
        ...
        PaymentModel checkout = (await user.GetController<UIInvoiceController>().Checkout(invoice.Id)).AssertViewModel<PaymentModel>();
    ...
    public async Task CanSetUnifiedQrCode()
    {
        ...
        IActionResult res = await user.GetController<UIInvoiceController>().Checkout(invoice.Id);
        PaymentModel paymentMethodFirst = Assert.IsType<PaymentModel>(
        ...
        LightningSettingsViewModel vm = Assert.IsType<LightningSettingsViewModel>(Assert
        ...
        PaymentModel paymentMethodSecond = Assert.IsType<PaymentModel>(
            Assert.IsType<ViewResult>(res).Model
        );
    ...
    public async Task CanSetPaymentMethodLimitsLightning()
    {
        ...
        CheckoutAppearanceViewModel vm = user.GetController<UIStoresController>().CheckoutAppearance().AssertViewModel<CheckoutAppearanceViewModel>();
        PaymentMethodCriteriaViewModel criteria = Assert.Single(vm.PaymentMethodCriteria);
        ...
        user.RegisterLightningNode(cryptoCode, LightningConnectionType.Charge);
        LightningSettingsViewModel lnSettingsVm = await user.GetController<UIStoresController>().LightningSettings(user.StoreId, cryptoCode).AssertViewModelAsync<LightningSettingsViewModel>();
    ...
    public async Task PosDataParser_ParsesCorrectly_Slower()
    {
        ...
        UIInvoiceController controller = tester.PayTester.GetController<UIInvoiceController>(null);
        ...
        foreach ((string input, Dictionary<string, object> expectedOutput) valueTuple in testCases)
        {
            tasks.Add(user.BitPay.CreateInvoiceAsync(new Invoice(1, "BTC") { PosData = valueTuple.input })
                .ContinueWith(async task =>
                {
                    IActionResult result = await controller.Invoice(task.Result.Id);
                    InvoiceDetailsModel viewModel =
    ...
    public async Task CanExportInvoicesJson()
    {
        ...
        {
            Match match = Regex.Match(input, $"\"{fieldName}\":([^,]*)");
        ...
        Money networkFee = new FeeRate(invoice.MinerFees["BTC"].SatoshiPerBytes).GetFee(100);
        // ensure 0 invoices exported because there are no payments yet
        IActionResult jsonResult = user.GetController<UIInvoiceController>().Export("json").GetAwaiter().GetResult();
        ContentResult result = Assert.IsType<ContentResult>(jsonResult);
        ...
        var invoiceAddress = BitcoinAddress.Create(invoice.CryptoInfo[0].Address, cashCow.Network);
        //
        Money firstPayment = invoice.CryptoInfo[0].TotalDue - 3 * networkFee;
        ...
        TestUtils.Eventually(() =>
        {
            IActionResult jsonResultPaid =
                user.GetController<UIInvoiceController>().Export("json").GetAwaiter().GetResult();
            ContentResult paidresult = Assert.IsType<ContentResult>(jsonResultPaid);
    ...
    public async Task CanExportInvoicesCsv()
    {
        ...
        NBitcoin.RPC.RPCClient cashCow = tester.ExplorerNode;
        var invoiceAddress = BitcoinAddress.Create(invoice.CryptoInfo[0].Address, cashCow.Network);
        Money firstPayment = invoice.CryptoInfo[0].TotalDue - Money.Coins(0.001m);
        cashCow.SendToAddress(invoiceAddress, firstPayment);
        TestUtils.Eventually(() =>
        {
            IActionResult exportResultPaid =
                user.GetController<UIInvoiceController>().Export("csv").GetAwaiter().GetResult();
            ContentResult paidresult = Assert.IsType<ContentResult>(exportResultPaid);
    ...
    public async Task CanCreateAndDeleteApps()
    {
        ...
        UIAppsController apps = user.GetController<UIAppsController>();
        UIAppsController apps2 = user2.GetController<UIAppsController>();
        CreateAppViewModel vm = Assert.IsType<CreateAppViewModel>(Assert.IsType<ViewResult>(apps.CreateApp(user.StoreId)).Model);
        Assert.NotNull(vm.SelectedAppType);
        Assert.Null(vm.AppName);
        vm.AppName = "test";
        vm.SelectedAppType = AppType.PointOfSale.ToString();
        RedirectToActionResult redirectToAction = Assert.IsType<RedirectToActionResult>(apps.CreateApp(user.StoreId, vm).Result);
        Assert.Equal(nameof(apps.UpdatePointOfSale), redirectToAction.ActionName);
        ListAppsViewModel appList = Assert.IsType<ListAppsViewModel>(Assert.IsType<ViewResult>(apps.ListApps(user.StoreId).Result).Model);
        ListAppsViewModel appList2 =
            Assert.IsType<ListAppsViewModel>(Assert.IsType<ViewResult>(apps2.ListApps(user2.StoreId).Result).Model);
        ListAppsViewModel.ListAppViewModel app = appList.Apps[0];
    ...
    public async Task CanCreateStrangeInvoice()
    {
        ...
        BitPayException ex = Assert.Throws<BitPayException>(() => user.BitPay.CreateInvoice(
            new Invoice()
            {
                Price = 0.000000012m,
                Currency = "USD",
        ...
        Invoice invoice2 = user.BitPay.CreateInvoice(new Invoice() { Price = 0.000000019m, Currency = "USD" },
            Facade.Merchant);
        ...
        Invoice invoice3 = user.BitPay.CreateInvoice(
        ...
        Invoice invoice4 = user.BitPay.CreateInvoice(
        ...
        Invoice invoice5 = user.BitPay.CreateInvoice(
        ...
        BTCPayServerClient greenfield = await user.CreateClient();
        Client.Models.InvoiceData invoice5g = await greenfield.CreateInvoice(user.StoreId, new CreateInvoiceRequest()
        {
            Amount = 50.513m,
            Currency = "USD",
        ...
        Client.Models.InvoiceData zeroInvoice = await greenfield.CreateInvoice(user.StoreId, new CreateInvoiceRequest()
        {
            Amount = 0m,
            Currency = "USD"
        });
        ...
        InvoicePaymentMethodDataModel[] zeroInvoicePM = await greenfield.GetInvoicePaymentMethods(user.StoreId, zeroInvoice.Id);
    ...
    public async Task InvoiceFlowThroughDifferentStatesCorrectly()
    {
        ...
        InvoiceRepository repo = tester.PayTester.GetService<InvoiceRepository>();
        ApplicationDbContext ctx = tester.PayTester.GetService<ApplicationDbContextFactory>().CreateContext();
        ...
        TestUtils.Eventually(() =>
        {
            InvoiceEntity[] textSearchResult = tester.PayTester.InvoiceRepository.GetInvoices(new InvoiceQuery()
            {
        ...
        InvoiceEntity invoiceEntity = repo.GetInvoice(invoice.Id, true).GetAwaiter().GetResult();
        ...
        TestUtils.Eventually(() =>
        {
            ...
            HistoricalAddressInvoiceData historical1 =
            ...
            HistoricalAddressInvoiceData historical2 =
        ...
        uint256 txId = cashCow.SendToAddress(invoiceAddress, invoice.BtcDue + Money.Coins(1));
    ...
    public async Task CheckLogsRoute()
    {
        ...
        UIServerController serverController = user.GetController<UIServerController>();
        LogsViewModel vm = Assert.IsType<LogsViewModel>(
    ...
    public async Task CanLoginWithNoSecondaryAuthSystemsOrRequestItWhenAdded()
    {
        ...
        UIAccountController accountController = tester.PayTester.GetController<UIAccountController>();
        ...
        UIManageController listController = user.GetController<UIManageController>();
        UIFido2Controller manageController = user.GetController<UIFido2Controller>();
        ...
        StatusMessageModel statusModel = manageController.TempData.GetStatusMessageModel();
        Assert.Equal(StatusMessageModel.StatusSeverity.Error, statusModel.Severity);

        ApplicationDbContextFactory contextFactory = tester.PayTester.GetService<ApplicationDbContextFactory>();
        ...
        ViewResult secondLoginResult = Assert.IsType<ViewResult>(await accountController.Login(new LoginViewModel()
        {
            Email = user.RegisterDetails.Email,
            Password = user.RegisterDetails.Password
        }));
        ...
        SecondaryLoginViewModel vm = Assert.IsType<SecondaryLoginViewModel>(secondLoginResult.Model);
    ...
    public async void CheckOnionlocationForNonOnionHtmlRequests()
    {
        ...
        HttpResponseMessage htmlResponse = await tester.PayTester.HttpClient.SendAsync(htmlRequest);
        htmlResponse.EnsureSuccessStatusCode();
        Assert.True(htmlResponse.Headers.TryGetValues("Onion-Location", out IEnumerable<string> onionLocation));
        ...
        HttpResponseMessage otherResponse = await tester.PayTester.HttpClient.SendAsync(otherRequest);
    ...
    private class MockVersionFetcher : IVersionFetcher
    {
    ...
    public async Task CanCheckForNewVersion()
    {
        ...
        BTCPayServerEnvironment mockEnv = tester.PayTester.GetService<BTCPayServerEnvironment>();
        Services.Notifications.NotificationSender mockSender = tester.PayTester.GetService<Services.Notifications.NotificationSender>();
        ...
        NewVersionCheckerDataHolder lastVersion = await settings.GetSettingAsync<NewVersionCheckerDataHolder>();
    ...
    public async Task CanDoLightningInternalNodeMigration()
    {
        ...
        Data.StoreData store = await tester.PayTester.StoreRepository.FindStore(acc.StoreId);
        ...
        store = await tester.PayTester.StoreRepository.FindStore(acc.StoreId);
        LightningSupportedPaymentMethod lnMethod = store.GetSupportedPaymentMethods(tester.NetworkProvider).OfType<LightningSupportedPaymentMethod>().First();
        ...
        LightningConnectionString url = lnMethod.GetExternalLightningUrl();
    ...
    private static async Task RestartMigration(ServerTester tester)
    {
        ...
        IStartupTask migrationStartupTask = tester.PayTester.GetService<IServiceProvider>().GetServices<IStartupTask>()
    ...
    public async Task EmailSenderTests()
    {
        ...
        SettingsRepository settings = tester.PayTester.GetService<SettingsRepository>();
        EmailSenderFactory emailSenderFactory = tester.PayTester.GetService<EmailSenderFactory>();
    ...
    public async Task CanConfigureStorage()
    {
        ...
        TestAccount user = tester.NewAccount();
        user.GrantAccess();
        UIServerController controller = tester.PayTester.GetController<UIServerController>(user.UserId, user.StoreId);


        //Once we select a provider, redirect to its view
        RedirectToActionResult localResult = Assert
        ...
        RedirectToActionResult AmazonS3result = Assert
            .IsType<RedirectToActionResult>(controller.Storage(new StorageSettings()
            {
        ...
        RedirectToActionResult GoogleResult = Assert
            .IsType<RedirectToActionResult>(controller.Storage(new StorageSettings()
            {
        ...
        RedirectToActionResult AzureResult = Assert
            .IsType<RedirectToActionResult>(controller.Storage(new StorageSettings()
            {
        ...
        FileSystemStorageConfiguration fileSystemStorageConfiguration = Assert.IsType<FileSystemStorageConfiguration>(Assert
        ...
        RedirectToActionResult shouldBeRedirectingToLocalStorageConfigPage =
    ...
    internal static async Task CanUploadRemoveFiles(UIServerController controller)
    {
        ...
        List<IFormFile> fileList = new List<IFormFile>
            {
                TestUtils.GetFormFile("uploadtestfile1.txt", fileContent)
            };

        RedirectToActionResult uploadFormFileResult = Assert.IsType<RedirectToActionResult>(await controller.CreateFiles(fileList));
        ...
        RedirectToActionResult tmpLinkGenerate = Assert.IsType<RedirectToActionResult>(await controller.CreateTemporaryFileUrl(fileId,
        ...
        StatusMessageModel statusMessageModel = controller.TempData.GetStatusMessageModel();
