...
public class ThirdPartyTests : UnitTestBase
{
    ...
    public async Task CanUseAzureBlobStorage()
    {
        ...
        AzureBlobStorageConfiguration azureBlobStorageConfiguration = Assert.IsType<AzureBlobStorageConfiguration>(Assert
        ...
        RedirectToActionResult shouldBeRedirectingToAzureStorageConfigPage =
    ...
    {
        RateProviderFactory factory = FastTests.CreateBTCPayRateFactory();
        ...
        foreach ((string ExpectedName, Task<PairRate[]> ResultAsync, BackgroundFetcherRateProvider Fetcher) result in factory
            .Providers
...
retry:
    ...
    public void CanSolveTheDogesRatesOnKraken()
    {
        ...
        Assert.True(RateRules.TryParse("X_X=kraken(X_BTC) * kraken(BTC_X)", out RateRules rule));
        foreach (var pair in new[] { "DOGE_USD", "DOGE_CAD", "DASH_CAD", "DASH_USD", "DASH_EUR" })
        {
            RateResult result = fetcher.FetchRate(CurrencyPair.Parse(pair), rule, default).GetAwaiter().GetResult();
    ...
    public void CanGetRateCryptoCurrenciesByDefault()
    {
        ...
        RateRules rules = new StoreBlob().GetDefaultRateRules(provider);
        Dictionary<CurrencyPair, Task<RateResult>> result = fetcher.FetchRates(pairs, rules, default);
        foreach (KeyValuePair<CurrencyPair, Task<RateResult>> value in result)
        {
            RateResult rateResult = value.Value.GetAwaiter().GetResult();
    ...
    private string GetFileContent(params string[] path)
    {
        var l = path.ToList();
    ...
    public async Task CanExportBackgroundFetcherState()
    {
        ...
        BackgroundFetcherState state = provider.GetState();
        ...
        BackgroundFetcherState state2 = JsonConvert.DeserializeObject<BackgroundFetcherState>(str);
    ...
    public async Task CanUseExchangeSpecificRate()
    {
        ...
        List<decimal> rates = new List<decimal>
            {
                await CreateInvoice(tester, user, "coingecko")
            };
        ...
        string currency = "USD")
    {
        ...
        Invoice invoice2 = await user.BitPay.CreateInvoiceAsync(
            new Invoice()
            {
                Price = 5000.0m,
                Currency = currency,
