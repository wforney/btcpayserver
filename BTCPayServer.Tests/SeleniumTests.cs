...
public class ChromeTests : UnitTestBase
{
    ...
    public async Task CanUseCPFP()
    {
        ...
        s.Driver.FindElement(By.Id("ActionsDropdownToggle")).Click();
        s.Driver.FindElement(By.Id("BumpFee")).Click();
        IWebElement err = s.FindAlertMessage(StatusMessageModel.StatusSeverity.Error);
    ...
    public async Task CanUseLndSeedBackup()
    {
        ...
        IWebElement seedEl = s.Driver.FindElement(By.Id("Seed"));
        Assert.True(seedEl.Displayed);
        Assert.Contains("about over million", seedEl.Text, StringComparison.OrdinalIgnoreCase);
        IWebElement passEl = s.Driver.FindElement(By.Id("WalletPassword"));
    ...
    public async Task CanChangeUserMail()
    {
        ...
        ServerTester tester = s.Server;
        TestAccount u1 = tester.NewAccount();
        await u1.GrantAccessAsync();
        await u1.MakeAdmin(false);

        TestAccount u2 = tester.NewAccount();
        ...
        UserManager<ApplicationUser> manager = tester.PayTester.GetService<UserManager<ApplicationUser>>();
    ...
    public async Task CanUseSSHService()
    {
        using SeleniumTester s = CreateSeleniumTester();
        await s.StartAsync();
        SettingsRepository settings = s.Server.PayTester.GetService<SettingsRepository>();
        ...
        using (Renci.SshNet.SshClient client = await s.Server.PayTester.GetService<Configuration.BTCPayServerOptions>().SSHSettings
            .ConnectAsync())
        {
            SSH.SSHCommandResult result = await client.RunBash("echo hello");
    ...
    public async Task CanCreateStores()
    {
        ...
        s.AddLightningNode();
        s.Driver.AssertNoError();
        IWebElement successAlert = s.FindAlertMessage();
    ...
    public async Task CanUsePairing()
    {
        ...
        NBitpayClient.PairingCode code = await client.RequestClientAuthorizationAsync("hehe", NBitpayClient.Facade.Merchant);
    ...
    {
        using SeleniumTester s = CreateSeleniumTester(newDb: true);
        await s.StartAsync();
        ...
        ReadOnlyCollection<string> windows = s.Driver.WindowHandles;
    ...
    public async Task CanUseCoinSelection()
    {
        ...
        var addressStr = s.Driver.FindElement(By.Id("address")).GetDomProperty("value");
        ...
        uint256 targetTx = await s.Server.ExplorerNode.SendToAddressAsync(address, Money.Coins(1.2m));
        Transaction tx = await s.Server.ExplorerNode.GetRawTransactionAsync(targetTx);
        var spentOutpoint = new OutPoint(targetTx,
            tx.Outputs.FindIndex(txout => txout.Value == Money.Coins(1.2m)));
        await TestUtils.EventuallyAsync(async () =>
        {
            Data.StoreData store = await s.Server.PayTester.StoreRepository.FindStore(storeId);
            DerivationSchemeSettings x = store.GetSupportedPaymentMethods(s.Server.NetworkProvider)
                .OfType<DerivationSchemeSettings>()
                .Single(settings => settings.PaymentId.CryptoCode == walletId.CryptoCode);
            BTCPayWallet wallet = s.Server.PayTester.GetService<BTCPayWalletProvider>().GetWallet(walletId.CryptoCode);
        ...
        IWebElement el = s.Driver.FindElement(By.Id(spentOutpoint.ToString()));
        s.Driver.FindElement(By.Id(spentOutpoint.ToString())).Click();
        IWebElement inputSelectionSelect = s.Driver.FindElement(By.Name("SelectedInputs"));
        Assert.Single(inputSelectionSelect.FindElements(By.CssSelector("[selected]")));

        BitcoinPubKeyAddress bob = new Key().PubKey.Hash.GetAddress(Network.RegTest);
        SetTransactionOutput(s, 0, bob, 0.3m);
        s.Driver.FindElement(By.Id("SignTransaction")).Click();
        s.Driver.FindElement(By.CssSelector("button[value=broadcast]")).Click();
        IWebElement happyElement = s.FindAlertMessage();
    ...
    public async Task CanUseWebhooks()
    {
        ...
        ReadOnlyCollection<IWebElement> deletes = s.Driver.FindElements(By.LinkText("Delete"));
        ...
        s.AddDerivationScheme();
        s.CreateInvoice();
        Microsoft.AspNetCore.Http.HttpContext request = await server.GetNextRequest();
        Microsoft.AspNetCore.Http.IHeaderDictionary headers = request.Request.Headers;
        ...
        s.GoToStore(StoreNavPages.Webhooks);
        s.Driver.FindElement(By.LinkText("Modify")).Click();
        ReadOnlyCollection<IWebElement> elements = s.Driver.FindElements(By.ClassName("redeliver"));
        ...
        CanBrowseContent(s);
        IWebElement element = s.Driver.FindElement(By.ClassName("redeliver"));
        element.Click();
    ...
    public async Task CanManageWallet()
    {
        ...
        NBXplorer.WebsocketNotificationSession sess = await s.Server.ExplorerClient.CreateWebsocketNotificationSessionAsync();
        await sess.ListenAllTrackedSourceAsync();
        Task<NBXplorer.Models.NewEventBase> nextEvent = sess.NextEventAsync();
        ...
        InvoiceEntity invoice = await s.Server.PayTester.InvoiceRepository.GetInvoice(invoiceId);
        var address = invoice.EntityToDTO().Addresses["BTC"];

        //wallet should have been imported to bitcoin core wallet in watch only mode.
        NBitcoin.RPC.GetAddressInfoResponse result =
            await s.Server.ExplorerNode.GetAddressInfoAsync(BitcoinAddress.Create(address, Network.RegTest));
        Assert.True(result.IsWatchOnly);
        s.GoToStore(storeId);
        Mnemonic mnemonic = s.GenerateWallet(cryptoCode, "", true, true);

        //lets import and save private keys
        ExtKey root = mnemonic.DeriveExtKey();
        ...
        uint256 tx = s.Server.ExplorerNode.SendToAddress(BitcoinAddress.Create(address, Network.RegTest),
            Money.Coins(3.0m));
        await s.Server.ExplorerNode.GenerateAsync(1);
        ...
        BitcoinPubKeyAddress jack = new Key().PubKey.Hash.GetAddress(Network.RegTest);
    ...
    public async Task CanImportWallet()
    {
        ...
        s.CreateNewStore();
        const string cryptoCode = "BTC";
        Mnemonic mnemonic = s.GenerateWallet(cryptoCode, "click chunk owner kingdom faint steak safe evidence bicycle repeat bulb wheel");
    ...
    public async Task CanUsePullPaymentsViaUI()
    {
        ...
        s.Driver.FindElement(By.LinkText("View")).Click();
        BitcoinAddress address = await s.Server.ExplorerNode.GetNewAddressAsync();
        ...
        s.GoToStore(s.StoreId, StoreNavPages.PullPayments);
        ReadOnlyCollection<IWebElement> payouts = s.Driver.FindElements(By.ClassName("pp-payout"));
        ...
        await TestUtils.EventuallyAsync(async () =>
        {
            using ApplicationDbContext ctx = s.Server.PayTester.GetService<ApplicationDbContextFactory>().CreateContext();
            System.Collections.Generic.List<Data.PayoutData> payoutsData = await ctx.Payouts.Where(p => p.PullPaymentDataId == pullPaymentId).ToListAsync();
        ...
        s.FindAlertMessage();
        uint256 tx = await s.Server.ExplorerNode.SendToAddressAsync(address, Money.FromUnit(0.001m, MoneyUnit.BTC));
        ...
        LightningInvoice inv = await s.Server.MerchantLnd.Client.CreateInvoice(minimumReserve + payoutAmount, "Donation to merchant", TimeSpan.FromHours(1), default);
        PayResponse resp = await s.Server.CustomerLightningD.Pay(inv.BOLT11);
        ...
        ReadOnlyCollection<IWebElement> paymentMethodOptions = s.Driver.FindElements(By.CssSelector("input[name='PaymentMethods']"));
    ...
    public async Task CanUsePOSPrint()
    {
        ...
        s.Driver.FindElement(By.Id("ViewApp")).Click();
        ReadOnlyCollection<IWebElement> btns = s.Driver.FindElements(By.ClassName("lnurl"));
        foreach (IWebElement webElement in btns)
        {
            ...
            Uri parsed = LNURL.LNURL.Parse(lnurl, out _);
    ...
    public async Task CanUseLNURL()
    {
        ...
        s.RegisterNewUser(true);
        (_, string storeId) = s.CreateNewStore();
        Network network = s.Server.NetworkProvider.GetNetwork<BTCPayNetwork>(cryptoCode).NBitcoinNetwork;
        s.AddLightningNode(LightningConnectionType.CLightning, false);
        s.GoToLightningSettings();
        ...
        Uri parsed = LNURL.LNURL.Parse(lnurl, out var tag);
        LNURLPayRequest fetchedReuqest =
        ...
        LNURLPayRequest.LNURLPayRequestCallbackResponse lnurlResponse = await fetchedReuqest.SendRequest(new LightMoney(0.000001m, LightMoneyUnit.BTC),
        ...
        LNURLPayRequest.LNURLPayRequestCallbackResponse lnurlResponse2 = await fetchedReuqest.SendRequest(new LightMoney(0.000002m, LightMoneyUnit.BTC),
        ...
        await TestUtils.EventuallyAsync(async () =>
        {
            InvoiceEntity inv = await s.Server.PayTester.InvoiceRepository.GetInvoice(i);
            Assert.Equal(InvoiceStatusLegacy.Complete, inv.Status);
        });
        Client.BTCPayServerClient greenfield = await s.AsTestAccount().CreateClient();
        InvoicePaymentMethodDataModel[] paymentMethods = await greenfield.GetInvoicePaymentMethods(s.StoreId, i);
        ...
        await TestUtils.EventuallyAsync(async () =>
        {
            InvoiceEntity inv = await s.Server.PayTester.InvoiceRepository.GetInvoice(invForPP);
            Assert.Equal(InvoiceStatusLegacy.Complete, inv.Status);

            await using ApplicationDbContext ctx = s.Server.PayTester.GetService<ApplicationDbContextFactory>().CreateContext();
            System.Collections.Generic.List<Data.PayoutData> payoutsData = await ctx.Payouts.Where(p => p.PullPaymentDataId == pullPaymentId).ToListAsync();
            Assert.True(payoutsData.All(p => p.State == PayoutState.Completed));
        });
    ...
    public async Task CanUseLNAddress()
    {
        ...
        ReadOnlyCollection<IWebElement> addresses = s.Driver.FindElements(By.ClassName("lightning-address-value"));
...
retry:
    ...
    public async Task CanUseLNURLAuth()
    {
        ...
        System.Collections.Generic.IEnumerable<string> links = s.Driver.FindElements(By.CssSelector(".tab-content a")).Select(element => element.GetAttribute("href"));
        Assert.Equal(2, links.Count());
        Uri prevEndpoint = null;
        foreach (string link in links)
        {
            Uri endpoint = LNURL.LNURL.Parse(link, out var tag);
        ...
        LNAuthRequest request = Assert.IsType<LNAuthRequest>(await LNURL.LNURL.FetchInformation(prevEndpoint, null));
        _ = await request.SendChallenge(linkingKey, new HttpClient());
        TestUtils.Eventually(() => s.FindAlertMessage());

        s.Logout();
        s.LogIn(user, "123456");
        IWebElement section = s.Driver.FindElement(By.Id("lnurlauth-section"));
    ...
    {
        Match regex = Regex.Match(new Uri(s.Driver.Url, UriKind.Absolute).Query, "pairingCode=([^&]*)");
    ...
    private void SetTransactionOutput(SeleniumTester s, int index, BitcoinAddress dest, decimal amount, bool subtract = false)
    {
        ...
        IWebElement amountElement = s.Driver.FindElement(By.Id($"Outputs_{index}__Amount"));
        amountElement.Clear();
        amountElement.SendKeys(amount.ToString(CultureInfo.InvariantCulture));
        IWebElement checkboxElement = s.Driver.FindElement(By.Id($"Outputs_{index}__SubtractFeesFromOutput"));
