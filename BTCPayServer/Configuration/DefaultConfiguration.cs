...
public class DefaultConfiguration : StandardConfiguration.DefaultConfiguration
{
    protected override CommandLineApplication CreateCommandLineApplicationCore()
    {
        ...
        foreach (BTCPayNetwork network in provider.GetAll().OfType<BTCPayNetwork>())
        {
    ...
    public static ChainName GetNetworkType(IConfiguration conf)
    {
        ...
        ChainName net = conf.GetOrDefault<bool>("regtest", false) ? ChainName.Regtest :
    ...
    {
        ChainName networkType = GetNetworkType(conf);
        ...
        foreach (BTCPayNetwork n in new BTCPayNetworkProvider(networkType).GetAll().OfType<BTCPayNetwork>())
        {
