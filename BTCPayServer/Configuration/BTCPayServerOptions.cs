...
public class BTCPayServerOptions
{
    ...
    public void LoadArgs(IConfiguration conf, Logs Logs)
    {
        ...
        {
            if (!Utils.TryParseEndpoint(socksEndpointString, 9050, out EndPoint endpoint))
            {
        ...
        SSHSettings sshSettings = ParseSSHConfiguration(conf);
        ...
        if (!string.IsNullOrEmpty(fingerPrints))
        {
            ...
            {
                if (!SSHFingerprint.TryParse(fingerprint, out SSHFingerprint f))
                {
        ...
        Uri old = conf.GetOrDefault<Uri>("internallightningnode", null);
        if (old != null)
        {
        ...
        if (CheatMode && NetworkType == ChainName.Mainnet)
        {
    ...
    private SSHSettings ParseSSHConfiguration(IConfiguration conf)
    {
        ...
        {
            Server = conf.GetOrDefault<string>("sshconnection", null)
        };
