...
public class NBXplorerDashboard
{
    ...
    private readonly ConcurrentDictionary<string, NBXplorerSummary> _Summaries = new ConcurrentDictionary<string, NBXplorerSummary>();
    ...
    {
        _Summaries.TryGetValue(cryptoCode.ToUpperInvariant(), out NBXplorerSummary summary);
        return summary;
    }
...
{
    private readonly List<NBXplorerWaiter> _Waiters = new List<NBXplorerWaiter>();
    ...
    {
        foreach ((BTCPayNetwork, ExplorerClient) explorer in explorerClientProvider.GetAll())
        {
