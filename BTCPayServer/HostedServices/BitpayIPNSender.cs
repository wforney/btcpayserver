...
public class BitpayIPNSender : IHostedService
{
    ...
    private readonly MultiProcessingQueue _Queue = new MultiProcessingQueue();
    ...
    private async Task Notify(InvoiceEntity invoice, InvoiceEvent invoiceEvent, bool extendedNotification, bool sendMail)
    {
        ...
        Services.Invoices.InvoiceCryptoInfo btcCryptoInfo = dto.CryptoInfo.FirstOrDefault(c => c.GetpaymentMethodId() == new PaymentMethodId("BTC", Payments.PaymentTypes.BTCLike) && !string.IsNullOrEmpty(c.Address));
...
#pragma warning restore CS0618
    ...
    private readonly Dictionary<string, Task> _SendingRequestsByInvoiceId = new Dictionary<string, Task>();
    private readonly int MaxTry = 6;
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        leases.Add(_EventAggregator.SubscribeAsync<InvoiceEvent>(async e =>
        {
            ...
            InvoiceEntity invoice = await _InvoiceRepository.GetInvoice(e.Invoice.Id);
            if (invoice == null)
            {
                return;
            }
