...
{
    private class UpdateInvoiceContext
    {
    ...
    {
        InvoiceEntity invoice = context.Invoice;
        ...
        PaymentMethodDictionary allPaymentMethods = invoice.GetPaymentMethods();
        PaymentMethod paymentMethod = GetNearestClearedPayment(allPaymentMethods, out PaymentMethodAccounting accounting);
        ...
        if (invoice.Status == InvoiceStatusLegacy.Paid)
        {
            PaymentMethodAccounting confirmedAccounting =
        ...
        {
            PaymentMethodAccounting completedAccounting = paymentMethod?.Calculate(p => p.GetCryptoPaymentData().PaymentCompleted(p)) ??
                                      accounting;
    ...
    public static PaymentMethod GetNearestClearedPayment(PaymentMethodDictionary allPaymentMethods, out PaymentMethodAccounting accounting)
    {
        ...
        foreach (PaymentMethod paymentMethod in allPaymentMethods)
        {
            PaymentMethodAccounting currentAccounting = paymentMethod.Calculate();
    ...
    private async Task Wait(string invoiceId)
    {
        ...
        try
        {
            ...
            TimeSpan delay = invoice.ExpirationTime.AddSeconds(1) - DateTimeOffset.UtcNow;
    ...
    private readonly Channel<string> _WatchRequests = Channel.CreateUnbounded<string>();
    private Task _Loop;
    ...
    private async Task<bool> UpdateConfirmationCount(InvoiceEntity invoice)
    {
        ...
        Task<PaymentEntity>[] updateConfirmationCountIfNeeded = invoice
            .GetPayments(false)
            .Select<PaymentEntity, Task<PaymentEntity>>(async payment =>
            {
                CryptoPaymentData paymentData = payment.GetCryptoPaymentData();
                if (paymentData is Payments.Bitcoin.BitcoinLikePaymentData onChainPaymentData)
                {
                    ...
                    {
                        NBXplorer.Models.TransactionResult transactionResult = await _explorerClientProvider.GetExplorerClient(payment.GetCryptoCode())?.GetTransactionAsync(onChainPaymentData.Outpoint.Hash);
