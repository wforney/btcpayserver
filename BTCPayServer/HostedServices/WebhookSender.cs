...
public class WebhookSender : EventHostedServiceBase
{
    ...
    private class WebhookDeliveryRequest
    {
    ...
    private readonly MultiProcessingQueue _processingQueue = new MultiProcessingQueue();
    ...
    {
        using ApplicationDbContext? ctx = StoreRepository.CreateDbContext();
        ...
        Data.WebhookDeliveryData? newDelivery = NewDelivery(webhookDelivery.Webhook.Id);
        var newDeliveryBlob = new WebhookDeliveryBlob
        {
            Request = oldDeliveryBlob.Request
        };
    ...
    {
        WebhookInvoiceEvent? webhookEvent = GetWebhookEvent(webhookEventType);
    ...
    {
        Data.WebhookDeliveryData? delivery = NewDelivery(webhookId);
    ...
    protected override async Task ProcessEvent(object evt, CancellationToken cancellationToken)
    {
        ...
        {
            WebhookData[]? webhooks = await StoreRepository.GetWebhooks(invoiceEvent.Invoice.StoreId);
    ...
    private async Task<DeliveryResult> SendDelivery(WebhookDeliveryRequest ctx, CancellationToken cancellationToken)
    {
        ...
        WebhookDeliveryBlob? deliveryBlob = ctx.Delivery.Blob is null ? new WebhookDeliveryBlob() : ctx.Delivery.GetBlob();
