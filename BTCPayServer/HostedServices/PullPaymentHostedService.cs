...
public class PullPaymentHostedService : BaseAsyncService
{
    ...
    public async Task<string> CreatePullPayment(CreatePullPayment create)
    {
        ...
        {
            StartDate = create.StartsAt is DateTimeOffset date ? date : DateTimeOffset.UtcNow - TimeSpan.FromSeconds(1.0),
            EndDate = create.ExpiresAt is DateTimeOffset date2 ? new DateTimeOffset?(date2) : null,
            Period = create.Period is TimeSpan period ? (long?)period.TotalSeconds : null,
    ...
    public Task<RateResult> GetRate(PayoutData payout, string explicitRateRule, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                StoreBlob storeBlob = payout.PullPaymentData.StoreData.GetStoreBlob();
                Rating.RateRules rules = storeBlob.GetRateRules(_networkProvider);
                rules.Spread = 0.0m;
    ...
    private async Task HandleApproval(PayoutApproval req)
    {
        ...
        {
            using ApplicationDbContext ctx = _dbContextFactory.CreateContext();
            PayoutData payout = await ctx.Payouts.Include(p => p.PullPaymentData).Where(p => p.Id == req.PayoutId).FirstOrDefaultAsync();
            if (payout is null)
            {
            ...
            if (!PaymentMethodId.TryParse(payout.PaymentMethodId, out PaymentMethodId paymentMethod))
            {
                req.Completion.SetResult(PayoutApproval.Result.NotFound);
                return;
            }
            ...
            IPayoutHandler payoutHandler = _payoutHandlers.FindPayoutHandler(paymentMethod);
            if (payoutHandler is null)
            {
                throw new InvalidOperationException($"No payout handler for {paymentMethod}");
            }

            (IClaimDestination destination, string error) dest = await payoutHandler.ParseClaimDestination(paymentMethod, payoutBlob.Destination);
    ...
    private async Task HandleMarkPaid(InternalPayoutPaidRequest req)
    {
        ...
        {
            await using ApplicationDbContext ctx = _dbContextFactory.CreateContext();
            PayoutData payout = await ctx.Payouts.Include(p => p.PullPaymentData).Where(p => p.Id == req.Request.PayoutId).FirstOrDefaultAsync();
            if (payout is null)
            {
    ...
    private async Task HandleCreatePayout(PayoutRequest req)
    {
        try
        {
            ...
            await using ApplicationDbContext ctx = _dbContextFactory.CreateContext();
            Data.PullPaymentData pp = await ctx.PullPayments.FindAsync(req.ClaimRequest.PullPaymentId);
    ...
    private class InternalPayoutPaidRequest
    {
