...
public class PayJoinEndpointController : ControllerBase
{
    ...
    internal class UTXODeterministicComparer : IComparer<UTXO>
    {
        ...
        private static readonly UTXODeterministicComparer _Instance;
    ...
    public async Task<IActionResult> Submit(string cryptoCode,
        ...
        int v = 1)
    {
        BTCPayNetwork network = _btcPayNetworkProvider.GetNetwork<BTCPayNetwork>(cryptoCode);
        if (network == null)
        {
            return NotFound();
        }
        ...
        ExplorerClient explorer = _explorerClientProvider.GetExplorerClient(network);
        ...
        if (PSBT.TryParse(rawBody, network.NBitcoinNetwork, out PSBT psbt))
        {
        ...
        {
            psbtFormat = false;
            if (!Transaction.TryParse(rawBody, network.NBitcoinNetwork, out Transaction tx))
            {
        ...
        ScriptPubKeyType? sendersInputType = psbt.GetInputsScriptPubKeyType();
        ...
        BroadcastResult mempool = await explorer.BroadcastAsync(ctx.OriginalTransaction, true);
        if (!mempool.Success)
        {
            ctx.DoNotBroadcast();
            return BadRequest(CreatePayjoinError("original-psbt-rejected",
        ...
        foreach (PSBTOutput output in psbt.Outputs)
        {
            Tuple<WalletId, KeyPathInformation> walletReceiveMatch =
            ...
            {
                StoreData store = await _storeRepository.FindStore(walletReceiveMatch.Item1.StoreId);
            ...
            ScriptPubKeyType receiverInputsType = derivationSchemeSettings.AccountDerivation.ScriptPubKeyType();
            ...
            UTXO[] utxos = (await explorer.GetUTXOsAsync(derivationSchemeSettings.AccountDerivation))
            ...
            foreach (UTXO utxo in (await SelectUTXO(network, utxos, psbt.Inputs.Select(input => input.WitnessUtxo.Value.ToDecimal(MoneyUnit.BTC)), output.Value.ToDecimal(MoneyUnit.BTC),
        ...
        Money originalPaymentValue = originalPaymentOutput.Value;
        ...
        Transaction newTx = ctx.OriginalTransaction.Clone();
        TxOut ourNewOutput = newTx.Outputs[originalPaymentOutput.Index];
        ...
        foreach (UTXO selectedUTXO in selectedUTXOs.Select(o => o.Value))
        {
            contributedAmount += (Money)selectedUTXO.Value;
            TxIn newInput = newTx.Inputs.Add(selectedUTXO.Outpoint);
            newInput.Sequence = newTx.Inputs[(int)(RandomUtils.GetUInt32() % senderInputCount)].Sequence;
        }
        ourNewOutput.Value += contributedAmount;
        FeeRate minRelayTxFee = _dashboard.Get(network.CryptoCode).Status.BitcoinStatus?.MinRelayTxFee ??
                            new FeeRate(1.0m);

        // Remove old signatures as they are not valid anymore
        foreach (TxIn input in newTx.Inputs)
        {
        ...
        Coin[] coins = psbt.Inputs.Select(i => i.GetSignableCoin())
        ...
        if (additionalFee > Money.Zero)
        {
            ...
            if (additionalFee > Money.Zero)
            {
                ...
                Money newFeePaid = newTx.GetFee(txBuilder.FindSpentCoins(newTx));
        ...
        foreach (UTXO selectedUtxo in selectedUTXOs.Select(o => o.Value))
        {
            PSBTInput signedInput = newPsbt.Inputs.FindIndexedInput(selectedUtxo.Outpoint);
            Coin coin = selectedUtxo.AsCoin(derivationSchemeSettings.AccountDerivation);
            signedInput.UpdateFromCoin(coin);
            Key privateKey = accountKey.Derive(selectedUtxo.KeyPath).PrivateKey;
        ...
        if (invoice != null)
        {
            PaymentEntity payment = await _paymentService.AddPayment(invoice.Id, DateTimeOffset.UtcNow, originalPaymentData, network, true);
            if (payment is null)
            {
                return UnprocessableEntity(CreatePayjoinError("already-paid",
    ...
    private uint256 GetExpectedHash(PSBT psbt, Coin[] coins)
    {
        psbt = psbt.Clone();
        psbt.AddCoins(coins);
        if (!psbt.TryGetFinalizedHash(out uint256 hash))
        {
        ...
        var o = new JObject
            {
                ...
                new JProperty("message", friendlyMessage)
            };
        return o;
    }
    ...
    private JObject CreatePayjoinError(PayjoinReceiverWellknownErrors error, string debug)
    {
        var o = new JObject
            {
                new JProperty("errorCode", PayjoinReceiverHelper.GetErrorCode(error))
            };
        ...
        IEnumerable<decimal> otherOutputs)
    {
        ...
        foreach (UTXO availableUtxo in availableUtxos)
        {
            ...
            {
                IEnumerable<decimal> computedOutputs =
        ...
        foreach (UTXO utxo in availableUtxos.Where(u => !locked.Contains(u.Outpoint)))
        {
            if (currentTry >= maxTries)
            {
                break;
