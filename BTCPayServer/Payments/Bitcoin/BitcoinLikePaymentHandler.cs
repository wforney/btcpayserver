...
public class BitcoinLikePaymentHandler : PaymentMethodHandlerBase<DerivationSchemeSettings, BTCPayNetwork>
{
        ...
        StoreBlob storeBlob, IPaymentMethod paymentMethod)
    {
        ...
        {
            InvoiceCryptoInfo lightningInfo = invoiceResponse.CryptoInfo.FirstOrDefault(a =>
        ...
        BTCPayNetwork network, object preparePaymentObject)
    {
        ...
        KeyPathInformation reserved = await prepare.ReserveAddress;
        if (paymentMethod.ParentEntity.Type != InvoiceType.TopUp)
        {
            TxOut txOut = network.NBitcoinNetwork.Consensus.ConsensusFactory.CreateTxOut();
            txOut.ScriptPubKey = reserved.Address.ScriptPubKey;
            Money dust = txOut.GetDustThreshold();
            Money amount = paymentMethod.Calculate().Due;
            if (amount < dust)
            {
