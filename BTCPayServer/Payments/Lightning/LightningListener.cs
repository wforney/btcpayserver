...
public class LightningListener : IHostedService
{
    ...
    private Task _CheckingInvoice;
    private readonly Dictionary<(string, string), LightningInstanceListener> _InstanceListeners = new Dictionary<(string, string), LightningInstanceListener>();
    ...
    private async Task CheckingInvoice(CancellationToken cancellation)
    {
                    ...
                    _CheckInvoices.Reader.TryRead(out var invoiceId))
        {
            try
            {
                foreach (ListenedInvoice listenedInvoice in (await GetListenedInvoices(invoiceId)).Where(i => !i.IsExpired()))
                {
                    (string CryptoCode, string) instanceListenerKey = (listenedInvoice.Network.CryptoCode, GetLightningUrl(listenedInvoice.SupportedPaymentMethod).ToString());
                    if (!_InstanceListeners.TryGetValue(instanceListenerKey, out LightningInstanceListener instanceListener) ||
                        !instanceListener.IsListening)
                    {
                        ...
                        LightningInvoiceStatus? status = await instanceListener.PollPayment(listenedInvoice, cancellation);
                        if (status is null ||
                ...
                foreach (KeyValuePair<(string, string), LightningInstanceListener> kv in _InstanceListeners)
                {
                    kv.Value.RemoveExpiredInvoices();
                }
                foreach ((string, string) k in _InstanceListeners
    ...
    private Task<List<ListenedInvoice>> GetListenedInvoices(string invoiceId)
    {
        return _memoryCache.GetOrCreateAsync(GetCacheKey(invoiceId), async (cacheEntry) =>
       {
           ...
           InvoiceEntity invoice = await _InvoiceRepository.GetInvoice(invoiceId);
           foreach (PaymentMethod paymentMethod in invoice.GetPaymentMethods()
                                                         .Where(c => new[] { PaymentTypes.LightningLike, LNURLPayPaymentType.Instance }.Contains(c.GetId().PaymentType)))
           {
               ...
               BTCPayNetwork network = _NetworkProvider.GetNetwork<BTCPayNetwork>(paymentMethod.GetId().CryptoCode);
           ...
           TimeSpan expiredIn = DateTimeOffset.UtcNow - invoice.ExpirationTime;
    ...
    private readonly ConcurrentDictionary<string, LightningInstanceListener> _ListeningInstances = new ConcurrentDictionary<string, LightningInstanceListener>();
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        leases.Add(_Aggregator.SubscribeAsync<Events.InvoiceEvent>(async inv =>
        {
            ...
            {
                PaymentMethod pm = inv.Invoice.GetPaymentMethods().First();
        ...
        leases.Add(_Aggregator.SubscribeAsync<Events.InvoiceDataChangedEvent>(async inv =>
        {
            ...
            {
                InvoiceEntity invoice = await _InvoiceRepository.GetInvoice(inv.InvoiceId);
    ...
    {
        PaymentMethod[] paymentMethods = invoice.GetPaymentMethods()
            .Where(method => new[] { PaymentTypes.LightningLike, LNURLPayPaymentType.Instance }.Contains(method.GetId().PaymentType))
            .ToArray();
        Data.StoreData store = await _storeRepository.FindStore(invoice.StoreId);
            ...
            foreach (PaymentMethod paymentMethod in paymentMethods)
            {
                try
                {
                    ...
                    if (oldDetails is LNURLPayPaymentMethodDetails lnurlPayPaymentMethodDetails && !string.IsNullOrEmpty(lnurlPayPaymentMethodDetails.BOLT11))
                    {
                        try
                        {
                            ILightningClient client = _lightningLikePaymentHandler.CreateLightningClient(lnurlPayPaymentMethodDetails.LightningSupportedPaymentMethod,
                    ...
                    try
                    {
                        ILightningClient client = _lightningLikePaymentHandler.CreateLightningClient(supportedMethod,
                            (BTCPayNetwork)paymentMethod.Network);
                        await client.CancelInvoice(oldDetails.InvoiceId);
                    }
                    catch
                    ...
                    (string CryptoCode, string) instanceListenerKey = (paymentMethod.Network.CryptoCode,
    ...
    {
        LightningConnectionString url = supportedMethod.GetExternalLightningUrl();
        if (url != null)
        {
            return url;
        }

        if (Options.Value.InternalLightningByCryptoCode.TryGetValue(supportedMethod.CryptoCode, out LightningConnectionString conn))
        {
            return conn;
        }
    ...
    private TimeSpan _PollInterval = TimeSpan.FromMinutes(1.0);
...
public class LightningInstanceListener
{
    ...
    {
        ILightningClient client = _lightningClientFactory.Create(ConnectionString, _network);
    ...
    private async Task Listen(CancellationToken cancellation)
    {
        ...
        try
        {
            ILightningClient lightningClient = _lightningClientFactory.Create(ConnectionString, _network);
            using ILightningInvoiceListener session = await lightningClient.Listen(cancellation);
            ...
            {
                LightningInvoice notification = await session.WaitInvoice(cancellation);
                if (!_ListenedInvoices.TryGetValue(notification.Id, out ListenedInvoice listenedInvoice))
                {
                    continue;
                }
                     ...
                     listenedInvoice.PaymentMethodDetails.GetPaymentHash(_network.NBitcoinNetwork)))
                {
                        ...
                        notification.PaidAt.HasValue && notification.Amount != null)
                    {
                        ...
                        _ListenedInvoices.TryRemove(notification.Id, out ListenedInvoice _);
                    }
    ...
    {
        foreach (ListenedInvoice invoice in _ListenedInvoices.Values)
        {
            LightningInvoiceStatus? status = await PollPayment(invoice, cancellation);
            if (status is null ||
                    status is LightningInvoiceStatus.Paid ||
                    status is LightningInvoiceStatus.Expired)
            {
                _ListenedInvoices.TryRemove(invoice.PaymentMethodDetails.InvoiceId, out ListenedInvoice _);
            }
        }
    ...
    private bool _ErrorAlreadyLogged = false;
    private readonly ConcurrentDictionary<string, ListenedInvoice> _ListenedInvoices = new ConcurrentDictionary<string, ListenedInvoice>();

    public async Task<bool> AddPayment(LightningInvoice notification, string invoiceId, PaymentType paymentType)
    {
        PaymentEntity payment = await _paymentService.AddPayment(invoiceId, notification.PaidAt.Value, new LightningLikePaymentData()
        {
    ...
    {
        foreach (KeyValuePair<string, ListenedInvoice> invoice in _ListenedInvoices)
        {
            if (invoice.Value.IsExpired())
            {
                _ListenedInvoices.TryRemove(invoice.Key, out ListenedInvoice _);
            }
        }
        if (_ListenedInvoices.IsEmpty)
        {
...
internal class ListenedInvoice
{
