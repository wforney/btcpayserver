...
public class LightningLikePaymentHandler : PaymentMethodHandlerBase<LightningSupportedPaymentMethod, BTCPayNetwork>
{
        ...
        BTCPayNetwork network, object preparePaymentObject)
    {
        ...
        Task<NodeInfo[]>? nodeInfo = GetNodeInfo(supportedPaymentMethod, network, logs, paymentMethod.PreferOnion);

        InvoiceEntity? invoice = paymentMethod.ParentEntity;
        ...
        ILightningClient? client = supportedPaymentMethod.CreateLightningClient(network, Options.Value, _lightningClientFactory);
        TimeSpan expiry = invoice.ExpirationTime - DateTimeOffset.UtcNow;
    ...
    {
        if (!_Dashboard.IsFullySynched(network.CryptoCode, out NBXplorerDashboard.NBXplorerSummary? summary))
        {
        ...
        {
            using var cts = new CancellationTokenSource(LIGHTNING_TIMEOUT);
            ILightningClient? client = CreateLightningClient(supportedPaymentMethod, network);
            ...
            NodeInfo[]? nodeInfo = preferOnion != null && info.NodeInfoList.Any(i => i.IsTor == preferOnion)
    ...
    public ILightningClient CreateLightningClient(LightningSupportedPaymentMethod supportedPaymentMethod, BTCPayNetwork network)
    {
        ...
        {
            if (!Options.Value.InternalLightningByCryptoCode.TryGetValue(network.CryptoCode, out LightningConnectionString? connectionString))
            {
    ...
    public async Task TestConnection(NodeInfo nodeInfo, CancellationToken cancellation)
    {
        try
        {
            if (!Utils.TryParseEndpoint(nodeInfo.Host, nodeInfo.Port, out System.Net.EndPoint? endpoint))
            {
                throw new PaymentMethodUnavailableException($"Could not parse the endpoint {nodeInfo.Host}");
            }

            using System.Net.Sockets.Socket? tcp = await _socketFactory.ConnectAsync(endpoint, cancellation);
        }
        catch (Exception ex)
        {
    ...
    {
        PaymentMethodId? paymentMethodId = paymentMethod.GetId();

        InvoiceCryptoInfo? cryptoInfo = invoiceResponse.CryptoInfo.First(o => o.GetpaymentMethodId() == paymentMethodId);
        BTCPayNetwork? network = _networkProvider.GetNetwork<BTCPayNetwork>(model.CryptoCode);
