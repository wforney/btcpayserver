...
public static class BTCPayServerServices
{
    ...
    public static IServiceCollection AddBTCPayServer(this IServiceCollection services, IConfiguration configuration, Logs logs)
    {
        ...
        services.AddOptions<LightningNetworkOptions>().Configure<BTCPayNetworkProvider>(
            (options, btcPayNetworkProvider) =>
            {
                foreach (BTCPayNetwork net in btcPayNetworkProvider.GetAll().OfType<BTCPayNetwork>())
                {
                    var lightning = configuration.GetOrDefault<string>($"{net.CryptoCode}.lightning", string.Empty);
                    if (lightning.Length != 0)
                    {
                        if (!LightningConnectionString.TryParse(lightning, true, out LightningConnectionString connectionString,
                            out var error))
                        {
        ...
        services.AddOptions<ExternalServicesOptions>().Configure<BTCPayNetworkProvider>(
            (options, btcPayNetworkProvider) =>
            {
                ...
                if (services != null)
                {
                    foreach ((string Name, string Link) service in services.Split(new[] { ';', ',' }, StringSplitOptions.RemoveEmptyEntries)
                    ...
                    {
                        if (Uri.TryCreate(service.Link, UriKind.RelativeOrAbsolute, out Uri uri))
                        {
        ...
        services.AddSingleton<IUIExtension>(new UIExtension("LNURL/CrowdfundLNURL",
"crowdfund-head"));
        ...
        {
            BTCPayServerOptions opts = provider.GetRequiredService<BTCPayServerOptions>();
            var bundle = new BundleOptions
            {
                UseBundles = opts.BundleJsCss,
                AppendVersion = true
            };
            return bundle;
        });
        ...
        {
            BTCPayServerEnvironment btcPayEnv = provider.GetService<BTCPayServerEnvironment>();
