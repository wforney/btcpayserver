...
public class MigrationStartupTask : IStartupTask
{
    ...
    public async Task ExecuteAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            MigrationSettings settings = (await _Settings.GetSettingAsync<MigrationSettings>()) ?? new MigrationSettings();
            ...
            {
                ThemeSettings themeSettings = await _Settings.GetSettingAsync<ThemeSettings>() ?? new ThemeSettings();
                IList<ApplicationUser> admin = await _userManager.GetUsersInRoleAsync(Roles.ServerAdmin);
    ...
    {
        UILNURLController.LightningAddressSettings old = await _Settings.GetSettingAsync<UILNURLController.LightningAddressSettings>("BTCPayServer.LNURLController+LightningAddressSettings");
        if (old is not null)
        {
    ...
    private async Task AddInitialUserBlob()
    {
        ...
        foreach (ApplicationUser user in await ctx.Users.AsQueryable().ToArrayAsync())
        {
    ...
    {
        await using ApplicationDbContext ctx = _DBContextFactory.CreateContext();
        foreach (Data.PayoutData payoutData in await ctx.Payouts.AsQueryable().ToArrayAsync())
        {
            PaymentMethodId pmi = payoutData.GetPaymentMethodId();
            if (pmi is null)
            {
                continue;
            }
            IPayoutHandler handler = _payoutHandlers
                .FindPayoutHandler(pmi);
            if (handler is null)
            {
                continue;
            }
            (IClaimDestination destination, string error) claim = await handler?.ParseClaimDestination(pmi, payoutData.GetBlob(_btcPayNetworkJsonSerializerSettings).Destination);
    ...
    {
        await using ApplicationDbContext ctx = _DBContextFactory.CreateContext();
        foreach (AppData app in await ctx.Apps.Include(data => data.StoreData).AsQueryable().ToArrayAsync())
        {
            ...
            switch (app.AppType)
            {
                case nameof(AppType.Crowdfund):
                    CrowdfundSettings settings1 = app.GetSettings<CrowdfundSettings>();
                ...
                case nameof(AppType.PointOfSale):
                    ...
                    UIAppsController.PointOfSaleSettings settings2 = app.GetSettings<UIAppsController.PointOfSaleSettings>();
    ...
    {
        await using ApplicationDbContext ctx = _DBContextFactory.CreateContext();
        foreach (Data.StoreData store in await ctx.Stores.AsQueryable().ToArrayAsync())
        {
            foreach (DerivationSchemeSettings paymentMethod in store.GetSupportedPaymentMethods(_NetworkProvider).OfType<DerivationSchemeSettings>())
            {
    ...
    {
        await using ApplicationDbContext ctx = _DBContextFactory.CreateContext();
        List<U2FDevice> u2fDevices = await ctx.U2FDevices.ToListAsync();
...
#pragma warning disable CS0618 // Type or member is obsolete
            ...
            bool updated = false;
            foreach (JProperty prop in strats.Properties().Where(p => p.Name.EndsWith("LightningLike", StringComparison.OrdinalIgnoreCase)))
            {
                var method = ((JObject)prop.Value);
                JProperty lightningCharge = method.Property("LightningChargeUrl", StringComparison.OrdinalIgnoreCase);
                JProperty ln = method.Property("LightningConnectionString", StringComparison.OrdinalIgnoreCase);
                ...
                JProperty paymentId = method.Property("PaymentId", StringComparison.OrdinalIgnoreCase);
                if (paymentId != null)
                {
                    paymentId.Remove();
                    updated = true;
...
#pragma warning disable CS0618 // Type or member is obsolete
            ...
            if (blob.AdditionalData.TryGetValue("walletKeyPathRoots", out JToken walletKeyPathRootsJToken))
            {
                Dictionary<string, string> walletKeyPathRoots = walletKeyPathRootsJToken.ToObject<Dictionary<string, string>>();
                ...
                foreach (DerivationSchemeSettings scheme in store.GetSupportedPaymentMethods(_NetworkProvider)
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    {
        using ApplicationDbContext ctx = _DBContextFactory.CreateContext();
        foreach (AppData app in await ctx.Apps.Where(a => a.AppType == "Crowdfund").ToArrayAsync())
        {
            CrowdfundSettings settings = app.GetSettings<Services.Apps.CrowdfundSettings>();
#pragma warning disable CS0618 // Type or member is obsolete
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    private async Task MigratePaymentMethodCriteria()
    {
        ...
        foreach (Data.StoreData store in await ctx.Stores.AsQueryable().ToArrayAsync())
        {
            ...
            if (blob.AdditionalData.TryGetValue("onChainMinValue", out JToken onChainMinValueJToken))
            {
                CurrencyValue.TryParse(onChainMinValueJToken.Value<string>(), out onChainMinValue);
                blob.AdditionalData.Remove("onChainMinValue");
            }
            if (blob.AdditionalData.TryGetValue("lightningMaxValue", out JToken lightningMaxValueJToken))
            {
           ...
           {
               PaymentMethodCriteria matchedFromBlob =
    ...
    private async Task ConvertNetworkFeeProperty()
    {
        ...
        {
            StoreBlob blob = store.GetStoreBlob();
            if (blob.AdditionalData.TryGetValue("networkFeeDisabled", out JToken networkFeeModeJToken))
            {
    ...
    private async Task ConvertMultiplierToSpread()
    {
        ...
        foreach (Data.StoreData store in await ctx.Stores.AsQueryable().ToArrayAsync())
        {
            ...
            if (blob.AdditionalData.TryGetValue("rateRules", out JToken rateRulesJToken))
            {
                List<RateRule_Obsolete> rateRules = new Serializer(null).ToObject<List<RateRule_Obsolete>>(rateRulesJToken.ToString());
                if (rateRules != null && rateRules.Count != 0)
                {
                    foreach (RateRule_Obsolete rule in rateRules)
                    {
    ...
    private async Task DeprecatedLightningConnectionStringCheck()
    {
        ...
        foreach (Data.StoreData store in await ctx.Stores.AsQueryable().ToArrayAsync())
        {
            foreach (LightningSupportedPaymentMethod method in store.GetSupportedPaymentMethods(_NetworkProvider).OfType<Payments.Lightning.LightningSupportedPaymentMethod>())
            {
                Lightning.LightningConnectionString lightning = method.GetExternalLightningUrl();
