...
public class BTCPayMiddleware
{
    ...
    public async Task Invoke(HttpContext httpContext)
    {
        ...
        try
        {
            (string Signature, string Id, string Authorization) bitpayAuth = GetBitpayAuth(httpContext, out bool isBitpayAuth);
            ...
            var isHtml = httpContext.Request.Headers.TryGetValue("Accept", out StringValues accept)
                        && accept.ToString().StartsWith("text/html", StringComparison.OrdinalIgnoreCase);
            var isModal = httpContext.Request.Query.TryGetValue("view", out StringValues view)
