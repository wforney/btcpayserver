...
public class PaymentRequestStreamer : EventHostedServiceBase
{
    ...
    private async Task CheckingPendingPayments(CancellationToken cancellationToken)
    {
        ...
        (int total, Data.PaymentRequestData[] items) = await _PaymentRequestRepository.FindPaymentRequests(new PaymentRequestQuery()
        {
    ...
    private Task _CheckingPendingPayments;
    ...
    protected override async Task ProcessEvent(object evt, CancellationToken cancellationToken)
    {
        if (evt is InvoiceEvent invoiceEvent)
        {
            foreach (var paymentId in PaymentRequestRepository.GetPaymentIdsFromInternalTags(invoiceEvent.Invoice))
            {
                if (invoiceEvent.Name == InvoiceEvent.ReceivedPayment || invoiceEvent.Name == InvoiceEvent.MarkedCompleted || invoiceEvent.Name == InvoiceEvent.MarkedInvalid)
                {
                    ...
                    Services.Invoices.CryptoPaymentData data = invoiceEvent.Payment?.GetCryptoPaymentData();
                    if (data != null)
                    {
        ...
        else if (evt is PaymentRequestUpdated updated)
        {
            ...
            DateTimeOffset? expiry = updated.Data.GetBlob().ExpiryDate;
    ...
    {
        Models.PaymentRequestViewModels.ViewPaymentRequestViewModel req = await _PaymentRequestService.GetPaymentRequest(paymentRequestId);
        if (req != null)
        {
