...
{
    private readonly ConcurrentDictionary<ulong, (DateTimeOffset expiration, LightningConfigurations config)> _Map = new ConcurrentDictionary<ulong, (DateTimeOffset expiration, LightningConfigurations config)>();
    ...
    {
        CleanExpired();
        if (!_Map.TryGetValue(secret, out (DateTimeOffset expiration, LightningConfigurations config) value))
        {
            return null;
        }
    ...
    {
        foreach (KeyValuePair<ulong, (DateTimeOffset expiration, LightningConfigurations config)> item in _Map)
        {
            if (item.Value.expiration < DateTimeOffset.UtcNow)
            {
                _Map.TryRemove(item.Key, out (DateTimeOffset expiration, LightningConfigurations config) unused);
            }
        }
    }
}
