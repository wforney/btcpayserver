...
public class WalletRepository
{
    ...
    public async Task SetWalletInfo(WalletId walletId, WalletBlobInfo blob)
    {
        ...
        Microsoft.EntityFrameworkCore.ChangeTracking.EntityEntry<WalletData> entity = await ctx.Wallets.AddAsync(walletData);
    ...
    public async Task<WalletBlobInfo> GetWalletInfo(WalletId walletId)
    {
        ArgumentNullException.ThrowIfNull(walletId);
        using ApplicationDbContext ctx = _ContextFactory.CreateContext();
        WalletData data = await ctx.Wallets
    ...
    public async Task SetWalletTransactionInfo(WalletId walletId, string transactionId, WalletTransactionInfo walletTransactionInfo)
    {
        ...
        Microsoft.EntityFrameworkCore.ChangeTracking.EntityEntry<WalletTransactionData> entity = await ctx.WalletTransactions.AddAsync(walletData);
        entity.State = EntityState.Modified;
        try
        {
            await ctx.SaveChangesAsync();
