...
public class BTCPayWallet
{
    ...
    public async Task<KeyPathInformation> ReserveAddressAsync(DerivationStrategyBase derivationStrategy)
    {
        ...
        KeyPathInformation pathInfo = await _Client.GetUnusedAsync(derivationStrategy, DerivationFeature.Deposit, 0, true).ConfigureAwait(false);
    ...
    {
        ArgumentNullException.ThrowIfNull(derivationStrategy);
        KeyPathInformation pathInfo = await _Client.GetUnusedAsync(derivationStrategy, DerivationFeature.Change, 0, false).ConfigureAwait(false);
        // Might happen on some broken install
        if (pathInfo == null)
        {
            await _Client.TrackAsync(derivationStrategy).ConfigureAwait(false);
    ...
    public async Task<TransactionResult> GetTransactionAsync(uint256 txId, bool includeOffchain = false, CancellationToken cancellation = default(CancellationToken))
    {
        ...
        {
            Transaction offchainTx = await GetOffchainTransactionAsync(txId);
            if (offchainTx != null)
            {
    ...
    public async Task<Transaction> GetOffchainTransactionAsync(uint256 txid)
    {
        ...
        OffchainTransactionData txData = await ctx.OffchainTransactions.FindAsync(txid.ToString());
        if (txData is null)
        {
            return null;
        }

        return Transaction.Load(txData.Blob, _Network.NBitcoinNetwork);
    }
    ...
    private readonly ConcurrentDictionary<string, TaskCompletionSource<UTXOChanges>> _FetchingUTXOs = new ConcurrentDictionary<string, TaskCompletionSource<UTXOChanges>>();
    ...
    private async Task<UTXOChanges> GetUTXOChanges(DerivationStrategyBase strategy, CancellationToken cancellation)
    {
        ...
        TaskCompletionSource<UTXOChanges> completionSource = _FetchingUTXOs.GetOrAdd(strategy.ToString(), (s) => thisCompletionSource);
        ...
        try
        {
            UTXOChanges utxos = await _MemoryCache.GetOrCreateAsync("CACHEDCOINS_" + strategy.ToString(), async entry =>
            {
                ...
                TimeSpan spentTime = DateTimeOffset.UtcNow - now;
    ...
    {
        TransactionInformation tx = await _Client.GetTransactionAsync(derivationStrategyBase, transactionId);
    ...
    public Task<GetBalanceResponse> GetBalance(DerivationStrategyBase derivationStrategy, CancellationToken cancellation = default(CancellationToken))
    {
        ...
        {
            GetBalanceResponse result = await _Client.GetBalanceAsync(derivationStrategy, cancellation);
            entry.AbsoluteExpiration = DateTimeOffset.UtcNow + CacheSpan;
            return result;
        });
    }
