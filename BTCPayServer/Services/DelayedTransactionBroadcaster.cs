...
public class DelayedTransactionBroadcaster
{
    ...
    public async Task Schedule(DateTimeOffset broadcastTime, Transaction transaction, BTCPayNetwork network)
    {
        ...
        using ApplicationDbContext db = _dbContextFactory.CreateContext();
    ...
    public async Task<int> ProcessAll(CancellationToken cancellationToken = default)
    {
        ...
        Task<bool>[] broadcasts = scheduled.Select(async (record) =>
        {
            NBXplorer.ExplorerClient explorer = _explorerClientProvider.GetExplorerClient(record.Network);
            if (explorer is null)
            {
                return false;
            }
            ...
            try
            {
                ...
                NBXplorer.Models.BroadcastResult result = await explorer.BroadcastAsync(record.Transaction, cancellationToken);
                if (result.Success)
                {
