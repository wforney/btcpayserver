...
public class BTCPayNetworkJsonSerializerSettings
{
    public BTCPayNetworkJsonSerializerSettings(BTCPayNetworkProvider networkProvider, IEnumerable<IJsonConverterRegistration> jsonSerializers)
    {
        foreach (BTCPayNetwork network in networkProvider.GetAll().OfType<BTCPayNetwork>())
        {
            ...
            foreach (IJsonConverterRegistration jsonSerializer in jsonSerializers)
            {
                serializer.Converters.Add(jsonSerializer.CreateJsonConverter(network));
            }
            foreach (JsonConverter converter in network.NBXplorerNetwork.JsonSerializerSettings.Converters)
            {
    ...
    private readonly Dictionary<string, JsonSerializerSettings> _Serializers = new Dictionary<string, JsonSerializerSettings>();
    ...
    public JsonSerializerSettings GetSerializer(string cryptoCode)
    {
        ...
        _Serializers.TryGetValue(cryptoCode, out JsonSerializerSettings serializer);
        return serializer;
    }
}
