...
public class NotificationManager
{
    ...
    public async Task<NotificationsViewModel> GetSummaryNotifications(ClaimsPrincipal user)
    {
        ...
        {
            (List<NotificationViewModel> Items, int Count) resp = await GetNotifications(new NotificationsQuery()
            {
                Seen = false,
                Skip = 0,
                Take = 5,
    ...
    {
        await using ApplicationDbContext dbContext = _factory.CreateContext();

        (IQueryable<NotificationData> withoutPaging, IQueryable<NotificationData> withPaging) queryables = GetNotificationsQueryable(dbContext, query);
    ...
    {
        IQueryable<NotificationData> queryable = dbContext.Notifications.AsQueryable();
        ...
        IQueryable<NotificationData> queryable2 = queryable;
    ...
    public async Task<List<NotificationViewModel>> ToggleSeen(NotificationsQuery notificationsQuery, bool? setSeen)
    {
        ...
        (IQueryable<NotificationData> withoutPaging, IQueryable<NotificationData> withPaging) queryables = GetNotificationsQueryable(dbContext, notificationsQuery);
        List<NotificationData> items = await queryables.withPaging.ToListAsync();
        IEnumerable<string> userIds = items.Select(data => data.ApplicationUserId).Distinct();
    ...
    {
        INotificationHandler handler = GetHandler(data.NotificationType);
        if (handler is null)
        {
            return null;
        }
    ...
    {
        _handlersByNotificationType.TryGetValue(notificationId, out INotificationHandler h);
        return h;
    }
}
