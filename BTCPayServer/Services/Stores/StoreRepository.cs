...
public class StoreRepository
{
    ...
    public async Task<StoreData> FindStore(string storeId)
    {
        ...
        StoreData result = await ctx.FindAsync<StoreData>(storeId).ConfigureAwait(false);
        return result;
    }
    ...
    public async Task<StoreData> FindStore(string storeId, string userId)
    {
        ...
        await using ApplicationDbContext ctx = _ContextFactory.CreateContext();
        return (await ctx
                .UserStore
    ...
    {
        await using ApplicationDbContext context = _ContextFactory.CreateContext();
        InvoiceData matched = await context.Invoices.Include(data => data.StoreData)
    ...
    public async Task CleanUnreachableStores()
    {
        ...
        foreach (StoreData store in await ctx.Stores.Where(s => !s.UserStores.Where(u => u.Role == StoreRoles.Owner).Any()).ToArrayAsync())
        {
    ...
    private async Task DeleteStoreIfOrphan(string storeId)
    {
        ...
        if (ctx.Database.SupportDropForeignKey())
        {
            ...
            {
                StoreData store = await ctx.Stores.FindAsync(storeId);
                if (store != null)
                {
                    ctx.Stores.Remove(store);
                    await ctx.SaveChangesAsync();
    ...
    public async Task<StoreData> CreateStore(string ownerId, string name, string defaultCurrency, string preferredExchange)
    {
        ...
        Data.StoreBlob blob = store.GetStoreBlob();
    ...
    public async Task AddWebhookDelivery(WebhookDeliveryData delivery)
    {
        ...
        InvoiceWebhookDeliveryData invoiceWebhookDelivery = delivery.GetBlob().ReadRequestAs<InvoiceWebhookDeliveryData>();
    ...
    public async Task DeleteWebhook(string storeId, string webhookId)
    {
        ...
        ArgumentNullException.ThrowIfNull(storeId);
        using ApplicationDbContext ctx = _ContextFactory.CreateContext();
        WebhookData hook = await ctx.StoreWebhooks
            .Where(s => s.StoreId == storeId && s.WebhookId == webhookId)
            .Select(s => s.Webhook)
            .FirstOrDefaultAsync();
        if (hook is null)
    ...
    public async Task RemoveStore(string storeId, string userId)
    {
        using (ApplicationDbContext ctx = _ContextFactory.CreateContext())
        {
            UserStore storeUser = await ctx.UserStore.AsQueryable().FirstOrDefaultAsync(o => o.StoreDataId == storeId && o.ApplicationUserId == userId);
            if (storeUser == null)
            {
                return;
            }
    ...
    {
        using ApplicationDbContext ctx = _ContextFactory.CreateContext();
        StoreData existing = await ctx.FindAsync<StoreData>(store.Id);
    ...
    public async Task<bool> DeleteStore(string storeId)
    {
        ...
        WebhookData[] webhooks = await ctx.StoreWebhooks
            .Where(o => o.StoreId == storeId)
            .Select(o => o.Webhook)
            .ToArrayAsync();
        foreach (WebhookData w in webhooks)
        {
