...
public class TorServices : BaseAsyncService
{
    ...
    internal async Task Refresh()
    {
        ...
        try
        {
            ...
            if (!Torrc.TryParse(torrcContent, out Torrc torrc))
            {
            ...
            (string ServiceName, Task<string[]> ReadingLines, int VirtualPort)[] services = torrc.ServiceDirectories.SelectMany(d =>
                ...
                .ToArray();
            foreach ((string ServiceName, Task<string[]> ReadingLines, int VirtualPort) service in services)
            {
                try
                {
                    var onionHost = (await service.ReadingLines)[0].Trim();
                    TorService torService = ParseService(service.ServiceName, onionHost, service.VirtualPort);
    ...
    private TorService ParseService(string serviceName, string onionHost, int virtualPort)
    {
        ...
        if (Enum.TryParse<TorServiceType>(serviceName, true, out TorServiceType serviceType))
        {
            torService.ServiceType = serviceType;
        }
        else if (TryParseCryptoSpecificService(serviceName, out BTCPayNetworkBase network, out serviceType))
        {
            torService.ServiceType = serviceType;
