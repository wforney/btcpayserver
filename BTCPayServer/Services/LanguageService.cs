...
public class LanguageService
{
    ...
    public Language FindLanguageInAcceptLanguageHeader(string acceptLanguageHeader)
    {
        ...
        IOrderedEnumerable<KeyValuePair<string, float>> sortedAcceptedLocales = from entry in acceptedLocales orderby entry.Value descending select entry;
        foreach (KeyValuePair<string, float> pair in sortedAcceptedLocales)
        {
            Language lang = FindLanguage(pair.Key);
            if (lang != null)
            {
                return lang;
            }
    ...
    {
        Language[] supportedLangs = GetLanguages();
        ...
        List<Language> countryMatches = langMatches;
    ...
    public Language AutoDetectLanguageUsingHeader(IHeaderDictionary headerDictionary, string defaultLang)
    {
        if (headerDictionary?.TryGetValue("Accept-Language",
            out Microsoft.Extensions.Primitives.StringValues acceptLanguage) is true && !string.IsNullOrEmpty(acceptLanguage))
        {
