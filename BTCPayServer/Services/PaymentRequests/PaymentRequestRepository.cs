...
public class PaymentRequestRepository
{
    ...
    public async Task<PaymentRequestData> FindPaymentRequest(string id, string userId, CancellationToken cancellationToken = default)
    {
        ...
        using ApplicationDbContext context = _ContextFactory.CreateContext();
        PaymentRequestData result = await context.PaymentRequests.Include(x => x.StoreData)
            .Where(data =>
    ...
    {
        using ApplicationDbContext context = _ContextFactory.CreateContext();
        PaymentRequestData invoiceData = await context.FindAsync<PaymentRequestData>(paymentRequestId);
        if (invoiceData == null)
        {
            return;
        }
    ...
    {
        using ApplicationDbContext context = _ContextFactory.CreateContext();
        IQueryable<PaymentRequestData> queryable = context.PaymentRequests.Include(data => data.StoreData).AsQueryable();
