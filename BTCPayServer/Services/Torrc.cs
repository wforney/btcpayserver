...
public class Torrc
{
    public static bool TryParse(string str, out Torrc value)
    {
        ...
        {
            if (HiddenServiceDir.TryParse(line, out HiddenServiceDir dir))
            {
                serviceDirectories.Add(dir);
                currentDirectory = dir;
            }
            else if (HiddenServicePortDefinition.TryParse(line, out HiddenServicePortDefinition portDef) && currentDirectory != null)
            {
    ...
    public override string ToString()
    {
        ...
        foreach (HiddenServiceDir serviceDir in ServiceDirectories)
        {
            builder.AppendLine(serviceDir.ToString());
            foreach (HiddenServicePortDefinition port in serviceDir.ServicePorts)
            {
