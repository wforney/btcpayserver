...
public class InvoiceMetadata
{
    ...
    {
        if (AdditionalData == null || !(AdditionalData.TryGetValue(propName, out JToken jt) is true))
        {
            return default;
        }
...
{
    private class BuyerInformation
    {
...
#nullable enable
    ...
    {
        PaymentMethodId.TryParse(DefaultPaymentMethod, out PaymentMethodId? id);
        return id;
    }
#nullable restore
    [JsonExtensionData]
...
#pragma warning restore CS0618
    ...
    internal void SetSupportedPaymentMethods(IEnumerable<ISupportedPaymentMethod> derivationStrategies)
    {
        ...
        foreach (ISupportedPaymentMethod strat in derivationStrategies)
        {
...
#pragma warning restore CS0618
    ...
    private Uri FillPlaceholdersUri(string v)
    {
        ...
        if (Uri.TryCreate(uriStr, UriKind.Absolute, out Uri uri) && (uri.Scheme == "http" || uri.Scheme == "https"))
        {
            return uri;
        }
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        foreach (PaymentMethod info in GetPaymentMethods())
        {
            PaymentMethodAccounting accounting = info.Calculate();
            var cryptoInfo = new InvoiceCryptoInfo();
            Money subtotalPrice = accounting.TotalDue - accounting.NetworkFee;
            var cryptoCode = info.GetId().CryptoCode;
            IPaymentMethodDetails details = info.GetPaymentMethodDetails();
            ...
            if (details?.Activated is true)
            {
                ...
                if (paymentId.PaymentType == PaymentTypes.BTCLike)
                {
                    ...
                    {
                        TotalFee = accounting.NetworkFee.Satoshi,
                        SatoshiPerBytes = ((BitcoinLikeOnChainPaymentMethod)details).FeeRate
                        .GetFee(1).Satoshi
                    };
...
#pragma warning restore 618
        ...
        dto.Buyer = new JObject
            {
                new JProperty("name", Metadata.BuyerName),
                new JProperty("address1", Metadata.BuyerAddress1),
                new JProperty("address2", Metadata.BuyerAddress2),
                new JProperty("locality", Metadata.BuyerCity),
                new JProperty("region", Metadata.BuyerState),
                new JProperty("postalCode", Metadata.BuyerZip),
                new JProperty("country", Metadata.BuyerCountry),
                new JProperty("phone", Metadata.BuyerPhone),
                new JProperty("email", string.IsNullOrWhiteSpace(Metadata.BuyerEmail) ? RefundMail : Metadata.BuyerEmail)
            };
    ...
    {
        PaymentMethodDictionary rates = GetPaymentMethods();
        return rates.TryGet(paymentMethodId) != null;
    }

    public PaymentMethod GetPaymentMethod(PaymentMethodId paymentMethodId)
    {
        GetPaymentMethods().TryGetValue(paymentMethodId, out PaymentMethod data);
        return data;
    }
...
#pragma warning disable CS0618
        ...
        {
            foreach (JProperty prop in PaymentMethod.Properties())
            {
                PaymentMethod r = serializer.ToObject<PaymentMethod>(prop.Value.ToString());
                if (!PaymentMethodId.TryParse(prop.Name, out PaymentMethodId paymentMethodId))
                {
                    continue;
                }
...
#pragma warning restore CS0618
    ...
    {
        PaymentMethodDictionary dict = GetPaymentMethods();
    ...
    public void SetPaymentMethods(PaymentMethodDictionary paymentMethods)
    {
        ...
        var serializer = new Serializer(null);
#pragma warning disable CS0618
        foreach (PaymentMethod v in paymentMethods)
        {
            PaymentMethod clone = serializer.ToObject<PaymentMethod>(serializer.ToString(v));
        ...
        PaymentMethod = obj;
        foreach (PaymentMethod cryptoData in paymentMethods)
        {
...
#pragma warning restore CS0618
    ...
    internal void MigrateLegacyInvoice()
    {
        ...
        {
            if (AdditionalData.TryGetValue(field, out JToken token) && token is JObject obj)
            {
                ...
                TryParseMetadata<ProductInformation>("productInformation") is ProductInformation productInformation)
        {
            ...
            if (AdditionalData.TryGetValue("posData", out JToken token) &&
                token is JValue val &&
                val.Type == JTokenType.String)
            {
                wellknown.PosData = val.Value<string>();
            }
            if (AdditionalData.TryGetValue("orderId", out JToken token2) &&
...
{
    private static readonly Dictionary<string, InvoiceStatusLegacy> _StringToInvoiceStatus;
    private static readonly Dictionary<InvoiceStatusLegacy, string> _InvoiceStatusToString;
    private static readonly Dictionary<string, InvoiceExceptionStatus> _StringToExceptionStatus;
    private static readonly Dictionary<InvoiceExceptionStatus, string> _ExceptionStatusToString;

    static InvoiceState()
    {
        _StringToInvoiceStatus = new Dictionary<string, InvoiceStatusLegacy>
            {
                { "paid", InvoiceStatusLegacy.Paid },
                { "expired", InvoiceStatusLegacy.Expired },
                { "invalid", InvoiceStatusLegacy.Invalid },
                { "complete", InvoiceStatusLegacy.Complete },
                { "new", InvoiceStatusLegacy.New },
                { "confirmed", InvoiceStatusLegacy.Confirmed }
            };
        ...
        _StringToExceptionStatus = new Dictionary<string, InvoiceExceptionStatus>
            {
                { string.Empty, InvoiceExceptionStatus.None },
                { "paidPartial", InvoiceExceptionStatus.PaidPartial },
                { "paidLate", InvoiceExceptionStatus.PaidLate },
                { "paidOver", InvoiceExceptionStatus.PaidOver },
                { "marked", InvoiceExceptionStatus.Marked }
            };
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    public PaymentMethodAccounting Calculate(Func<PaymentEntity, bool> paymentPredicate = null)
    {
        ...
        PaymentMethodDictionary paymentMethods = ParentEntity.GetPaymentMethods();
...
#pragma warning disable CS0618 // Type or member is obsolete
        ...
        {
            PaymentMethodId paymentMethodId = GetPaymentMethodId();
...
#pragma warning restore CS0618
    ...
    internal decimal GetValue(PaymentMethodDictionary paymentMethods, PaymentMethodId paymentMethodId, decimal? value, int precision)
    {
        ...
        PaymentMethodId from = GetPaymentMethodId();
        if (to == from)
        {
