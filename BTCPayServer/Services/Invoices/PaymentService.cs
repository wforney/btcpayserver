...
public class PaymentService
{
    ...
    public async Task<PaymentEntity> AddPayment(string invoiceId, DateTimeOffset date, CryptoPaymentData paymentData, BTCPayNetworkBase network, bool accounted = false)
    {
        ...
        Data.InvoiceData invoice = await context.Invoices.FindAsync(invoiceId);
        if (invoice == null)
        {
            return null;
        }
...
#pragma warning restore CS0618
    ...
    public async Task UpdatePayments(List<PaymentEntity> payments)
    {
        ...
        Dictionary<string, PaymentData> dbPayments = await context.Payments
        ...
        {
            PaymentData dbPayment = dbPayments[payment.Key];
            InvoiceEntity invBlob = dbPayment.InvoiceData.GetBlob(_btcPayNetworkProvider);
            PaymentEntity dbPaymentEntity = dbPayment.GetBlob(_btcPayNetworkProvider);
