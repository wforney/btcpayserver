...
public static class InvoiceExtensions
{
        ...
        StoreData store, InvoiceEntity invoice, PaymentMethodId paymentMethodId)
    {
        ...
        bool success = false;
        PaymentMethod eligibleMethodToActivate = invoice.GetPaymentMethod(paymentMethodId);
        if (!eligibleMethodToActivate.GetPaymentMethodDetails().Activated)
        {
            IPaymentMethodHandler payHandler = paymentMethodHandlerDictionary[paymentMethodId];
            ISupportedPaymentMethod supportPayMethod = invoice.GetSupportedPaymentMethod()
                .Single(method => method.PaymentId == paymentMethodId);
            PaymentMethod paymentMethod = invoice.GetPaymentMethod(paymentMethodId);
            BTCPayNetworkBase network = btcPayNetworkProvider.GetNetwork(paymentMethodId.CryptoCode);
            ...
            try
            {
                ...
                IPaymentMethodDetails newDetails = await
