...
public class InvoiceRepository
{
    ...
    {
        using ApplicationDbContext db = _applicationDbContextFactory.CreateContext();
...
#pragma warning restore CS0618
    ...
    {
        using ApplicationDbContext ctx = _applicationDbContextFactory.CreateContext();
        InvoiceData invoiceData = await ctx.Invoices.FindAsync(invoiceId);

        InvoiceEntity invoice = invoiceData.GetBlob(_btcPayNetworkProvider);
...
#pragma warning restore CS0618 // Type or member is obsolete
            ...
            foreach (PaymentMethod paymentMethod in invoice.GetPaymentMethods())
            {
                ...
                IPaymentMethodDetails details = paymentMethod.GetPaymentMethodDetails();
    ...
    public async Task AddInvoiceLogs(string invoiceId, InvoiceLogs logs)
    {
        ...
        foreach (InvoiceLog log in logs.ToList())
        {
    ...
    {
        await using ApplicationDbContext context = _applicationDbContextFactory.CreateContext();
        InvoiceData invoice = (await context.Invoices.Where(i => i.Id == invoiceId).ToListAsync()).FirstOrDefault();
        if (invoice == null)
        {
            return false;
        }
        ...
        PaymentMethod paymentMethod = invoiceEntity.GetPaymentMethod(network, paymentMethodDetails.GetPaymentType());
        if (paymentMethod == null)
        {
            return false;
        }

        IPaymentMethodDetails existingPaymentMethod = paymentMethod.GetPaymentMethodDetails();
...
#pragma warning restore CS0618
    ...
    {
        using ApplicationDbContext context = _applicationDbContextFactory.CreateContext();
        InvoiceData invoice = await context.Invoices.FindAsync(invoiceId);
        if (invoice == null)
        {
            return;
        }

        BTCPayNetworkBase network = paymentMethod.Network;
        InvoiceEntity invoiceEntity = invoice.GetBlob(_btcPayNetworkProvider);
        IPaymentMethodDetails newDetails = paymentMethod.GetPaymentMethodDetails();
        PaymentMethod existing = invoiceEntity.GetPaymentMethod(paymentMethod.GetId());
        ...
        PaymentMethodId paymentMethodId)
    {
        ...
        IQueryable<HistoricalAddressInvoiceData> addresses = context.HistoricalAddressInvoices.Where(data =>
            (data.InvoiceDataId == invoiceId && paymentMethodIdStr == null ||
#pragma warning disable CS0618 // Type or member is obsolete
                 data.CryptoCode == paymentMethodIdStr) &&
#pragma warning restore CS0618 // Type or member is obsolete
                data.UnAssigned == null);
        foreach (HistoricalAddressInvoiceData historicalAddressInvoiceData in addresses)
        {
    ...
    {
        IEnumerable<InvoiceSearchData> filteredTerms = terms.Where(t => !string.IsNullOrWhiteSpace(t)
    ...
    {
        using ApplicationDbContext context = _applicationDbContextFactory.CreateContext();
        InvoiceData invoiceData = await context.FindAsync<Data.InvoiceData>(invoiceId).ConfigureAwait(false);
        if (invoiceData == null)
        {
            return;
        }
    ...
    internal async Task UpdateInvoicePrice(string invoiceId, InvoiceEntity invoice)
    {
        ...
        InvoiceEntity blob = invoiceData.GetBlob(_btcPayNetworkProvider);
    ...
    {
        using ApplicationDbContext context = _applicationDbContextFactory.CreateContext();
        IQueryable<InvoiceData> items = context.Invoices.Where(a => invoiceIds.Contains(a.Id));
        if (items == null)
        {
            return;
        }
    ...
    {
        using ApplicationDbContext context = _applicationDbContextFactory.CreateContext();
        InvoiceData invoiceData = await context.FindAsync<InvoiceData>(invoiceId).ConfigureAwait(false);
    ...
    {
        using ApplicationDbContext context = _applicationDbContextFactory.CreateContext();
        InvoiceData invoiceData = await GetInvoiceRaw(invoiceId, context);
    ...
    {
        using ApplicationDbContext context = _applicationDbContextFactory.CreateContext();
        InvoiceData res = await GetInvoiceRaw(id, context, includeAddressData);
    ...
    private async Task<InvoiceData> GetInvoiceRaw(string id, ApplicationDbContext dbContext, bool includeAddressData = false)
    {
        ...
        InvoiceData invoice = (await query.ToListAsync()).FirstOrDefault();
        if (invoice == null)
        {
            return null;
        }
    ...
    {
        InvoiceEntity entity = invoice.GetBlob(_btcPayNetworkProvider);
        PaymentMethodDictionary paymentMethods = null;
#pragma warning disable CS0618
        entity.Payments = invoice.Payments.Select(p =>
        {
            PaymentEntity paymentEntity = p.GetBlob(_btcPayNetworkProvider);
            ...
            if (paymentEntity.Version == 0)
            {
                ...
                IPaymentMethodDetails paymentMethodDetails = paymentMethods.TryGet(paymentEntity.GetPaymentMethodId())?.GetPaymentMethodDetails();
...
#pragma warning restore CA1310 // Specify StringComparison
    ...
    {
        using ApplicationDbContext context = _applicationDbContextFactory.CreateContext();
        IQueryable<InvoiceData> query = GetInvoiceQuery(context, queryObject);
    ...
    public async Task<InvoiceEntity[]> GetInvoices(InvoiceQuery queryObject)
    {
        ...
        InvoiceData[] data = await query.ToArrayAsync().ConfigureAwait(false);
