...
public class InvoiceExport
{
    ...
    private string processCsv(List<ExportInvoiceHolder> invoices)
    {
        ...
        using var csvWriter = new CsvHelper.CsvWriter(writer, CultureInfo.InvariantCulture, true);
    ...
    private IEnumerable<ExportInvoiceHolder> convertFromDb(InvoiceEntity invoice)
    {
        ...
        NumberFormatInfo currency = Currencies.GetNumberFormatInfo(invoice.Currency, true);
        var invoiceDue = invoice.Price;
        // in this first version we are only exporting invoices that were paid
        foreach (PaymentEntity payment in invoice.GetPayments(true))
        {
            var cryptoCode = payment.GetPaymentMethodId().CryptoCode;
            CryptoPaymentData pdata = payment.GetCryptoPaymentData();

            PaymentMethod pmethod = invoice.GetPaymentMethod(payment.GetPaymentMethodId());
