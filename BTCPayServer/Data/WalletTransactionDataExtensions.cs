...
public static class WalletTransactionDataExtensions
{
    public static WalletTransactionInfo GetBlobInfo(this WalletTransactionData walletTransactionData)
    {
        ...
        if (!string.IsNullOrEmpty(walletTransactionData.Labels))
        {
            ...
            {
                foreach (JToken jtoken in JArray.Parse(walletTransactionData.Labels))
                {
                    Label l = jtoken.Type == JTokenType.String ? Label.Parse(jtoken.Value<string>())
    ...
    private static readonly JsonSerializerSettings LabelSerializerSettings = new JsonSerializerSettings()
    {
