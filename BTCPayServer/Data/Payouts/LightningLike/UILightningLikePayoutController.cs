...
public class UILightningLikePayoutController : Controller
{
    ...
    public async Task<IActionResult> ConfirmLightningPayout(string cryptoCode, string[] payoutIds)
    {
        ...
        await using ApplicationDbContext ctx = _applicationDbContextFactory.CreateContext();
        List<PayoutData> payouts = await GetPayouts(ctx, pmi, payoutIds);

        var vm = payouts.Select(payoutData =>
        {
            PayoutBlob blob = payoutData.GetBlob(_btcPayNetworkJsonSerializerSettings);
    ...
    {
        var pmi = new PaymentMethodId(cryptoCode, PaymentTypes.LightningLike);
        IPayoutHandler payoutHandler = _payoutHandlers.FindPayoutHandler(pmi);

        await using ApplicationDbContext ctx = _applicationDbContextFactory.CreateContext();

        IEnumerable<IGrouping<string, PayoutData>> payouts = (await GetPayouts(ctx, pmi, payoutIds)).GroupBy(data => data.PullPaymentData.StoreId);
        var results = new List<ResultVM>();
        BTCPayNetwork network = _btcPayNetworkProvider.GetNetwork<BTCPayNetwork>(pmi.CryptoCode);
        ...
        async Task TrypayBolt(ILightningClient lightningClient, PayoutBlob payoutBlob, PayoutData payoutData, BOLT11PaymentRequest bolt11PaymentRequest)
        {
            ...
            PayResponse result = await lightningClient.Pay(bolt11PaymentRequest.ToString());
        ...
        foreach (IGrouping<string, PayoutData> payoutDatas in payouts)
        {
            StoreData store = payoutDatas.First().PullPaymentData.StoreData;

            LightningSupportedPaymentMethod lightningSupportedPaymentMethod = store.GetSupportedPaymentMethods(_btcPayNetworkProvider)
            ...
            {
                PayoutBlob blob = payoutData.GetBlob(_btcPayNetworkJsonSerializerSettings);
                (IClaimDestination destination, string error) claim = await payoutHandler.ParseClaimDestination(pmi, blob.Destination);
                try
                {
                    switch (claim.destination)
                    {
                        case LNURLPayClaimDestinaton lnurlPayClaimDestinaton:
                            Uri endpoint = LNURL.LNURL.Parse(lnurlPayClaimDestinaton.LNURL, out var tag);
                            var lightningPayoutHandler = (LightningLikePayoutHandler)payoutHandler;
                            HttpClient httpClient = lightningPayoutHandler.CreateClient(endpoint);
                            var lnurlInfo =
                            ...
                            else
                            {
                                try
                                {
                                    LNURLPayRequest.LNURLPayRequestCallbackResponse lnurlPayRequestCallbackResponse =
