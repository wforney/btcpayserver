...
public class EventAggregator : IDisposable
{
    ...
    public async Task<T> WaitNext<T>(Func<T, bool> predicate, CancellationToken cancellation = default(CancellationToken))
    {
        ...
        using IEventAggregatorSubscription subscription = Subscribe<T>((a, b) => { if (predicate(b)) { tcs.TrySetResult(b); a.Unsubscribe(); } });
    ...
    public void Publish(object evt, Type evtType)
    {
        ...
        foreach (Action<object> sub in actionList)
        {
            try
            {
                sub(evt);
    ...
    private readonly Dictionary<Type, Dictionary<Subscription, Action<object>>> _Subscriptions = new Dictionary<Type, Dictionary<Subscription, Action<object>>>();
    ...
    private class ChannelSubscription<T> : IEventAggregatorSubscription
    {
        ...
        {
            await foreach (T item in _evts.Reader.ReadAllAsync())
            {
                try
                {
                    await _act(item);
    ...
    public IEventAggregatorSubscription SubscribeAsync<T>(Func<T, Task> subscription)
    {
        ...
        IEventAggregatorSubscription innerSubscription = Subscribe(new Action<IEventAggregatorSubscription, T>((sub, t) => evts.Writer.TryWrite(t)));
