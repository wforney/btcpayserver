...
{
    private static readonly Regex _WalletStoreRegex = new Regex("^S-([a-zA-Z0-9]{30,60})-([a-zA-Z]{2,5})$");
    public static bool TryParse(string str, out WalletId walletId)
    {
        ArgumentNullException.ThrowIfNull(str);
        walletId = null;
        WalletId w = new WalletId();
        Match match = _WalletStoreRegex.Match(str);
        if (!match.Success)
        {
            return false;
        }
