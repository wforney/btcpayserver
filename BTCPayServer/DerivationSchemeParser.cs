...
public class DerivationSchemeParser
{
    ...
    public (DerivationStrategyBase, RootedKeyPath[]) ParseOutputDescriptor(string str)
    {
            ...
            string suffix = "")
        {
            switch (pubKeyProvider)
            {
                ...
                case PubKeyProvider.Origin origin:
                    (DerivationStrategyBase, RootedKeyPath[]) innerResult = ExtractFromPkProvider(origin.Inner, suffix);
        ...
        switch (outputDescriptor)
        {
            ...
            case OutputDescriptor.Multi multi:
                IEnumerable<(DerivationStrategyBase, RootedKeyPath[])> xpubs = multi.PkProviders.Select(provider => ExtractFromPkProvider(provider));
                return (
                    Parse(
            ...
            case OutputDescriptor.SH sh:
                ...
                {
                    (DerivationStrategyBase, RootedKeyPath[]) ds = ParseOutputDescriptor(sh.Inner.ToString());
    ...
    public DerivationStrategyBase ParseElectrum(string str)
    {
        ...
        BitcoinExtPubKey extPubKey = GetBitcoinExtPubKeyByNetwork(Network, data);
        if (!BtcPayNetwork.ElectrumMapping.TryGetValue(prefix, out DerivationType type))
        {
            throw new FormatException();
        }
