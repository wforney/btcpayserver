...
public class ShopifyApiClient
{
    ...
    {
        HttpRequestMessage req = CreateRequest(_credentials.ShopName, HttpMethod.Post, $"webhooks.json");
    ...
    {
        HttpRequestMessage req = CreateRequest(_credentials.ShopName, HttpMethod.Delete, $"webhooks/{id}.json");
        var strResp = await SendRequest(req);
    }

    public async Task<string[]> CheckScopes()
    {
        HttpRequestMessage req = CreateRequest(_credentials.ShopName, HttpMethod.Get, null, "admin/oauth/access_scopes.json");
    ...
    {
        HttpRequestMessage req = CreateRequest(_credentials.ShopName, HttpMethod.Get, $"orders/{orderId}/transactions.json");

        var strResp = await SendRequest(req);

        TransactionsListResp parsed = JsonConvert.DeserializeObject<TransactionsListResp>(strResp);
    ...
    public async Task<TransactionsCreateResp> TransactionCreate(string orderId, TransactionsCreateReq txnCreate)
    {
        ...
        HttpRequestMessage req = CreateRequest(_credentials.ShopName, HttpMethod.Post, $"orders/{orderId}/transactions.json");
    ...
    {
        HttpRequestMessage req = CreateRequest(_credentials.ShopName, HttpMethod.Get,
    ...
    {
        HttpRequestMessage req = CreateRequest(_credentials.ShopName, HttpMethod.Get, $"orders/count.json");
        var strResp = await SendRequest(req);

        CountResponse parsed = JsonConvert.DeserializeObject<CountResponse>(strResp);
    ...
    {
        HttpRequestMessage req = CreateRequest(_credentials.ShopName, HttpMethod.Get, $"orders/{orderId}.json?fields=id");
        var strResp = await SendRequest(req);
