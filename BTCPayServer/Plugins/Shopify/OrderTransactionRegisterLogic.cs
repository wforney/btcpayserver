...
public class OrderTransactionRegisterLogic
{
    ...
    public async Task<TransactionsCreateResp> Process(string orderId, string invoiceId, string currency, string amountCaptured, bool success)
    {
        ...
        System.Collections.Generic.List<ApiModels.DataHolders.TransactionDataHolder> existingShopifyOrderTransactions = (await _client.TransactionsList(orderId)).transactions;

        //if there isn't a record for btcpay payment gateway, abort
        ApiModels.DataHolders.TransactionDataHolder baseParentTransaction = existingShopifyOrderTransactions.FirstOrDefault(holder => !_keywords.Any(a => holder.gateway.Contains(a, StringComparison.InvariantCultureIgnoreCase)));
        ...
        System.Collections.Generic.IEnumerable<ApiModels.DataHolders.TransactionDataHolder> existingShopifyOrderTransactionsOnSameInvoice =
            existingShopifyOrderTransactions.Where(holder => holder.authorization == invoiceId);

        //filter out the successful ones
        ApiModels.DataHolders.TransactionDataHolder[] successfulActions =
        ...
        ApiModels.DataHolders.TransactionDataHolder[] successfulCaptures = successfulActions.Where(holder => holder.kind == "capture").ToArray();

        //of the successful ones, get the ones we registered as a voiding of a previous successful payment
        ApiModels.DataHolders.TransactionDataHolder[] refunds = successfulActions.Where(holder => holder.kind == "refund").ToArray();
        ...
        TransactionsCreateResp createResp = await _client.TransactionCreate(orderId, createTransaction);
        return createResp;
    }
}
