...
public class UIShopifyController : Controller
{
    ...
    private async Task<string> GetJavascript()
    {
        ...
        {
            await using Stream stream = _webHostEnvironment.WebRootFileProvider
        ...
        string storeId, string orderId, decimal amount, bool checkOnly = false)
    {
        ...
        InvoiceEntity[] matchedExistingInvoices = await _invoiceRepository.GetInvoices(new InvoiceQuery()
        {
        ...
        InvoiceEntity firstInvoiceStillPending =
        ...
        InvoiceEntity firstInvoiceSettled =
        ...
        StoreData store = await _storeRepository.FindStore(storeId);
        ShopifySettings shopify = store?.GetStoreBlob()?.GetShopifySettings();
        ...
        if (shopify?.IntegratedAt.HasValue is true)
        {
            ...
            InvoiceEntity invoice = await _invoiceController.CreateInvoiceCoreRaw(
