...
public class ShopifyOrderMarkerHostedService : EventHostedServiceBase
{
    ...
    {
        Data.StoreData storeData = await _storeRepository.FindStore(invoice.StoreId);
        StoreBlob storeBlob = storeData.GetStoreBlob();

        // ensure that store in question has shopify integration turned on 
        // and that invoice's orderId has shopify specific prefix
        ShopifySettings settings = storeBlob.GetShopifySettings();
        if (settings?.IntegratedAt.HasValue == true)
        {
            ShopifyApiClient client = CreateShopifyApiClient(settings);
            ...
            try
            {
                ...
                ApiModels.TransactionsCreateResp resp = await logic.Process(shopifyOrderId, invoice.Id, invoice.Currency,
