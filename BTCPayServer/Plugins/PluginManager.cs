...
public static class PluginManager
{
        ...
        IConfiguration config, ILoggerFactory loggerFactory)
    {
        ...
        Assembly[] systemPlugins = GetDefaultLoadedPluginAssemblies();

        foreach (Assembly systemExtension in systemPlugins)
        {
            IEnumerable<IBTCPayServerPlugin> detectedPlugins = GetAllPluginTypesFromAssembly(systemExtension).Select(GetPluginInstanceFromType);
        ...
        {
            IEnumerable<string> order = File.ReadLines(orderFilePath);
        ...
        foreach (var dir in orderedDirs)
        {
            ...
            try
            {
                ...
                Assembly pluginAssembly = plugin.LoadDefaultAssembly();
                _pluginAssemblies.Add(pluginAssembly);
                _plugins.Add(plugin);
                IFileProvider fileProvider = CreateEmbeddedFileProviderForAssembly(pluginAssembly);
        ...
        foreach (IBTCPayServerPlugin plugin in plugins)
        {
            try
            {
    ...
    {
        foreach (IBTCPayServerPlugin extension in applicationBuilder.ApplicationServices
        ...
        IWebHostEnvironment webHostEnvironment = applicationBuilder.ApplicationServices.GetService<IWebHostEnvironment>();
    ...
    {
        (string command, string plugin)[] pendingCommands = GetPendingCommands(pluginsFolder);
        foreach ((string command, string plugin) command in pendingCommands)
        {
    ...
    {
        (string command, string plugin)[] cmds = GetPendingCommands(pluginDir).Where(tuple =>
