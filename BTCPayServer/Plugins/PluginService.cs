...
public class PluginService
{
    ...
    public async Task<AvailablePlugin[]> GetRemotePlugins(string path = "")
    {
        ...
        GithubFile[] files = JsonConvert.DeserializeObject<GithubFile[]>(resp);
        IEnumerable<GithubFile> dirs = files.Where(file => file.Type == "dir");
        IEnumerable<Task<AvailablePlugin[]>> result = dirs.Select(file => GetRemotePlugins(file.Path));
        ...
        var fileTask = Task.WhenAll(files
                ...
                StringComparison.InvariantCulture)).Select(async file =>
            {
                return await _githubClient.GetStringAsync(file.DownloadUrl).ContinueWith(
                    task =>
                    {
                        AvailablePlugin r = JsonConvert.DeserializeObject<AvailablePlugin>(task.Result);
                        r.Path = path;
                        return r;
    ...
    public async Task DownloadRemotePlugin(string plugin, string path)
    {
        ...
        var resp = await _githubClient
            .GetStringAsync(new Uri($"https://api.github.com/repos/{_btcPayServerOptions.PluginRemote}/contents/{path}"));
        GithubFile[] files = JsonConvert.DeserializeObject<GithubFile[]>(resp);
        GithubFile ext = files.SingleOrDefault(file => file.Name == $"{plugin}{PluginManager.BTCPayPluginSuffix}");
        if (ext is null)
        {
        ...
        using HttpResponseMessage resp2 = await _githubClient.GetAsync(ext.DownloadUrl);
