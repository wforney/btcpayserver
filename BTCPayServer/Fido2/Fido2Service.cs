...
public class Fido2Service
{
    ...
    {
        await using ApplicationDbContext dbContext = _contextFactory.CreateContext();
        ApplicationUser user = await dbContext.Users.Include(applicationUser => applicationUser.Fido2Credentials)
    ...
    public async Task<bool> CompleteCreation(string userId, string name, string data)
    {
        try
        {
            ...
            AuthenticatorAttestationRawResponse attestationResponse = JObject.Parse(data).ToObject<AuthenticatorAttestationRawResponse>();
            await using ApplicationDbContext dbContext = _contextFactory.CreateContext();
            ApplicationUser user = await dbContext.Users.Include(applicationUser => applicationUser.Fido2Credentials)
                .FirstOrDefaultAsync(applicationUser => applicationUser.Id == userId);
            if (user == null || !CreationStore.TryGetValue(userId, out CredentialCreateOptions options))
            {
                return false;
            }
    ...
    {
        await using ApplicationDbContext context = _contextFactory.CreateContext();
        Fido2Credential device = await context.Fido2Credentials.FindAsync(id);
    ...
    public async Task<bool> CompleteLogin(string userId, AuthenticatorAssertionRawResponse response)
    {
        ...
        ApplicationUser user = await dbContext.Users.Include(applicationUser => applicationUser.Fido2Credentials)
            .FirstOrDefaultAsync(applicationUser => applicationUser.Id == userId);
        if (user == null || !LoginStore.TryGetValue(userId, out AssertionOptions options))
        {
            return false;
        }

        (Fido2Credential fido2Credential, Fido2CredentialBlob) credential = user.Fido2Credentials
        ...
        AssertionVerificationResult res = await _fido2.MakeAssertionAsync(response, options, credential.Item2.PublicKey,
