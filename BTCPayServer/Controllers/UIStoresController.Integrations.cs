...
public partial class UIStoresController
{
    ...
    {
        WebhookData[]? webhooks = await _Repo.GetWebhooks(CurrentStore.Id);
        return View(nameof(Webhooks), new WebhooksViewModel()
        {
            Webhooks = webhooks.Select(async w =>
            {
                Data.WebhookDeliveryData? lastDelivery = await LastDeliveryForWebhook(w.Id);
                WebhookDeliveryBlob? lastDeliveryBlob = lastDelivery?.GetBlob();
    ...
    {
        WebhookData? webhook = await _Repo.GetWebhook(CurrentStore.Id, webhookId);
        if (webhook is null)
        {
            return NotFound();
        }
    ...
    public async Task<IActionResult> ModifyWebhook(string webhookId)
    {
        ...
        WebhookBlob? blob = webhook.GetBlob();
        Data.WebhookDeliveryData[]? deliveries = await _Repo.GetWebhookDeliveries(CurrentStore.Id, webhookId, 20);
    ...
    {
        HostedServices.WebhookSender.DeliveryResult? result = await WebhookNotificationManager.TestWebhook(CurrentStore.Id, webhookId, viewModel.Type, cancellationToken);
    ...
    {
        Data.WebhookDeliveryData? delivery = await _Repo.GetWebhookDelivery(CurrentStore.Id, webhookId, deliveryId);
        if (delivery is null)
        {
            return NotFound();
        }
