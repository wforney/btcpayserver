...
public class UINotificationsController : Controller
{
    ...
    public async Task<IActionResult> GenerateJunk(int x = 100, bool admin = true)
    {
        for (int i = 0; i < x; i++)
        {
            await _notificationSender.SendNotification(
                admin ? new AdminScope() : new UserScope(_userManager.GetUserId(User)),
    ...
    public async Task<IActionResult> Index(int skip = 0, int count = 50, int timezoneOffset = 0)
    {
        ...
        (System.Collections.Generic.List<Abstractions.Contracts.NotificationViewModel> Items, int Count) res = await _notificationManager.GetNotifications(new NotificationsQuery()
        {
            Skip = skip,
            Take = count,
            UserId = userId
    ...
    public async Task<IActionResult> NotificationPassThrough(string id)
    {
        if (ValidUserClaim(out var userId))
        {
            System.Collections.Generic.List<Abstractions.Contracts.NotificationViewModel> items = await
