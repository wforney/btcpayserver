...
#pragma warning restore CA1307 // Specify StringComparison
    ...
    public new async Task<IActionResult> User(string userId, UsersViewModel.UserViewModel viewModel)
    {
        ...
        IList<ApplicationUser> admins = await _UserManager.GetUsersInRoleAsync(Roles.ServerAdmin);
        IList<string> roles = await _UserManager.GetRolesAsync(user);
    ...
    public async Task<IActionResult> CreateUser(RegisterFromAdminViewModel model)
    {
        ...
        if (ModelState.IsValid)
        {
            ...
            if (result.Succeeded)
            {
                ...
                Uri callbackUrl = await tcs.Task;
    ...
    {
        ApplicationUser user = userId == null ? null : await _UserManager.FindByIdAsync(userId);
        if (user == null)
        {
            return NotFound();
        }
