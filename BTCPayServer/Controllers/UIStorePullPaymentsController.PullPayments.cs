...
public class UIStorePullPaymentsController : Controller
{
    ...
    public async Task<IActionResult> NewPullPayment(string storeId)
    {
        ...
        List<PaymentMethodId> paymentMethods = await _payoutHandlers.GetSupportedPaymentMethods(CurrentStore);
    ...
    public async Task<IActionResult> NewPullPayment(string storeId, NewPullPaymentModel model)
    {
        ...
        List<PaymentMethodId> paymentMethodOptions = await _payoutHandlers.GetSupportedPaymentMethods(CurrentStore);
        ...
        PaymentMethodId[] selectedPaymentMethodIds = model.PaymentMethods.Select(PaymentMethodId.Parse).ToArray();
    ...
    )
    {
        ...
        IQueryable<Data.PullPaymentData> ppsQuery = ctx.PullPayments
        ...
        List<PaymentMethodId> paymentMethods = await _payoutHandlers.GetSupportedPaymentMethods(HttpContext.GetStoreData());
        if (!paymentMethods.Any())
        {
            TempData.SetStatusMessageModel(new StatusMessageModel
            {
        ...
        PullPaymentsModel vm = this.ParseListQuery(new PullPaymentsModel()
        {
            Skip = skip,
            Count = count,
        ...
        List<Data.PullPaymentData> pps = (await ppsQuery
                .Skip(vm.Skip)
                .Take(vm.Count)
                .ToListAsync()
            );
        foreach (Data.PullPaymentData pp in pps)
        {
            ...
            ;
            PullPaymentBlob ppBlob = pp.GetBlob();
            CurrencyData ni = _currencyNameTable.GetCurrencyData(ppBlob.Currency, true);
            System.Globalization.NumberFormatInfo nfi = _currencyNameTable.GetNumberFormatInfo(ppBlob.Currency, true);
            (DateTimeOffset Start, DateTimeOffset? End)? period = pp.GetPeriod(now);
        ...
        string storeId, PayoutsModel vm, CancellationToken cancellationToken)
    {
        ...
        PayoutState commandState = Enum.Parse<PayoutState>(vm.Command.Split("-").First());
        ...
        if (handler != null)
        {
            StatusMessageModel result = await handler.DoSpecificAction(command, payoutIds, storeId);
            if (result != null)
            {
            ...
            case "approve":
                {
                    ...
                    List<PayoutData> payouts = await GetPayoutsForPaymentMethod(paymentMethodId, ctx, payoutIds, storeId, cancellationToken);

                    var failed = false;
                    for (int i = 0; i < payouts.Count; i++)
                    {
                        PayoutData payout = payouts[i];
                        if (payout.State != PayoutState.AwaitingApproval)
                        {
                            continue;
                        }

                        RateResult rateResult = await _pullPaymentService.GetRate(payout, null, cancellationToken);
                        ...
                        PullPaymentHostedService.PayoutApproval.Result approveResult = await _pullPaymentService.Approve(new HostedServices.PullPaymentHostedService.PayoutApproval()
                        {
                            PayoutId = payout.Id,
            ...
            case "mark-paid":
                {
                    ...
                    for (int i = 0; i < payouts.Count; i++)
                    {
                        ...
                        PayoutPaidRequest.PayoutPaidResult result = await _pullPaymentService.MarkPaid(new PayoutPaidRequest()
                        {
                            PayoutId = payout.Id
                        });
        ...
        int skip = 0, int count = 50)
    {
        ...
        PayoutsModel vm = this.ParseListQuery(new PayoutsModel
        {
        ...
        await using ApplicationDbContext ctx = _dbContextFactory.CreateContext();
        IQueryable<PayoutData> payoutRequest = ctx.Payouts.Where(p => p.PullPaymentData.StoreId == storeId && !p.PullPaymentData.Archived);
        ...
        {
            PullPaymentBlob ppBlob = item.PullPayment.GetBlob();
            PayoutBlob payoutBlob = item.Payout.GetBlob(_jsonSerializerSettings);
            ...
            IPayoutProof proofBlob = handler?.ParseProof(item.Payout);
