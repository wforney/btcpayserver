...
public partial class UIManageController
{
    ...
    {
        APIKeyData key = await _apiKeyRepository.GetKey(id);
        ...
        bool strict = true, bool selectiveStores = false, string applicationIdentifier = null)
    {
        ...
        IEnumerable<Permission> requestPermissions = Permission.ToPermissions(permissions);
        ...
        if (!string.IsNullOrEmpty(applicationIdentifier) && redirect != null)
        {
            ...
            List<APIKeyData> keys = await _apiKeyRepository.GetKeys(new APIKeyRepository.APIKeyQuery()
            {
                UserId = new[] { _userManager.GetUserId(User) }
            });
            foreach (APIKeyData key in keys)
            {
                APIKeyBlob blob = key.GetBlob();
                ...
                IEnumerable<IGrouping<string, Permission>> alreadyPresentPermissions = Permission.ToPermissions(blob.Permissions)
                    .GroupBy(permission => permission.Policy);
                var fail = false;
                foreach (IGrouping<string, Permission> permission in requestPermissions.GroupBy(permission => permission.Policy))
                {
                    IGrouping<string, Permission> presentPermission =
        ...
        AuthorizeApiKeysViewModel vm = await SetViewModelValues(new AuthorizeApiKeysViewModel()
        {
            RedirectUrl = redirect,
            Label = applicationName,
            ApplicationName = applicationName,
    ...
    private void AdjustVMForAuthorization(AuthorizeApiKeysViewModel vm)
    {
        ...
        IEnumerable<IGrouping<string, Permission>> parsedPermissions = Permission.ToPermissions(permissionsWithStoreIDs.ToArray()).GroupBy(permission => permission.Policy);

        for (var index = vm.PermissionValues.Count - 1; index >= 0; index--)
        {
            AddApiKeyViewModel.PermissionValueItem permissionValue = vm.PermissionValues[index];
            IGrouping<string, Permission> wanted = parsedPermissions?.SingleOrDefault(permission =>
    ...
    public async Task<IActionResult> AuthorizeAPIKey([FromForm] AuthorizeApiKeysViewModel viewModel)
    {
        ...
        IActionResult ar = HandleCommands(viewModel);
        ...
        switch (command)
        {
            ...
            case "authorize":
            case "confirm":
                APIKeyData key = command == "authorize"
    ...
    public async Task<IActionResult> AddApiKey(AddApiKeyViewModel viewModel)
    {
        ...
        APIKeyData key = await CreateKey(viewModel);
    ...
    private IActionResult HandleCommands(AddApiKeyViewModel viewModel)
    {
        ...
        AddApiKeyViewModel.PermissionValueItem permissionValueItem = viewModel.PermissionValues.Single(item => item.Permission == permission);
        var command = parts[1];
    ...
    private IEnumerable<Permission> GetPermissionsFromViewModel(AddApiKeyViewModel viewModel)
    {
        ...
        foreach (AddApiKeyViewModel.PermissionValueItem p in viewModel.PermissionValues.Where(tuple => !tuple.Forbidden))
        {
            ...
            else if (p.Value && Permission.TryCreatePermission(p.Permission, null, out Permission pp))
            {
                permissions.Add(pp);
            }
        }
    ...
    private async Task<T> SetViewModelValues<T>(T viewModel) where T : AddApiKeyViewModel
    {
        ...
        if (!isAdmin)
        {
            foreach (AddApiKeyViewModel.PermissionValueItem p in viewModel.PermissionValues.Where(item => item.Permission is null || Policies.IsServerPolicy(item.Permission)))
            {
                p.Forbidden = true;
            }
        }
