...
public partial class UIStoresController
{
    ...
    public async Task<IActionResult> SetupLightningNode(string storeId, LightningNodeViewModel vm, string command, string cryptoCode)
    {
        ...
        BTCPayNetwork? network = _ExplorerProvider.GetNetwork(vm.CryptoCode);
        ...
        else
        {
            ...
            if (!LightningConnectionString.TryParse(vm.ConnectionString, false, out LightningConnectionString? connectionString, out var error))
            {
        ...
        switch (command)
        {
            ...
            case "test":
                LightningLikePaymentHandler? handler = _ServiceProvider.GetRequiredService<LightningLikePaymentHandler>();
                try
                {
                    NodeInfo[]? info = await handler.GetNodeInfo(paymentMethod, network, new InvoiceLogs(), Request.IsOnion(), true);
                    if (!vm.SkipPortTest)
                    {
    ...
    public async Task<IActionResult> LightningSettings(string storeId, string cryptoCode)
    {
        ...
        LightningSupportedPaymentMethod? lightning = GetExistingLightningSupportedPaymentMethod(cryptoCode, store);
        if (lightning == null)
        {
        ...
        LNURLPaySupportedPaymentMethod? lnurl = GetExistingLNURLSupportedPaymentMethod(vm.CryptoCode, store);
        if (lnurl != null)
        {
    ...
    public async Task<IActionResult> LightningSettings(LightningSettingsViewModel vm)
    {
        ...
        LightningSupportedPaymentMethod? lightning = GetExistingLightningSupportedPaymentMethod(vm.CryptoCode, store);
    ...
    public async Task<IActionResult> SetLightningNodeEnabled(string storeId, string cryptoCode, bool enabled)
    {
        ...
        BTCPayNetwork? network = _ExplorerProvider.GetNetwork(cryptoCode);
        LightningSupportedPaymentMethod? lightning = GetExistingLightningSupportedPaymentMethod(cryptoCode, store);
        if (lightning == null)
        {
            return NotFound();
    ...
    private LightningSupportedPaymentMethod? GetExistingLightningSupportedPaymentMethod(string cryptoCode, StoreData store)
    {
        ...
        LightningSupportedPaymentMethod? existing = store.GetSupportedPaymentMethods(_NetworkProvider)
    ...
    private LNURLPaySupportedPaymentMethod? GetExistingLNURLSupportedPaymentMethod(string cryptoCode, StoreData store)
    {
        ...
        LNURLPaySupportedPaymentMethod? existing = store.GetSupportedPaymentMethods(_NetworkProvider)
