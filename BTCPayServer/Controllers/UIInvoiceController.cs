...
public partial class UIInvoiceController : Controller
{
    ...
    {
        InvoiceEntity? entity = await CreateInvoiceCoreRaw(invoice, store, serverUrl, additionalTags, cancellationToken);
        InvoiceResponse? resp = entity.EntityToDTO();
    ...
    internal async Task<InvoiceEntity> CreateInvoiceCoreRaw(BitpayCreateInvoiceRequest invoice, StoreData store, string serverUrl, List<string>? additionalTags = null, CancellationToken cancellationToken = default)
    {
        ...
        InvoiceEntity? entity = _InvoiceRepository.CreateNewInvoice();
        ...
        if (invoice.SupportedTransactionCurrencies != null && invoice.SupportedTransactionCurrencies.Count != 0)
        {
            var supportedTransactionCurrencies = invoice.SupportedTransactionCurrencies
                                                        ...
                                                        .Select(c => PaymentMethodId.TryParse(c.Key, out PaymentMethodId? p) ? p : null)
                                                        .Where(c => c != null)
                                                        .ToHashSet();
    ...
    internal async Task<InvoiceEntity> CreateInvoiceCoreRaw(CreateInvoiceRequest invoice, StoreData store, string serverUrl, List<string>? additionalTags = null, CancellationToken cancellationToken = default)
    {
        ...
        if (invoice.Checkout.PaymentMethods != null)
        {
            var supportedTransactionCurrencies = invoice.Checkout.PaymentMethods
                                                        .Select(c => PaymentMethodId.TryParse(c, out PaymentMethodId? p) ? p : null)
                                                        .ToHashSet();
            excludeFilter = PaymentFilter.Where(p => !supportedTransactionCurrencies.Contains(p));
        }
    ...
    internal async Task<InvoiceEntity> CreateInvoiceCoreRaw(InvoiceEntity entity, StoreData store, IPaymentFilter? invoicePaymentMethodFilter, string[]? additionalSearchTerms = null, CancellationToken cancellationToken = default)
    {
        ...
        System.Globalization.NumberFormatInfo? currencyInfo = _CurrencyNameTable.GetNumberFormatInfo(entity.Currency, false);
        ...
        Task<AppData[]>? getAppsTaggingStore = _InvoiceRepository.GetAppsTaggingStore(store.Id);
        ...
        RateRules? rules = storeBlob.GetRateRules(_NetworkProvider);
        IPaymentFilter? excludeFilter = storeBlob.GetExcludedPaymentMethods(); // Here we can compose filters from other origin with PaymentFilter.Any()
        if (invoicePaymentMethodFilter != null)
        {
            excludeFilter = PaymentFilter.Or(excludeFilter,
                                             invoicePaymentMethodFilter);
        }
        foreach (BTCPayNetworkBase? network in store.GetSupportedPaymentMethods(_NetworkProvider)
                                            ...
                                            .Where(c => c != null))
        {
            ...
            foreach (PaymentMethodCriteria? paymentMethodCriteria in storeBlob.PaymentMethodCriteria)
            {
        ...
        RateRules? rateRules = storeBlob.GetRateRules(_NetworkProvider);
        Dictionary<CurrencyPair, Task<RateResult>>? fetchingByCurrencyPair = _RateProvider.FetchRates(currencyPairsToFetch, rateRules, cancellationToken);
        Task? fetchingAll = WhenAllFetched(logs, fetchingByCurrencyPair);
        ...
        if (!noNeedForMethods)
        {
            ...
            foreach ((ISupportedPaymentMethod SupportedPaymentMethod, Task<PaymentMethod?> PaymentMethod) o in store.GetSupportedPaymentMethods(_NetworkProvider)
                .Where(s => !excludeFilter.Match(s.PaymentId) &&
                ...
                .ToList())
            {
                PaymentMethod? paymentMethod = await o.PaymentMethod;
            ...
            if (supported.Count == 0)
            {
                ...
                foreach (InvoiceLog? error in logs.ToList())
                {
        ...
        foreach (AppData? app in await getAppsTaggingStore)
        {
    ...
    private Task WhenAllFetched(InvoiceLogs logs, Dictionary<CurrencyPair, Task<RateResult>> fetchingByCurrencyPair)
    {
        ...
        {
            RateResult? rateResult = await pair.Value;
            ...
            foreach (ExchangeException? ex in rateResult.ExchangeExceptions)
            {
        ...
        StoreData store, InvoiceLogs logs)
    {
        try
        {
            ...
            RateResult? rate = await fetchingByCurrencyPair[new CurrencyPair(network.CryptoCode, entity.Currency)];
            if (rate.BidAsk == null)
            {
                return null;
            }
            var paymentMethod = new PaymentMethod
            {
                ...
                PreferOnion = Uri.TryCreate(entity.ServerUrl, UriKind.Absolute, out Uri? u) && u.DnsSafeHost.EndsWith(".onion", StringComparison.OrdinalIgnoreCase)
            };
            paymentMethod.SetId(supportedPaymentMethod.PaymentId);

            using (logs.Measure($"{logPrefix} Payment method details creation"))
            {
                IPaymentMethodDetails? paymentDetails = await handler.CreatePaymentMethodDetails(logs, supportedPaymentMethod, paymentMethod, store, network, preparePayment);
                paymentMethod.SetPaymentMethodDetails(paymentDetails);
            }

            PaymentMethodCriteria? criteria = storeBlob.PaymentMethodCriteria?.Find(methodCriteria => methodCriteria.PaymentMethod == supportedPaymentMethod.PaymentId);
            if (criteria?.Value != null && entity.Type != InvoiceType.TopUp)
            {
                RateResult? currentRateToCrypto =
...
#pragma warning restore CS0618
    ...
    {
        InvoiceMetadata? buyerInformation = invoiceEntity.Metadata;
