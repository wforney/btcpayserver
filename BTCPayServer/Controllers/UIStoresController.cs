...
public partial class UIStoresController : Controller
{
    ...
    {
        StoreRepository.StoreUser[] users = await _Repo.GetStoreUsers(CurrentStore.Id);
    ...
    {
        IEnumerable<AvailableRateProvider> exchanges = GetSupportedExchanges();
        StoreBlob storeBlob = CurrentStore.GetStoreBlob();
    ...
    public async Task<IActionResult> Rates(RatesViewModel model, string command = null, string storeId = null, CancellationToken cancellationToken = default)
    {
        ...
        StoreBlob blob = CurrentStore.GetStoreBlob();
        ...
        if (model.ShowScripting)
        {
            if (!RateRules.TryParse(model.Script, out rules, out List<RateRulesErrors> errors))
            {
        ...
        if (command == "Test")
        {
            ...
            {
                if (!CurrencyPair.TryParse(pair, out CurrencyPair currencyPair))
                {
            ...
            Dictionary<CurrencyPair, Task<RateResult>> fetchs = _RateFactory.FetchRates(pairs.ToHashSet(), rules, cancellationToken);
            var testResults = new List<RatesViewModel.TestResultViewModel>();
            foreach (KeyValuePair<CurrencyPair, Task<RateResult>> fetch in fetchs)
            {
                RateResult testResult = await (fetch.Value);
    ...
    public IActionResult CheckoutAppearance()
    {
        ...
        vm.PaymentMethodCriteria = CurrentStore.GetSupportedPaymentMethods(_NetworkProvider)
                                ...
                                .Select(method =>
        {
            PaymentMethodCriteria existing =
    ...
    private void SetCryptoCurrencies(CheckoutAppearanceViewModel vm, Data.StoreData storeData)
    {
        PaymentMethodId[] enabled = storeData.GetEnabledPaymentIds(_NetworkProvider);
        PaymentMethodId defaultPaymentId = storeData.GetDefaultPaymentId();
        PaymentMethodId defaultChoice = defaultPaymentId is not null ? defaultPaymentId.FindNearest(enabled) : null;
        ...
        CheckoutAppearanceViewModel.Format[] choices = enabled
            .Select(o =>
        ...
        CheckoutAppearanceViewModel.Format chosen = defaultChoice is null ? null : choices.FirstOrDefault(c => defaultChoice.ToString().Equals(c.Value, StringComparison.OrdinalIgnoreCase));
    ...
    public async Task<IActionResult> CheckoutAppearance(CheckoutAppearanceViewModel model)
    {
        ...
        StoreBlob blob = CurrentStore.GetStoreBlob();
        PaymentMethodId defaultPaymentMethodId = model.DefaultPaymentMethod == null ? null : PaymentMethodId.Parse(model.DefaultPaymentMethod);
        ...
        {
            PaymentMethodCriteriaViewModel methodCriterion = model.PaymentMethodCriteria[index];
            if (!string.IsNullOrWhiteSpace(methodCriterion.Value))
            {
                if (!CurrencyValue.TryParse(methodCriterion.Value, out CurrencyValue value))
                {
        ...
        foreach (PaymentMethodCriteriaViewModel newCriteria in model.PaymentMethodCriteria.ToList())
        {
        ...
        foreach (PaymentMethodCriteriaViewModel newCriteria in model.PaymentMethodCriteria)
        {
            var paymentMethodId = PaymentMethodId.Parse(newCriteria.PaymentMethod);
            PaymentMethodCriteria existingCriteria = blob.PaymentMethodCriteria.FirstOrDefault(c => c.PaymentMethod == paymentMethodId);
            if (existingCriteria != null)
            {
                blob.PaymentMethodCriteria.Remove(existingCriteria);
            }

            CurrencyValue.TryParse(newCriteria.Value, out CurrencyValue cv);
    ...
    internal void AddPaymentMethods(StoreData store, StoreBlob storeBlob,
        out List<StoreDerivationScheme> derivationSchemes, out List<StoreLightningNode> lightningNodes)
    {
        IPaymentFilter excludeFilters = storeBlob.GetExcludedPaymentMethods();
        ...
        foreach (PaymentMethodId paymentMethodId in _paymentMethodHandlerDictionary.Distinct().SelectMany(handler => handler.GetSupportedPaymentMethods()))
        {
            switch (paymentMethodId.PaymentType)
            {
                case BitcoinPaymentType _:
                    DerivationSchemeSettings strategy = derivationByCryptoCode.TryGet(paymentMethodId.CryptoCode);
                    BTCPayNetwork network = _NetworkProvider.GetNetwork<BTCPayNetwork>(paymentMethodId.CryptoCode);
    ...
    {
        IEnumerable<AvailableRateProvider> exchanges = _RateFactory.RateProviderFactory.GetSupportedExchanges();
        return exchanges
    ...
    private DerivationSchemeSettings ParseDerivationStrategy(string derivationScheme, BTCPayNetwork network)
    {
        ...
        try
        {
            ...
            (NBXplorer.DerivationStrategy.DerivationStrategyBase, RootedKeyPath[]) result = parser.ParseOutputDescriptor(derivationScheme);
    ...
    public async Task<IActionResult> ListTokens()
    {
        ...
        BitTokenEntity[] tokens = await _TokenRepository.GetTokensByStoreIdAsync(CurrentStore.Id);
    ...
    {
        BitTokenEntity token = await _TokenRepository.GetToken(tokenId);
    ...
    public async Task<IActionResult> CreateToken(string storeId, CreateTokenViewModel model)
    {
        ...
        StoreData store = CurrentStore ?? await _Repo.FindStore(storeId, userId);
        if (store == null)
        {
            return Challenge(AuthenticationSchemes.Cookie);
        }
    ...
    public async Task<IActionResult> CreateToken()
    {
        ...
        StoreData[] stores = await _Repo.GetStoresByUserId(userId);
    ...
    public async Task<IActionResult> RequestPairing(string pairingCode, string selectedStore = null)
    {
        ...
        {
            StoreData store = await _Repo.FindStore(selectedStore, userId);
            if (store == null)
            {
                return NotFound();
            }
        ...
        PairingCodeEntity pairing = await _TokenRepository.GetPairingAsync(pairingCode);
        if (pairing == null)
        {
    ...
    public async Task<IActionResult> Pair(string pairingCode, string storeId)
    {
        ...
        PairingResult pairingResult = await _TokenRepository.PairWithStoreAsync(pairingCode, store.Id);
    ...
    public async Task<IActionResult> PayButton()
    {
        ...
        Models.AppViewModels.ListAppsViewModel.ListAppViewModel[] apps = await _appService.GetAllApps(_UserManager.GetUserId(User), false, store.Id);
