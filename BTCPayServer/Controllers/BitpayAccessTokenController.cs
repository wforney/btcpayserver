...
public class BitpayAccessTokenController : Controller
{
    ...
    {
        BitTokenEntity[] tokens = await _TokenRepository.GetTokens(User.GetSIN());
    ...
    public async Task<DataWrapper<List<PairingCodeResponse>>> Tokens([FromBody] TokenRequest request)
    {
        ...
        else
        {
            ...
            PairingResult result = await _TokenRepository.PairWithSINAsync(request.PairingCode, sin);
