...
public class UIVaultController : Controller
{
            ...
            WalletId walletId = null)
    {
        ...
        using (var cts = new CancellationTokenSource(TimeSpan.FromMinutes(10)))
        {
            ...
            BTCPayNetwork network = Networks.GetNetwork<BTCPayNetwork>(cryptoCode);
            if (network == null)
            {
                return NotFound();
            }

            System.Net.WebSockets.WebSocket websocket = await HttpContext.WebSockets.AcceptWebSocketAsync();
                ...
                while (true)
                {
                    ...
                    switch (command)
                    {
                        ...
                        case "ask-sign":
                            ...
                            AuthorizationResult authorization = await _authorizationService.AuthorizeAsync(User, Policies.CanModifyStoreSettings);
                            ...
                            AccountKeySettings signing = derivationSettings.GetSigningAccountKeySettings();
                            if (signing.GetRootedKeyPath()?.MasterFingerprint != fingerprint)
                            {
                                await websocketHelper.Send("{ \"error\": \"wrong-wallet\"}", cancellationToken);
                                continue;
                            }
                            KeyValuePair<PubKey, RootedKeyPath>[] signableInputs = psbt.Inputs
                                            .SelectMany(i => i.HDKeyPaths)
                                            .Where(i => i.Value.MasterFingerprint == fingerprint)
                                            .ToArray();
                            if (signableInputs.Length > 0)
                            {
                                PubKey actualPubKey = (await device.GetXPubAsync(signableInputs[0].Value.KeyPath)).GetPublicKey();
                        ...
                        case "ask-xpub":
                            ...
                            DerivationStrategyFactory factory = network.NBXplorerNetwork.DerivationStrategyFactory;
                            if (fingerprint is null)
                            {
                                fingerprint = (await device.GetXPubAsync(new KeyPath("44'"), cancellationToken)).ExtPubKey.ParentFingerprint;
                            }
...
askdevice:
                            if (!versionChecked)
                            {
                                Version version = await hwi.GetVersionAsync(cancellationToken);
                                if (version.Major < 2)
                                {
    ...
    {
        DerivationSchemeSettings paymentMethod = CurrentStore
