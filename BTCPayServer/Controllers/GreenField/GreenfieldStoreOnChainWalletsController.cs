...
public class GreenfieldStoreOnChainWalletsController : Controller
{
    ...
    public async Task<IActionResult> ShowOnChainWalletOverview(string storeId, string cryptoCode)
    {
        ...
        BTCPayWallet? wallet = _btcPayWalletProvider.GetWallet(network);
        GetBalanceResponse? balance = await wallet.GetBalance(derivationScheme.AccountDerivation);
    ...
    public async Task<IActionResult> GetOnChainWalletReceiveAddress(string storeId, string cryptoCode, bool forceGenerate = false)
    {
        ...
        KeyPathInformation? kpi = await _walletReceiveService.GetOrGenerate(new WalletId(storeId, cryptoCode), forceGenerate);
        if (kpi is null)
        {
            return BadRequest();
        }

        Common.PaymentUrlBuilder? bip21 = network.GenerateBIP21(kpi.Address?.ToString(), null);
    ...
    )
    {
        ...
        Dictionary<string, WalletTransactionInfo>? walletTransactionsInfoAsync = await _walletRepository.GetWalletTransactionsInfo(walletId);

        GetTransactionsResponse? txs = await wallet.FetchTransactions(derivationScheme.AccountDerivation);
        ...
        {
            walletTransactionsInfoAsync.TryGetValue(information.TransactionId.ToString(), out WalletTransactionInfo? transactionInfo);
        ...
        string transactionId)
    {
        ...
        BTCPayWallet? wallet = _btcPayWalletProvider.GetWallet(network);
        TransactionInformation? tx = await wallet.FetchTransaction(derivationScheme.AccountDerivation, uint256.Parse(transactionId));
        if (tx is null)
        {
        ...
        var walletId = new WalletId(storeId, cryptoCode);
        WalletTransactionInfo? walletTransactionsInfoAsync =
    ...
    public async Task<IActionResult> GetOnChainWalletUTXOs(string storeId, string cryptoCode)
    {
        ...
        var walletId = new WalletId(storeId, cryptoCode);
        Dictionary<string, WalletTransactionInfo>? walletTransactionsInfoAsync = await _walletRepository.GetWalletTransactionsInfo(walletId);
        ReceivedCoin[]? utxos = await wallet.GetUnspentCoins(derivationScheme.AccountDerivation);
        ...
        [FromBody] CreateOnChainTransactionRequest request)
    {
        ...
        ExplorerClient? explorerClient = _explorerClientProvider.GetExplorerClient(cryptoCode);
        BTCPayWallet? wallet = _btcPayWalletProvider.GetWallet(network);
        ...
        {
            CreateOnChainTransactionRequest.CreateOnChainTransactionRequestDestination? destination = request.Destinations[index];
        ...
        FeeRate? minRelayFee = _nbXplorerDashboard.Get(network.CryptoCode).Status.BitcoinStatus?.MinRelayTxFee ??
                          new FeeRate(1.0m);
        ...
        AccountKeySettings? signingKeySettings = derivationScheme.GetSigningAccountKeySettings();
        ...
        ExtKey? accountKey = signingKey.Derive(rootedKeyPath.KeyPath);
        ...
        psbt.PSBT.Finalize();
        Transaction? transaction = psbt.PSBT.ExtractTransaction();
        ...
        if (!string.IsNullOrEmpty(signingContext?.PayJoinBIP21))
        {
            ...
            try
            {
                await _delayedTransactionBroadcaster.Schedule(DateTimeOffset.UtcNow + TimeSpan.FromMinutes(2.0),
                    transaction, network);
                PSBT? payjoinPSBT = await _payjoinClient.RequestPayjoin(
                ...
                Transaction? payjoinTransaction = payjoinPSBT.ExtractTransaction();
                uint256? hash = payjoinTransaction.GetHash();
    ...
    {
        DerivationSchemeSettings? paymentMethod = Store
