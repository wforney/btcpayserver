...
public class GreenfieldApiKeysController : ControllerBase
{
    ...
    {
        if (!ControllerContext.HttpContext.GetAPIKey(out Microsoft.Extensions.Primitives.StringValues apiKey))
        {
            return
                this.CreateAPIError(404, "api-key-not-found", "The api key was not present.");
        }
        APIKeyData data = await _apiKeyRepository.GetKey(apiKey);
