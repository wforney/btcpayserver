...
public class GreenfieldUsersController : ControllerBase
{
    ...
    {
        ApplicationUser? user = (await _userManager.FindByIdAsync(idOrEmail)) ?? await _userManager.FindByEmailAsync(idOrEmail);
        if (user != null)
        {
    ...
    public async Task<IActionResult> CreateUser(CreateApplicationUserRequest request, CancellationToken cancellationToken = default)
    {
        ...
        IdentityResult? passwordValidation = await _passwordValidator.ValidateAsync(_userManager, user, request.Password);
        if (!passwordValidation.Succeeded)
        {
            foreach (IdentityError? error in passwordValidation.Errors)
            {
        ...
        if (!isAdmin)
        {
            if (!await _throttleService.Throttle(ZoneLimits.Register, HttpContext.Connection.RemoteIpAddress, cancellationToken))
            {
        ...
        if (request.IsAdministrator is true)
        {
            ...
            if (!anyAdmin)
            {
                ThemeSettings? settings = await _settingsRepository.GetSettingAsync<ThemeSettings>();
                if (settings != null)
                {
        ...
        ApplicationUserData? model = await FromModel(user);
