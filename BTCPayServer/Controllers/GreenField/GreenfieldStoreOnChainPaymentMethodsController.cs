...
public partial class GreenfieldStoreOnChainPaymentMethodsController : ControllerBase
{
        ...
        string cryptoCode)
    {
        ...
        OnChainPaymentMethodData method = GetExistingBtcLikePaymentMethod(cryptoCode);
        if (method is null)
        {
    ...
    {
        AssertCryptoCodeWallet(cryptoCode, out BTCPayNetwork network, out _);

        OnChainPaymentMethodData paymentMethod = GetExistingBtcLikePaymentMethod(cryptoCode);
        ...
        try
        {
            ...
            NBXplorer.KeyPathTemplate deposit = new NBXplorer.KeyPathTemplates(null).GetKeyPathTemplate(DerivationFeature.Deposit);
            ...
            {
                Derivation address = line.Derive((uint)i);
        ...
        int offset = 0, int amount = 10)
    {
        ...
        for (var i = offset; i < amount; i++)
        {
            Derivation derivation = line.Derive((uint)i);
            result.Addresses.Add(
                new
    ...
    {
        var id = new PaymentMethodId(cryptoCode, PaymentTypes.BTCLike);
        AssertCryptoCodeWallet(cryptoCode, out BTCPayNetwork network, out BTCPayWallet wallet);
        ...
        try
        {
            ...
            AccountKeySettings signing = strategy.GetSigningAccountKeySettings();
