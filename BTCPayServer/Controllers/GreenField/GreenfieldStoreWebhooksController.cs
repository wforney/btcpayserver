...
public class GreenfieldStoreWebhooksController : ControllerBase
{
    ...
    public async Task<IActionResult> ListWebhooks(string storeId, string webhookId)
    {
        ...
        {
            WebhookData w = await StoreRepository.GetWebhook(CurrentStoreId, webhookId);
            if (w is null)
            {
    ...
    private string CurrentStoreId
    {
        get
        {
            return HttpContext.GetStoreData()?.Id;
        }
    }
    ...
    {
        if (!Uri.TryCreate(create?.Url, UriKind.Absolute, out Uri uri))
        {
    ...
    private IActionResult WebhookNotFound()
    {
        return this.CreateAPIError(404, "webhook-not-found", "The webhook was not found");
    }

    private IActionResult WebhookDeliveryNotFound()
    {
    ...
    public async Task<IActionResult> ListDeliveries(string storeId, string webhookId, string deliveryId, int? count = null)
    {
        ...
        {
            Data.WebhookDeliveryData delivery = await StoreRepository.GetWebhookDelivery(CurrentStoreId, webhookId, deliveryId);
            if (delivery is null)
            {
    ...
    public async Task<IActionResult> RedeliverWebhook(string storeId, string webhookId, string deliveryId)
    {
        ...
        return Ok(new JValue(await WebhookSender.Redeliver(deliveryId)));
    }
    ...
    {
        WebhookDeliveryBlob b = data.GetBlob();
    ...
    private Client.Models.StoreWebhookData FromModel(Data.WebhookData data, bool includeSecret)
    {
        WebhookBlob b = data.GetBlob();
