...
public class GreenfieldInvoiceController : Controller
{
    ...
    {
        Data.StoreData? store = HttpContext.GetStoreData();
        if (store == null)
        {
        ...
        if (startDate is DateTimeOffset s &&
            ...
            s > e)
        {
            ModelState.AddModelError(nameof(startDate), "startDate should not be above endDate");
            ModelState.AddModelError(nameof(endDate), "endDate should not be below startDate");
        }
    ...
    public async Task<IActionResult> UpdateInvoice(string storeId, string invoiceId, UpdateInvoiceRequest request)
    {
        ...
        InvoiceEntity? result = await _invoiceRepository.UpdateInvoiceMetadata(invoiceId, storeId, request.Metadata);
        if (result != null)
        {
    ...
    public async Task<IActionResult> CreateInvoice(string storeId, CreateInvoiceRequest request)
    {
        ...
        {
            Services.Language? lang = LanguageService.FindLanguage(request.Checkout.DefaultLanguage);
            if (lang == null)
            {
        ...
        try
        {
            InvoiceEntity? invoice = await _invoiceController.CreateInvoiceCoreRaw(request, store,
    ...
    public async Task<IActionResult> ActivateInvoicePaymentMethod(string storeId, string invoiceId, string paymentMethod)
    {
        ...
        if (PaymentMethodId.TryParse(paymentMethod, out PaymentMethodId? paymentMethodId))
        {
    ...
    private InvoicePaymentMethodDataModel[] ToPaymentMethodModels(InvoiceEntity entity, bool includeAccountedPaymentOnly)
    {
        return entity.GetPaymentMethods().Select(
            method =>
            {
                PaymentMethodAccounting? accounting = method.Calculate();
                IPaymentMethodDetails? details = method.GetPaymentMethodDetails();
                IEnumerable<PaymentEntity>? payments = method.ParentEntity.GetPayments(includeAccountedPaymentOnly).Where(paymentEntity =>
    ...
    {
        CryptoPaymentData? data = paymentEntity.GetCryptoPaymentData();
