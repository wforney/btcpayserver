...
public class GreenfieldPullPaymentController : ControllerBase
{
    ...
    public async Task<IActionResult> GetPullPayments(string storeId, bool includeArchived = false)
    {
        ...
        List<Data.PullPaymentData>? pps = await ctx.PullPayments
    ...
    public async Task<IActionResult> CreatePullPayment(string storeId, CreatePullPaymentRequest request)
    {
        ...
        if (request.PaymentMethods is { } paymentMethodsStr)
        {
            ...
            {
                PaymentMethodId.TryParse(s, out PaymentMethodId? pmi);
                return pmi;
            }).ToArray();
            PaymentMethodId[]? supported = (await _payoutHandlers.GetSupportedPaymentMethods(HttpContext.GetStoreData())).ToArray();
        ...
        Data.PullPaymentData? pp = await _pullPaymentService.GetPullPayment(ppId, false);
    ...
    public async Task<IActionResult> GetPullPayment(string pullPaymentId)
    {
        ...
        Data.PullPaymentData? pp = await _pullPaymentService.GetPullPayment(pullPaymentId, false);
        if (pp is null)
        {
            return PullPaymentNotFound();
        }
    ...
    public async Task<IActionResult> GetPayouts(string pullPaymentId, bool includeCancelled = false)
    {
        ...
        Data.PullPaymentData? pp = await _pullPaymentService.GetPullPayment(pullPaymentId, true);
        if (pp is null)
        {
            return PullPaymentNotFound();
        }
        ...
        CurrencyData? cd = _currencyNameTable.GetCurrencyData(pp.GetBlob().Currency, false);
    ...
    public async Task<IActionResult> GetPayout(string pullPaymentId, string payoutId)
    {
        ...
        Data.PayoutData? payout = pp.Payouts.FirstOrDefault(p => p.Id == payoutId);
        if (payout is null)
        {
            return PayoutNotFound();
        }

        CurrencyData? cd = _currencyNameTable.GetCurrencyData(payout.PullPaymentData.GetBlob().Currency, false);
        return base.Ok(ToModel(payout, cd));
    }

    private Client.Models.PayoutData ToModel(Data.PayoutData p, CurrencyData cd)
    {
        PayoutBlob? blob = p.GetBlob(_serializerSettings);
    ...
    {
        if (!PaymentMethodId.TryParse(request?.PaymentMethod, out PaymentMethodId? paymentMethodId))
        {
            ModelState.AddModelError(nameof(request.PaymentMethod), "Invalid payment method");
            return this.CreateValidationError(ModelState);
        }

        IPayoutHandler? payoutHandler = _payoutHandlers.FindPayoutHandler(paymentMethodId);
        ...
        PullPaymentBlob? ppBlob = pp.GetBlob();
        (IClaimDestination destination, string error) destination = await payoutHandler.ParseAndValidateClaimDestination(paymentMethodId, request!.Destination, ppBlob);
        ...
        CurrencyData? cd = _currencyNameTable.GetCurrencyData(pp.GetBlob().Currency, false);
        ClaimRequest.ClaimResponse? result = await _pullPaymentService.Claim(new ClaimRequest()
        {
    ...
    {
        using ApplicationDbContext? ctx = _dbContextFactory.CreateContext();
        Data.PayoutData? payout = await ctx.Payouts.GetPayout(payoutId, storeId);
        if (payout is null)
        {
            return PayoutNotFound();
        }
    ...
    public async Task<IActionResult> ApprovePayout(string storeId, string payoutId, ApprovePayoutRequest approvePayoutRequest, CancellationToken cancellationToken = default)
    {
        ...
        Data.PayoutData? payout = await ctx.Payouts.GetPayout(payoutId, storeId, true, true);
        if (payout is null)
        {
            return PayoutNotFound();
        }
        ...
        PullPaymentBlob? ppBlob = payout.PullPaymentData.GetBlob();
        CurrencyData? cd = _currencyNameTable.GetCurrencyData(ppBlob.Currency, false);
        PullPaymentHostedService.PayoutApproval.Result result = await _pullPaymentService.Approve(new PullPaymentHostedService.PayoutApproval()
        {
            PayoutId = payoutId,
