...
public abstract class GreenfieldLightningNodeApiController : Controller
{
    ...
    {
        ILightningClient lightningClient = await GetLightningClient(cryptoCode, true);
        LightningNodeInformation info = await lightningClient.GetInfo();
    ...
    public virtual async Task<IActionResult> ConnectToNode(string cryptoCode, ConnectToNodeRequest request)
    {
        ...
        ConnectionResult result = await lightningClient.ConnectTo(request.NodeURI);
        switch (result)
        {
    ...
    public virtual async Task<IActionResult> GetChannels(string cryptoCode)
    {
        ...
        LightningChannel[] channels = await lightningClient.ListChannels();
    ...
    public virtual async Task<IActionResult> OpenChannel(string cryptoCode, OpenLightningChannelRequest request)
    {
        ...
        OpenChannelResponse response = await lightningClient.OpenChannel(new Lightning.OpenChannelRequest()
        {
    ...
    public virtual async Task<IActionResult> PayInvoice(string cryptoCode, PayLightningInvoiceRequest lightningInvoice)
    {
        ...
        PayInvoiceParams param = lightningInvoice?.MaxFeeFlat != null || lightningInvoice?.MaxFeePercent != null
            ? new PayInvoiceParams { MaxFeePercent = lightningInvoice.MaxFeePercent, MaxFeeFlat = lightningInvoice.MaxFeeFlat }
            : null;
        PayResponse result = await lightningClient.Pay(lightningInvoice.BOLT11, param);
    ...
    {
        ILightningClient lightningClient = await GetLightningClient(cryptoCode, false);
        LightningInvoice inv = await lightningClient.GetInvoice(id);
    ...
    public virtual async Task<IActionResult> CreateInvoice(string cryptoCode, CreateLightningInvoiceRequest request)
    {
        ...
        try
        {
            LightningInvoice invoice = await lightningClient.CreateInvoice(
