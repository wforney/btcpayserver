...
public class GreenfieldStoreLightningNetworkPaymentMethodsController : ControllerBase
{
    ...
    public ActionResult<LightningNetworkPaymentMethodData> GetLightningNetworkPaymentMethod(string storeId, string cryptoCode)
    {
        ...
        LightningNetworkPaymentMethodData? method = GetExistingLightningLikePaymentMethod(_btcPayNetworkProvider, cryptoCode, Store);
        if (method is null)
        {
        ...
        [FromBody] UpdateLightningNetworkPaymentMethodRequest request)
    {
        ...
        LightningNetworkPaymentMethodData? existing = GetExistingLightningLikePaymentMethod(_btcPayNetworkProvider, cryptoCode, store);
        if (existing == null || existing.ConnectionString != request.ConnectionString)
        {
            ...
            else
            {
                if (!LightningConnectionString.TryParse(request.ConnectionString, false,
                    out LightningConnectionString? connectionString, out var error))
                {
        ...
        StoreData store)
    {
        ...
        StoreBlob? storeBlob = store.GetStoreBlob();
        var id = new PaymentMethodId(cryptoCode, PaymentTypes.LightningLike);
        LightningSupportedPaymentMethod? paymentMethod = store
