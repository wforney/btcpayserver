...
public partial class UIInvoiceController
{
    ...
    {
        InvoiceEntity? invoice = (await _InvoiceRepository.GetInvoices(new InvoiceQuery
        {
        ...
        Data.WebhookDeliveryData? delivery = await _InvoiceRepository.GetWebhookDelivery(invoiceId, deliveryId);
        if (delivery is null)
        {
            return NotFound();
        }
    ...
    public async Task<IActionResult> Invoice(string invoiceId)
    {
        ...
        InvoiceState? invoiceState = invoice.GetInvoiceState();
        ...
        InvoiceDetailsModel? details = InvoicePopulatePayments(invoice);
    ...
    private bool CanRefund(InvoiceState invoiceState)
    {
    ...
    {
        await using ApplicationDbContext? ctx = _dbContextFactory.CreateContext();
        ctx.ChangeTracker.QueryTrackingBehavior = QueryTrackingBehavior.NoTracking;
        Data.InvoiceData? invoice = await ctx.Invoices.Include(i => i.Payments)
        ...
        PaymentMethodDictionary? paymentMethods = invoice.GetBlob(_NetworkProvider).GetPaymentMethods();
        ...
        PaymentMethodId? defaultRefund = invoice.Payments
    ...
    {
        using ApplicationDbContext? ctx = _dbContextFactory.CreateContext();

        InvoiceEntity? invoice = GetCurrentInvoice();
        if (invoice == null)
        {
            return NotFound();
        }
        ...
        CurrencyData? cdCurrency = _CurrencyNameTable.GetCurrencyData(invoice.Currency, true);
        ...
        switch (model.RefundStep)
        {
            case RefundSteps.SelectPaymentMethod:
                ...
                PaymentMethodDictionary? pms = invoice.GetPaymentMethods();
                PaymentMethod? paymentMethod = pms.SingleOrDefault(method => method.GetId() == paymentMethodId);
    ...
    private InvoiceDetailsModel InvoicePopulatePayments(InvoiceEntity invoice)
    {
        return new InvoiceDetailsModel
        {
            ...
            CryptoPayments = invoice.GetPaymentMethods().Select(
                data =>
                {
                    PaymentMethodAccounting? accounting = data.Calculate();
                    PaymentMethodId? paymentMethodId = data.GetId();
        ...
        if (selectedItems != null)
        {
            switch (command)
            {
                ...
                case "cpfp":
                    ...
                    BTCPayNetworkBase? network = _NetworkProvider.DefaultNetwork;
                    ExplorerClient? explorer = _ExplorerClients.GetExplorerClient(network);
                    ...
                    if (GetCurrentStore().Role != StoreRoles.Owner)
                    {
                        return Forbid();
                    }

                    DerivationSchemeSettings? settings = (GetCurrentStore().GetDerivationSchemeSettings(_NetworkProvider, network.CryptoCode));
                    ...
                    UTXO[]? bumpableUTXOs = utxos.GetUnspentUTXOs().Where(u => u.Confirmations == 0 && bumpableAddresses.Contains(u.ScriptPubKey.Hash.ToString())).ToArray();
                    var parameters = new MultiValueDictionary<string, string>();
                    foreach (UTXO? utxo in bumpableUTXOs)
                    {
        ...
        [FromQuery] string? view = null, [FromQuery] string? lang = null)
    {
        ...
        PaymentModel? model = await GetInvoiceModel(invoiceId, paymentMethodId == null ? null : PaymentMethodId.Parse(paymentMethodId), lang);
        if (model == null)
        {
            return NotFound();
        }
    ...
    public async Task<IActionResult> CheckoutNoScript(string? invoiceId, string? id = null, string? paymentMethodId = null, [FromQuery] string? lang = null)
    {
        ...
        PaymentModel? model = await GetInvoiceModel(invoiceId, paymentMethodId is null ? null : PaymentMethodId.Parse(paymentMethodId), lang);
        if (model == null)
        {
            return NotFound();
        }
    ...
    private async Task<PaymentModel?> GetInvoiceModel(string invoiceId, PaymentMethodId? paymentMethodId, string? lang)
    {
        ...
        {
            PaymentMethodId[]? enabledPaymentIds = store.GetEnabledPaymentIds(_NetworkProvider);
        ...
        if (network is null || !invoice.Support(paymentMethodId))
        {
            ...
            PaymentMethod? paymentMethodTemp = invoice
                .GetPaymentMethods()
        ...
        PaymentMethod? paymentMethod = invoice.GetPaymentMethod(paymentMethodId);
        ...
        InvoiceResponse? dto = invoice.EntityToDTO();
        StoreBlob? storeBlob = store.GetStoreBlob();
        PaymentMethodAccounting? accounting = paymentMethod.Calculate();

        IPaymentMethodHandler? paymentMethodHandler = _paymentMethodHandlerDictionary[paymentMethodId];
...
#pragma warning restore CS0618 // Type or member is obsolete
            ...
            AvailableCryptos = invoice.GetPaymentMethods()
                                      ...
                                      .Select(kv =>
                                      {
                                          PaymentMethodId? availableCryptoPaymentMethodId = kv.GetId();
                                          IPaymentMethodHandler? availableCryptoHandler = _paymentMethodHandlerDictionary[availableCryptoPaymentMethodId];
    ...
    public async Task<IActionResult> GetStatusWebSocket(string invoiceId, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            while (true)
            {
                WebSocketReceiveResult? message = await webSocket.ReceiveAndPingAsync(DummyBuffer);
    ...
    private readonly ArraySegment<byte> DummyBuffer = new ArraySegment<byte>(new byte[1]);
    ...
    public async Task<IActionResult> ListInvoices(InvoicesModel? model = null)
    {
        ...
        Task<int>? counting = _InvoiceRepository.GetInvoicesTotal(invoiceQuery);
        invoiceQuery.Take = model.Count;
        invoiceQuery.Skip = model.Skip;
        InvoiceEntity[]? list = await _InvoiceRepository.GetInvoices(invoiceQuery);

        model.IncludeArchived = invoiceQuery.IncludeArchived;

        foreach (InvoiceEntity? invoice in list)
        {
            InvoiceState? state = invoice.GetInvoiceState();
    ...
    public async Task<IActionResult> Export(string format, string? searchTerm = null, int timezoneOffset = 0)
    {
        ...
        InvoiceEntity[]? invoices = await _InvoiceRepository.GetInvoices(invoiceQuery);
    ...
    {
        StoreData? store = GetCurrentStore();
        IPaymentFilter? excludeFilter = store.GetStoreBlob().GetExcludedPaymentMethods();
    ...
    {
        StoreBlob? storeBlob = store.GetStoreBlob();
        IPaymentFilter? excludeFilter = storeBlob.GetExcludedPaymentMethods();
    ...
    public async Task<IActionResult> CreateInvoice(InvoicesModel? model = null)
    {
        ...
        {
            StoreData? store = await _StoreRepository.FindStore(model.StoreId, GetUserId());
            if (store == null)
            {
                return NotFound();
            }
    ...
    {
        StoreData[]? stores = await _StoreRepository.GetStoresByUserId(GetUserId());
        ...
        try
        {
            DataWrapper<InvoiceResponse>? result = await CreateInvoiceCore(new BitpayCreateInvoiceRequest()
            {
                Price = model.Amount,
