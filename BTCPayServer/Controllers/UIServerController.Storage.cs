...
public partial class UIServerController
{
    ...
    {
        Data.StoredFile file = await _StoredFileRepository.GetFile(fileId);
    ...
    public async Task<IActionResult> CreateFiles(List<IFormFile> files)
    {
        if (files != null && files.Count > 0)
        {
            ...
            foreach (IFormFile file in files)
            {
                ...
                Data.StoredFile newFile = await _FileService.AddFile(file, GetUserId());
    ...
    {
        StorageSettings savedSettings = await _SettingsRepository.GetSettingAsync<StorageSettings>();
        if (forceChoice || savedSettings == null)
        {
            IEnumerable<SelectListItem> providersList = _StorageProviderServices.Select(a =>
    ...
    public async Task<IActionResult> StorageProvider(string provider)
    {
        ...
        StorageSettings data = (await _SettingsRepository.GetSettingAsync<StorageSettings>()) ?? new StorageSettings();

        Storage.Services.Providers.IStorageProviderService storageProviderService =
