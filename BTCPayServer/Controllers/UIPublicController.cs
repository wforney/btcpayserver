...
public class UIPublicController : Controller
{
    ...
    {
        StoreData store = await _StoreRepository.FindStore(model.StoreId);
        if (store == null)
        {
        ...
        System.Collections.Specialized.NameValueCollection additionalParamValues = HttpUtility.ParseQueryString(model.CheckoutQueryString);
