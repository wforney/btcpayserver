...
public partial class UIServerController : Controller
{
    ...
    public IActionResult Maintenance()
    {
        ...
        if (IPAddress.TryParse(vm.DNSDomain, out IPAddress? unused))
        {
            vm.DNSDomain = null;
        }
    ...
    public async Task<IActionResult> Maintenance(MaintenanceViewModel vm, string command)
    {
        ...
        if (command == "changedomain")
        {
            ...
            }))
            {
                try
                {
                    ...
                    Task<IPAddress[]>? addresses1 = GetAddressAsync(Request.Host.Host);
                    Task<IPAddress[]>? addresses2 = GetAddressAsync(vm.DNSDomain);
            ...
            IActionResult? error = await RunSSH(vm, $"changedomain.sh {vm.DNSDomain}");
            if (error != null)
            {
                return error;
            }
        ...
        {
            IActionResult? error = await RunSSH(vm, $"btcpay-update.sh");
            if (error != null)
            {
                return error;
            }
        ...
        {
            IActionResult? error = await RunSSH(vm, $"btcpay-clean.sh");
            if (error != null)
            {
                return error;
            }
        ...
        {
            IActionResult? error = await RunSSH(vm, $"btcpay-restart.sh");
            if (error != null)
            {
                return error;
            }
    ...
    private async Task RunSSHCore(SshClient sshClient, string ssh)
    {
        try
        {
            ...
            SSH.SSHCommandResult? result = await sshClient.RunBash(ssh, TimeSpan.FromMinutes(1.0));
    ...
    {
        PoliciesSettings? data = (await _SettingsRepository.GetSettingAsync<PoliciesSettings>()) ?? new PoliciesSettings();
    ...
    public async Task<IActionResult> Policies([FromServices] BTCPayNetworkProvider btcPayNetworkProvider, PoliciesSettings settings, string command = "")
    {
        ...
        if (appIdsToFetch.Any())
        {
            ...
            foreach (PoliciesSettings.DomainToAppMappingItem? domainToAppMappingItem in settings.DomainToAppMapping)
            {
    ...
    public async Task<IActionResult> Services()
    {
        ...
        {
            ExternalServices = _externalServiceOptions.Value.ExternalServices.ToList()
        };

        // other services
        foreach (KeyValuePair<string, Uri> externalService in _externalServiceOptions.Value.OtherExternalServices)
        {
        ...
        foreach (TorService? torService in _torServices.Services)
        {
            ...
            else if (TryParseAsExternalService(torService, out ExternalService? externalService))
            {
        ...
        StorageSettings? storageSettings = await _SettingsRepository.GetSettingAsync<StorageSettings>();
    ...
    {
        ExternalService? result = _externalServiceOptions.Value.ExternalServices.GetService(serviceName, cryptoCode);
        if (result != null)
        {
            return result;
        }
        ...
        foreach (TorService? torService in _torServices.Services)
        {
            if (TryParseAsExternalService(torService, out ExternalService? torExternalService) &&
    ...
    {
        ExternalService? service = GetService(serviceName, cryptoCode);
        if (service == null)
        {
            return NotFound();
        }
        ...
        try
        {
            ...
            ExternalConnectionString? connectionString = await service.ConnectionString.Expand(Request.GetAbsoluteUriNoPathBase(), service.Type, _Options.NetworkType);
            switch (service.Type)
            {
                ...
                case ExternalServiceTypes.Spark:
                    ...
                    {
                        ShowQR = showQR,
                        WalletName = service.DisplayName
                    };
    ...
    private IActionResult LightningChargeServices(ExternalService service, ExternalConnectionString connectionString, bool showQR = false)
    {
        ...
        {
            Uri = connectionString.Server.AbsoluteUri,
            APIToken = connectionString.APIToken
        };
        var builder = new UriBuilder(connectionString.Server)
        {
            UserName = "api-token",
            Password = vm.APIToken
        };
    ...
    private IActionResult LndServices(ExternalService service, ExternalConnectionString connectionString, ulong? nonce, string view = nameof(LndServices))
    {
        ...
        if (nonce != null)
        {
            ...
            LightningConfigurations? lnConfig = _LnConfigProvider.GetConfig(configKey);
            if (lnConfig != null)
            {
    ...
    {
        return ((uint)HashCode.Combine(type, serviceName, cryptoCode, nonce) | (nonce & 0xffffffff00000000UL));
    }
    ...
    {
        LightningConfigurations? conf = _LnConfigProvider.GetConfig(configKey);
        if (conf == null)
        {
            return NotFound();
        }
    ...
    {
        if (!_dashBoard.IsFullySynched(cryptoCode, out NBXplorerDashboard.NBXplorerSummary? unusud))
        {
            TempData[WellKnownTempData.ErrorMessage] = $"{cryptoCode} is not fully synched";
            return RedirectToAction(nameof(Services));
        }
        ...
        if (service.Type == ExternalServiceTypes.LNDGRPC)
        {
            ...
            {
                Type = "grpc",
                Host = connectionString.Server.DnsSafeHost,
                Port = connectionString.Server.Port,
                SSL = connectionString.Server.Scheme == "https"
            };
        ...
        else if (service.Type == ExternalServiceTypes.LNDRest || service.Type == ExternalServiceTypes.CLightningRest)
        {
            ...
            {
                Type = service.Type == ExternalServiceTypes.LNDRest ? "lnd-rest" : "clightning-rest",
                Uri = connectionString.Server.AbsoluteUri
            };
    ...
    {
        DynamicDnsSettings? settings = (await _SettingsRepository.GetSettingAsync<DynamicDnsSettings>()) ?? new DynamicDnsSettings();
    ...
    {
        DynamicDnsSettings? settings = (await _SettingsRepository.GetSettingAsync<DynamicDnsSettings>()) ?? new DynamicDnsSettings();
        DynamicDnsService? service = settings.Services.FirstOrDefault(s => s.Hostname.Equals(hostname, StringComparison.OrdinalIgnoreCase));
        if (service == null)
        {
            return NotFound();
        }
    ...
    public async Task<IActionResult> SSHService()
    {
        ...
        SSH.SSHSettings? settings = _Options.SSHSettings;
        ...
        if (vm.SSHKeyFileContent == null && _sshState.CanUseSSH)
        {
            try
            {
                using SshClient? sshClient = await _Options.SSHSettings.ConnectAsync();
                SSH.SSHCommandResult? result = await sshClient.RunBash("cat ~/.ssh/authorized_keys", TimeSpan.FromSeconds(10));
    ...
    private async Task<bool> CanShowSSHService()
    {
    ...
    {
        ThemeSettings? data = await _SettingsRepository.GetSettingAsync<ThemeSettings>() ?? new ThemeSettings();
        return View(data);
    }
    ...
    {
        EmailSettings? data = (await _SettingsRepository.GetSettingAsync<EmailSettings>()) ?? new EmailSettings();
    ...
    public async Task<IActionResult> Emails(EmailsViewModel model, string command)
    {
        ...
        if (command == "Test")
        {
            try
            {
                ...
                {
                    EmailSettings? settings = await _SettingsRepository.GetSettingAsync<EmailSettings>() ?? new EmailSettings();
                ...
                using (MailKit.Net.Smtp.SmtpClient? client = await model.Settings.CreateSmtpClient())
                using (MimeMessage? message = model.Settings.CreateMailMessage(new MailboxAddress(model.TestEmail, model.TestEmail), "BTCPay test", "BTCPay test", false))
                {
        ...
        {
            EmailSettings? oldSettings = await _SettingsRepository.GetSettingAsync<EmailSettings>() ?? new EmailSettings();
    ...
    public async Task<IActionResult> LogsView(string? file = null, int offset = 0)
    {
        ...
        {
            DirectoryInfo? di = Directory.GetParent(_Options.LogFile);
            if (di is null)
            {
            ...
            FileInfo[]? logFiles = di.GetFiles($"{fileNameWithoutExtension}*{fileExtension}");
            ...
            vm.Log = "";
            FileInfo? fi = vm.LogFiles.FirstOrDefault(o => o.Name == file);
            if (fi == null)
            {
                return NotFound();
            }
