...
public partial class UIStoresController
{
    ...
    {
        ActionResult checkResult = IsAvailable(vm.CryptoCode, out StoreData store, out _);
        if (checkResult != null)
        {
            return checkResult;
        }

        DerivationSchemeSettings derivation = GetExistingDerivationStrategy(vm.CryptoCode, store);
    ...
    {
        ActionResult checkResult = IsAvailable(vm.CryptoCode, out _, out BTCPayNetwork network);
        if (checkResult != null)
        {
            return checkResult;
        }
    ...
    {
        ActionResult checkResult = IsAvailable(vm.CryptoCode, out StoreData store, out BTCPayNetwork network);
        if (checkResult != null)
        {
            return checkResult;
        }
        ...
        else if (!string.IsNullOrEmpty(vm.DerivationScheme))
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(vm.AccountKey))
                {
                    ...
                    AccountKeySettings accountSettings =
    ...
    {
        ActionResult checkResult = IsAvailable(cryptoCode, out StoreData store, out BTCPayNetwork network);
        if (checkResult != null)
        {
            return checkResult;
        }
        ...
        ExplorerClient client = _ExplorerProvider.GetExplorerClient(cryptoCode);
        ...
        AccountKeySettings accountSettings = derivationSchemeSettings.GetSigningAccountKeySettings();
        ...
        IActionResult result = await UpdateWallet(vm);
    ...
    {
        ActionResult checkResult = IsAvailable(cryptoCode, out _, out BTCPayNetwork network);
        if (checkResult != null)
        {
            return checkResult;
        }
    ...
    public async Task<IActionResult> WalletSettings(string storeId, string cryptoCode)
    {
        ...
        DerivationSchemeSettings derivation = GetExistingDerivationStrategy(cryptoCode, store);
        if (derivation == null)
        {
            return NotFound();
        }
        ...
        ExplorerClient client = _ExplorerProvider.GetExplorerClient(network);
    ...
    public async Task<IActionResult> UpdateWalletSettings(WalletSettingsViewModel vm)
    {
        ...
        BitcoinExtPubKey signingKey = string.IsNullOrEmpty(vm.SelectedSigningKey)
            ? null
    ...
    {
        ActionResult checkResult = IsAvailable(cryptoCode, out StoreData store, out _);
        if (checkResult != null)
        {
            return checkResult;
        }
    ...
    {
        vm.DerivationScheme = strategy.AccountDerivation.ToString();
        KeyPathTemplate deposit = new KeyPathTemplates(null).GetKeyPathTemplate(DerivationFeature.Deposit);

        if (!string.IsNullOrEmpty(vm.DerivationScheme))
        {
            DerivationLine line = strategy.AccountDerivation.GetLineFor(deposit);

            for (uint i = 0; i < 10; i++)
            {
                KeyPath keyPath = deposit.GetKeyPath(i);
                RootedKeyPath rootedKeyPath = vm.GetAccountKeypath()?.Derive(keyPath);
                Derivation derivation = line.Derive(i);
    ...
    private DerivationSchemeSettings GetExistingDerivationStrategy(string cryptoCode, StoreData store)
    {
        ...
        DerivationSchemeSettings existing = store.GetSupportedPaymentMethods(_NetworkProvider)
            .OfType<DerivationSchemeSettings>()
