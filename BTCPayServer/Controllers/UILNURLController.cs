...
public class UILNURLController : Controller
{
    ...
    public async Task<IActionResult> GetLNURLForApp(string cryptoCode, string appId, string itemCode = null)
    {
        ...
        AppData app = await _appService.GetApp(appId, null, true);
        if (app is null)
        {
            return NotFound();
        }

        Data.StoreData store = app.StoreData;
        if (store is null)
        {
            return NotFound();
        }
        ...
        IEnumerable<ISupportedPaymentMethod> methods = store.GetSupportedPaymentMethods(_btcPayNetworkProvider);
        var lnUrlMethod =
            methods.FirstOrDefault(method => method.PaymentId == pmi) as LNURLPaySupportedPaymentMethod;
        ISupportedPaymentMethod lnMethod = methods.FirstOrDefault(method => method.PaymentId == lnpmi);
        ...
        switch (app.AppType)
        {
            case nameof(AppType.Crowdfund):
                CrowdfundSettings cfS = app.GetSettings<CrowdfundSettings>();
                currencyCode = cfS.TargetCurrency;
                items = _appService.Parse(cfS.PerksTemplate, cfS.TargetCurrency);
                break;
            case nameof(AppType.PointOfSale):
                UIAppsController.PointOfSaleSettings posS = app.GetSettings<UIAppsController.PointOfSaleSettings>();
                currencyCode = posS.Currency;
                items = _appService.Parse(posS.Template, posS.Currency);
                break;
        }

        ViewPointOfSaleViewModel.Item item = items.FirstOrDefault(item1 =>
    ...
    {
        LightningAddressSettings lightningAddressSettings = await GetSettings();
        if (!lightningAddressSettings.Items.TryGetValue(username.ToLowerInvariant(), out LightningAddressSettings.LightningAddressItem item))
        {
            ...
            internalDetails = null)
    {
        ...
        Data.StoreData store = await _storeRepository.FindStore(storeId);
        if (store is null)
        {
        ...
        InvoiceEntity i = await _invoiceController.CreateInvoiceCoreRaw(
        ...
        {
            PaymentMethod pm = i.GetPaymentMethod(pmi);
        ...
        [FromQuery] long? amount = null, string comment = null)
    {
        ...
        var pmi = new PaymentMethodId(cryptoCode, PaymentTypes.LNURLPay);
        InvoiceEntity i = await _invoiceRepository.GetInvoice(invoiceId, true);
        if (i.Status == InvoiceStatusLegacy.New)
        {
            var isTopup = i.IsUnsetTopUp();
            LNURLPaySupportedPaymentMethod lnurlSupportedPaymentMethod =
            ...
            PaymentMethod lightningPaymentMethod = i.GetPaymentMethod(pmi);
            PaymentMethodAccounting accounting = lightningPaymentMethod.Calculate();
            ...
            LightMoney max = isTopup ? LightMoney.FromUnit(6.12m, LightMoneyUnit.BTC) : min;
