...
public class UIPaymentRequestController : Controller
{
    ...
    public async Task<IActionResult> GetPaymentRequests(string storeId, ListPaymentRequestsViewModel model = null)
    {
        ...
        (int Total, PaymentRequestData[] Items) result = await _PaymentRequestRepository.FindPaymentRequests(new PaymentRequestQuery
        {
            UserId = GetUserId(),
            StoreId = store.Id,
            Skip = model.Skip,
    ...
    {
        StoreData store = GetCurrentStore();
        PaymentRequestData paymentRequest = GetCurrentPaymentRequest();
    ...
    public async Task<IActionResult> EditPaymentRequest(string payReqId, UpdatePaymentRequestViewModel viewModel)
    {
        ...
        PaymentRequestData data = paymentRequest ?? new PaymentRequestData();
        data.StoreDataId = viewModel.StoreId;
        data.Archived = viewModel.Archived;

        PaymentRequestBaseData blob = data.GetBlob();
    ...
    {
        ViewPaymentRequestViewModel result = await _PaymentRequestService.GetPaymentRequest(payReqId, GetUserId());
        if (result == null)
        {
            return NotFound();
        }
        ...
        decimal? amount = null, CancellationToken cancellationToken = default)
    {
        ...
        ViewPaymentRequestViewModel.PaymentRequestInvoice currentInvoice = result
            .Invoices
        ...
        PaymentRequestData pr = await _PaymentRequestRepository.FindPaymentRequest(payReqId, null, cancellationToken);
        PaymentRequestBaseData blob = pr.GetBlob();
        StoreData store = pr.StoreData;
        try
        {
            ...
            InvoiceEntity newInvoice = await _InvoiceController.CreateInvoiceCoreRaw(invoiceRequest, store, Request.GetAbsoluteRoot(), additionalTags, cancellationToken);
    ...
    public async Task<IActionResult> CancelUnpaidPendingInvoice(string payReqId, bool redirect = true)
    {
        ...
        IEnumerable<ViewPaymentRequestViewModel.PaymentRequestInvoice> invoices = result.Invoices.Where(requestInvoice =>
        ...
        foreach (ViewPaymentRequestViewModel.PaymentRequestInvoice invoice in invoices)
        {
    ...
    {
        StoreData store = GetCurrentStore();
        IActionResult result = EditPaymentRequest(store.Id, payReqId);
