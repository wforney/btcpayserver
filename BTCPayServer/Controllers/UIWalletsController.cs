...
public partial class UIWalletsController : Controller
{
    ...
    private readonly CurrencyNameTable _currencyTable;
    ...
    private readonly string[] LabelColorScheme =
    {
            "#fbca04",
            "#0e8a16",
            "#ff7619",
                            ...
                            string removelabel = null)
    {
        ...
        Task<WalletBlobInfo> walletBlobInfoAsync = WalletRepository.GetWalletInfo(walletId);
        ...
        BTCPayWallet wallet = _walletProvider.GetWallet(paymentMethod.Network);
        WalletBlobInfo walletBlobInfo = await walletBlobInfoAsync;
        ...
        if (addlabel != null)
        {
            ...
            IEnumerable<ColoredLabel> labels = _labelFactory.GetWalletColoredLabels(walletBlobInfo, Request);
            if (!walletTransactionsInfo.TryGetValue(transactionId, out WalletTransactionInfo walletTransactionInfo))
            {
    ...
    public async Task<IActionResult> ListWallets()
    {
        ...
        StoreData[] stores = await Repository.GetStoresByUserId(GetUserId());
        ...
        foreach ((BTCPayWallet Wallet, StoreData Store, Task<string> Balance, DerivationStrategyBase DerivationStrategy, BTCPayNetwork Network) wallet in onChainWallets)
        {
            ...
            IMoney money = await GetBalanceAsMoney(wallet.Wallet, wallet.DerivationStrategy);
    ...
    )
    {
        ...
        Task<Dictionary<string, WalletTransactionInfo>> walletTransactionsInfoAsync = WalletRepository.GetWalletTransactionsInfo(walletId);
        GetTransactionsResponse transactions = await wallet.FetchTransactions(paymentMethod.AccountDerivation);
        ...
        {
            foreach (TransactionInformation tx in transactions.UnconfirmedTransactions.Transactions
                .Concat(transactions.ConfirmedTransactions.Transactions).ToArray())
            {
                ...
                if (walletTransactionsInfo.TryGetValue(tx.TransactionId.ToString(), out WalletTransactionInfo transactionInfo))
                {
                    IEnumerable<ColoredLabel> labels = _labelFactory.ColorizeTransactionLabels(walletBlob, transactionInfo, Request);
            ...
            WalletId walletId)
    {
        ...
        StoreData store = GetCurrentStore();
        BitcoinAddress address = _walletReceiveService.Get(walletId)?.Address;
        var allowedPayjoin = paymentMethod.IsHotWallet && store.GetStoreBlob().PayJoinEnabled;
        Common.PaymentUrlBuilder bip21 = network.GenerateBIP21(address?.ToString(), null);
        if (allowedPayjoin)
        {
    ...
    {
        PoliciesSettings policies = await _settingsRepository.GetSettingAsync<PoliciesSettings>();
            ...
            WalletId walletId, string defaultDestination = null, string defaultAmount = null, string[] bip21 = null)
    {
        ...
        Rating.RateRules rateRules = store.GetStoreBlob().GetRateRules(NetworkProvider);
        ...
        IFeeProvider feeProvider = _feeRateProvider.CreateFeeProvider(network);
        Task<WalletSendModel.FeeRateOption>[] recommendedFees =
            new[]
                {
                ...
                }.Select(async time =>
                {
                    try
                    {
                        FeeRate result = await feeProvider.GetFeeRateAsync(
                ...
                .ToArray();
        Task<GetBalanceResponse> balance = _walletProvider.GetWallet(network).GetBalance(paymentMethod.AccountDerivation);
        model.NBXSeedAvailable = await GetSeed(walletId, network) != null;
        GetBalanceResponse Balance = await balance;
        ...
        using (CancellationTokenSource cts = new CancellationTokenSource())
        {
            try
            {
                ...
                RateResult result = await RateFetcher.FetchRate(currencyPair, rateRules, cts.Token).WithCancellation(cts.Token);
            ...
            WalletId walletId, WalletSendModel vm, string command = "", CancellationToken cancellation = default, string bip21 = "")
    {
        ...
        {
            DerivationSchemeSettings schemeSettings = GetDerivationSchemeSettings(walletId);
            WalletBlobInfo walletBlobAsync = await WalletRepository.GetWalletInfo(walletId);
            Dictionary<string, WalletTransactionInfo> walletTransactionsInfoAsync = await WalletRepository.GetWalletTransactionsInfo(walletId);

            ReceivedCoin[] utxos = await _walletProvider.GetWallet(network).GetUnspentCoins(schemeSettings.AccountDerivation, cancellation);
            vm.InputsAvailable = utxos.Select(coin =>
            {
                walletTransactionsInfoAsync.TryGetValue(coin.OutPoint.Hash.ToString(), out WalletTransactionInfo info);
        ...
        {
            WalletSendModel.TransactionOutput transactionOutput = vm.Outputs[i];
    ...
    private IActionResult ViewVault(WalletId walletId, SigningContextModel signingContext)
    {
        return View(nameof(WalletSendVault), new WalletSendVaultModel()
        {
            ...
            WebsocketPath = Url.Action(nameof(UIVaultController.VaultBridgeConnection), "UIVault", new { walletId = walletId.ToString() })
        });
    }
    ...
    private IActionResult RedirectToWalletPSBTReady(WalletPSBTReadyViewModel vm)
    {
        ...
        if (HttpContext.Request.Query["returnUrl"].FirstOrDefault() is string returnUrl)
        {
            ...
            WalletId walletId, SignWithSeedViewModel viewModel)
    {
        ...
        DerivationSchemeSettings settings = GetDerivationSchemeSettings(walletId);
        AccountKeySettings signingKeySettings = settings.GetSigningAccountKeySettings();
            ...
            WalletId walletId)
    {
        ...
        {
            IsFullySync = _dashboard.IsFullySynched(walletId.CryptoCode, out NBXplorerDashboard.NBXplorerSummary unused),
            IsServerAdmin = (await _authorizationService.AuthorizeAsync(User, Policies.CanModifyServerSettings)).Succeeded,
            IsSupportedByCurrency = _dashboard.Get(walletId.CryptoCode)?.Status?.BitcoinStatus?.Capabilities?.CanScanTxoutSet == true
        };
        ExplorerClient explorer = ExplorerClientProvider.GetExplorerClient(walletId.CryptoCode);
        ScanUTXOInformation scanProgress = await explorer.GetScanUTXOSetInformationAsync(paymentMethod.AccountDerivation);
    ...
    private static async Task<IMoney> GetBalanceAsMoney(BTCPayWallet wallet, DerivationStrategyBase derivationStrategy)
    {
        ...
        try
        {
            GetBalanceResponse b = await wallet.GetBalance(derivationStrategy, cts.Token);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        switch (command)
        {
            ...
            case "prune":
                {
                    PruneResponse result = await ExplorerClientProvider.GetExplorerClient(walletId.CryptoCode).PruneAsync(derivationScheme.AccountDerivation, new PruneRequest(), cancellationToken);
            ...
            case "clear" when User.IsInRole(Roles.ServerAdmin):
                {
                    if (Version.TryParse(_dashboard.Get(walletId.CryptoCode)?.Status?.Version ?? "0.0.0.0", out Version v) &&
                        v < new Version(2, 2, 4))
                    {
