...
public class UIPullPaymentController : Controller
{
    ...
    public async Task<IActionResult> ViewPullPayment(string pullPaymentId)
    {
        ...
        PullPaymentBlob blob = pp.GetBlob();
        ...
        CurrencyData cd = _currencyNameTable.GetCurrencyData(blob.Currency, false);
    ...
    public async Task<IActionResult> ClaimPullPayment(string pullPaymentId, ViewPullPaymentModel vm)
    {
        ...
        PaymentMethodId paymentMethodId = ppBlob.SupportedPaymentMethods.FirstOrDefault(id => vm.SelectedPaymentMethod == id.ToString());

        IPayoutHandler payoutHandler = paymentMethodId is null ? null : _payoutHandlers.FindPayoutHandler(paymentMethodId);
        if (payoutHandler is null)
        {
            ModelState.AddModelError(nameof(vm.SelectedPaymentMethod), $"Invalid destination with selected payment method");
            return await ViewPullPayment(pullPaymentId);
        }
        (IClaimDestination destination, string error) destination = await payoutHandler?.ParseAndValidateClaimDestination(paymentMethodId, vm.Destination, ppBlob);
        ...
        ClaimRequest.ClaimResponse result = await _pullPaymentHostedService.Claim(new ClaimRequest()
        {
