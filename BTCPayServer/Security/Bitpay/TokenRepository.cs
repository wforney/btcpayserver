...
public class TokenRepository
{
    ...
    public async Task<BitTokenEntity[]> GetTokens(string sin)
    {
        ...
        using ApplicationDbContext ctx = _Factory.CreateContext();
    ...
    public async Task GenerateLegacyAPIKey(string storeId)
    {
        ...
        using ApplicationDbContext ctx = _Factory.CreateContext();
        System.Collections.Generic.List<APIKeyData> existing = await ctx.ApiKeys.Where(o => o.StoreId == storeId && o.Type == APIKeyType.Legacy).ToListAsync();
        if (existing.Any())
        {
    ...
    public async Task<string> CreatePairingCodeAsync()
    {
        ...
        using (ApplicationDbContext ctx = _Factory.CreateContext())
        {
            ...
            DateTime expiration = DateTime.UtcNow + TimeSpan.FromMinutes(15);
    ...
    {
        using ApplicationDbContext ctx = _Factory.CreateContext();
        PairingCodeData pairingCode = await ctx.PairingCodes.FindAsync(pairingCodeEntity.Id);
    ...
    {
        using ApplicationDbContext ctx = _Factory.CreateContext();
        PairingCodeData pairingCode = await ctx.PairingCodes.FindAsync(pairingCodeId);
        ...
        PairingResult result = await ActivateIfComplete(ctx, pairingCode);
        await ctx.SaveChangesAsync();
        return result;
    }
    ...
    {
        using ApplicationDbContext ctx = _Factory.CreateContext();
        PairedSINData token = await ctx.PairedSINData.FindAsync(tokenId);
        if (token == null)
        {
            return false;
        }
