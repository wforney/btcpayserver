...
public class APIKeysAuthenticationHandler : AuthenticationHandler<GreenfieldAuthenticationOptions>
{
    ...
    {
        if (!Context.Request.HttpContext.GetAPIKey(out Microsoft.Extensions.Primitives.StringValues apiKey) || string.IsNullOrEmpty(apiKey))
        {
            return AuthenticateResult.NoResult();
        }

        APIKeyData key = await _apiKeyRepository.GetKey(apiKey, true);
        ...
        List<Claim> claims = new List<Claim>
            {
                new Claim(_identityOptions.CurrentValue.ClaimsIdentity.UserIdClaimType, key.UserId)
            };
