...
public class StoreSelector : ViewComponent
{
    ...
    public async Task<IViewComponentResult> InvokeAsync()
    {
        ...
        StoreData[] stores = await _storeRepo.GetStoresByUserId(userId);
        StoreData currentStore = ViewContext.HttpContext.GetStoreData();
        var options = stores
            .Select(store =>
            {
                var cryptoCode = store
                    ...
                    .FirstOrDefault()?
                    .Network.CryptoCode;
                WalletId walletId = cryptoCode != null ? new WalletId(store.Id, cryptoCode) : null;
