...
public class DerivationSchemeSettings : ISupportedPaymentMethod
{
    ...
    private static bool TryParseXpub(string xpub, DerivationSchemeParser derivationSchemeParser, ref DerivationSchemeSettings derivationSchemeSettings, bool electrum = true)
    {
        if (!electrum)
        {
            try
            {
                (DerivationStrategyBase, RootedKeyPath[]) result = derivationSchemeParser.ParseOutputDescriptor(xpub);
    ...
    private AccountKeySettings[] _AccountKeySettings;
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    {
        foreach (AccountKeySettings accountKey in AccountKeySettings)
        {
    ...
    {
        foreach (NBXplorer.Models.PSBTRebaseKeyRules rebase in GetPSBTRebaseKeyRules())
        {
