...
public static class Extensions
{
    ...
    public static bool IsSafe(this LightningConnectionString connectionString)
    {
        ...
        Uri uri = connectionString.BaseUri;
        if (uri.Scheme.Equals("unix", StringComparison.OrdinalIgnoreCase))
        {
            return false;
        }

        if (!NBitcoin.Utils.TryParseEndpoint(uri.DnsSafeHost, 80, out EndPoint endpoint))
        {
            return false;
        }
    ...
    public static async Task<Dictionary<uint256, TransactionResult>> GetTransactions(this BTCPayWallet client, uint256[] hashes, bool includeOffchain = false, CancellationToken cts = default(CancellationToken))
    {
        ...
        Task<TransactionResult>[] transactions = hashes
...
#nullable restore
    ...
    {
        UpdatePSBTResponse result = await explorerClientProvider.GetExplorerClient(psbt.Network.NetworkSet.CryptoCode).UpdatePSBTAsync(new UpdatePSBTRequest()
        {
            PSBT = psbt,
    ...
    {
        UserPrefsCookie prefCookie = ctx.GetUserPrefsCookie();
    ...
    public static string ToSql<TEntity>(this IQueryable<TEntity> query) where TEntity : class
    {
        ...
        Microsoft.EntityFrameworkCore.Query.SqlExpressions.SelectExpression selectExpression = relationalCommandCache.Private<Microsoft.EntityFrameworkCore.Query.SqlExpressions.SelectExpression>("_selectExpression");
        Microsoft.EntityFrameworkCore.Query.IQuerySqlGeneratorFactory factory = relationalCommandCache.Private<Microsoft.EntityFrameworkCore.Query.IQuerySqlGeneratorFactory>("_querySqlGeneratorFactory");

        Microsoft.EntityFrameworkCore.Query.QuerySqlGenerator sqlGenerator = factory.Create();
    ...
    {
        ChainName _networkType = DefaultConfiguration.GetNetworkType(configuration);
        ...
        BTCPayNetworkProvider filtered = networkProvider.Filter(supportedChains.ToArray());
#if ALTCOINS
...
#endif
        BTCPayNetworkProvider result = networkProvider.Filter(supportedChains.ToArray());
    ...
    {
        ChainName networkType = DefaultConfiguration.GetNetworkType(configuration);
