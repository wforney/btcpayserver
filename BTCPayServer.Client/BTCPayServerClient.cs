...
public partial class BTCPayServerClient
{
    ...
    protected async Task HandleResponse(HttpResponseMessage message)
    {
        if (!message.IsSuccessStatusCode && message.Content?.Headers?.ContentType?.MediaType?.StartsWith("application/json", StringComparison.OrdinalIgnoreCase) is true)
        {
            ...
            {
                Models.GreenfieldValidationError[] err = JsonConvert.DeserializeObject<Models.GreenfieldValidationError[]>(await message.Content.ReadAsStringAsync());
                throw new GreenfieldValidationException(err);
            }
            if (message.StatusCode == System.Net.HttpStatusCode.Forbidden)
            {
                Models.GreenfieldPermissionAPIError err = JsonConvert.DeserializeObject<Models.GreenfieldPermissionAPIError>(await message.Content.ReadAsStringAsync());
                throw new GreenfieldAPIException((int)message.StatusCode, err);
            }
            else
            {
                Models.GreenfieldAPIError err = JsonConvert.DeserializeObject<Models.GreenfieldAPIError>(await message.Content.ReadAsStringAsync());
                if (err.Code != null)
                {
                    throw new GreenfieldAPIException((int)message.StatusCode, err);
                }
    ...
    {
        using HttpResponseMessage resp = await _httpClient.SendAsync(CreateHttpRequest(path, queryPayload, method), cancellationToken);
    ...
    HttpMethod? method = null, CancellationToken cancellationToken = default)
    {
        using HttpResponseMessage resp = await _httpClient.SendAsync(CreateHttpRequest(path: path, bodyPayload: bodyPayload, method: method), cancellationToken);
        return await HandleResponse<T>(resp);
    }
    ...
    {
        UriBuilder uriBuilder = new(_btcpayHost) { Path = path };
    ...
    {
        HttpRequestMessage request = CreateHttpRequest(path, queryPayload, method);
