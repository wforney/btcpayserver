...
public class Permission
{
    ...
    {
        if (TryCreatePermission(policy, scope, out Permission r))
        {
            return r;
        }
    ...
    public static IEnumerable<Permission> ToPermissions(string[] permissions)
    {
        ...
        {
            if (TryParse(p, out Permission pp))
            {
                yield return pp;
            }
        }
    ...
    private bool ContainsPolicy(string subpolicy)
    {
        ...
        switch (subpolicy)
        {
            case Policies.CanViewInvoices when Policy == Policies.CanModifyStoreSettings:
            case Policies.CanViewInvoices when Policy == Policies.CanModifyInvoices:
            case Policies.CanModifyStoreWebhooks when Policy == Policies.CanModifyStoreSettings:
            case Policies.CanViewInvoices when Policy == Policies.CanViewStoreSettings:
            case Policies.CanViewStoreSettings when Policy == Policies.CanModifyStoreSettings:
            case Policies.CanCreateInvoice when Policy == Policies.CanModifyStoreSettings:
            case Policies.CanModifyInvoices when Policy == Policies.CanModifyStoreSettings:
            case Policies.CanViewProfile when Policy == Policies.CanModifyProfile:
            case Policies.CanModifyPaymentRequests when Policy == Policies.CanModifyStoreSettings:
            case Policies.CanViewPaymentRequests when Policy == Policies.CanModifyStoreSettings:
            case Policies.CanViewPaymentRequests when Policy == Policies.CanViewStoreSettings:
            case Policies.CanCreateLightningInvoiceInternalNode when Policy == Policies.CanUseInternalLightningNode:
            case Policies.CanCreateLightningInvoiceInStore when Policy == Policies.CanUseLightningNodeInStore:
            case Policies.CanViewNotificationsForUser when Policy == Policies.CanManageNotificationsForUser:
            case Policies.CanUseInternalLightningNode when Policy == Policies.CanModifyServerSettings:
                return true;
            default:
                return false;
        }
