...
public class BTCPayDefaultSettings
{
    ...
    private static readonly Dictionary<ChainName, BTCPayDefaultSettings> _Settings;

    public static BTCPayDefaultSettings GetDefaultSettings(ChainName chainType)
    {
        if (_Settings.TryGetValue(chainType, out BTCPayDefaultSettings v))
        {
            return v;
        }
...
public class BTCPayNetwork : BTCPayNetworkBase
{
    ...
    {
        var builder = new PaymentUrlBuilder(NBitcoinNetwork.UriScheme)
        {
            Host = cryptoInfoAddress
        };
