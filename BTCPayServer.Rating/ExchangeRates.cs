...
{
    private readonly Dictionary<string, ExchangeRate> _AllRates = new Dictionary<string, ExchangeRate>();
    ...
    {
        foreach (PairRate rate in rates)
        {
    ...
    private readonly List<ExchangeRate> _Rates = new List<ExchangeRate>();
    ...
    {
        if (ByExchange.TryGetValue(exchangeName, out IReadOnlyCollection<ExchangeRate> rates))
        {
            ExchangeRate rate = rates.FirstOrDefault(r => r.CurrencyPair == currencyPair);
            if (rate != null)
            {
                rate.BidAsk = bidAsk;
            }
            CurrencyPair invPair = currencyPair.Inverse();
            ExchangeRate invRate = rates.FirstOrDefault(r => r.CurrencyPair == invPair);
            if (invRate != null)
            {
...
public class BidAsk
{
    ...
    private static readonly BidAsk _One = new BidAsk(1.0m);
    ...
    private static readonly BidAsk _Zero = new BidAsk(0.0m);
