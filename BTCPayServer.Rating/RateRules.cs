...
{
    private class NormalizeCurrencyPairsRewritter : CSharpSyntaxRewriter
    {
        ...
        {
            if (CurrencyPair.TryParse(node.Identifier.ValueText, out CurrencyPair currencyPair))
            {
    ...
    private class RuleList : CSharpSyntaxWalker
    {
        ...
        public override void VisitAssignmentExpression(AssignmentExpressionSyntax node)
        {
            ...
            {
                if (CurrencyPair.TryParse(id.Identifier.ValueText, out CurrencyPair currencyPair))
                {
    ...
    private RateRules(SyntaxNode root)
    {
    ...
    {
        return TryParse(str, out rules, out List<RateRulesErrors> unused);
    }
    public static bool TryParse(string str, out RateRules rules, out List<RateRulesErrors> errors)
    {
        ...
        rules = null;
        errors = null;
        SyntaxTree expression = CSharpSyntaxTree.ParseText(str, new CSharpParseOptions(LanguageVersion.Default).WithKind(SourceCodeKind.Script));
        var rewriter = new NormalizeCurrencyPairsRewritter();
        // Rename BTC_usd to BTC_USD and verify structure
        SyntaxNode root = rewriter.Visit(expression.GetRoot());
    ...
    public RateRule GetRuleFor(CurrencyPair currencyPair)
    {
        ...
        ExpressionSyntax candidate = FindBestCandidate(currencyPair);
    ...
    public ExpressionSyntax FindBestCandidate(CurrencyPair p)
    {
        ...
        {
            if (ruleList.ExpressionsByPair.TryGetValue(pair.Pair, out (ExpressionSyntax Expression, SyntaxNode Trivia) expression))
            {
        ...
        (CurrencyPair Pair, int Priority, ExpressionSyntax Expression, bool Inverse) best = candidates
...
{
    private class ReplaceExchangeRateRewriter : CSharpSyntaxRewriter
    {
        public List<RateRulesErrors> Errors = new List<RateRulesErrors>();
        ...
        public override SyntaxNode VisitInvocationExpression(InvocationExpressionSyntax node)
        {
            ...
            if (currencyPair == null || !CurrencyPair.TryParse(currencyPair, out CurrencyPair pair))
            {
                Errors.Add(RateRulesErrors.InvalidCurrencyIdentifier);
            ...
            {
                BidAsk rate = Rates.GetRate(exchangeName, pair);
                if (rate == null)
                {
    ...
    private class CalculateWalker : CSharpSyntaxWalker
    {
        ...
        private Stack<decimal> _TupleValues = null;
    ...
    private class HasBinaryOperations : CSharpSyntaxWalker
    {
    ...
    private class FlattenExpressionRewriter : CSharpSyntaxRewriter
    {
        private readonly RateRules parent;
        ...
        public override SyntaxNode VisitInvocationExpression(InvocationExpressionSyntax node)
        {
            ...
            SyntaxNode result = base.VisitInvocationExpression(node);
            IsInvocation = false;
            return result;
        }

        private bool IsArgumentList;
        ...
        public List<RateRulesErrors> Errors = new List<RateRulesErrors>();
        private const int MaxNestedCount = 8;
        public override SyntaxNode VisitIdentifierName(IdentifierNameSyntax node)
        {
            if (
                (!IsInvocation || IsArgumentList) &&
                CurrencyPair.TryParse(node.Identifier.ValueText, out CurrencyPair currentPair))
            {
                ...
                {
                    ExpressionSyntax bestCandidate = parent.FindBestCandidate(replacedPair);
                    ...
                    FlattenExpressionRewriter innerFlatten = CreateNewContext(replacedPair);
                    SyntaxNode replaced = innerFlatten.Visit(bestCandidate);
    ...
    private readonly FlattenExpressionRewriter flatten;

    public static RateRule CreateFromExpression(string expression, CurrencyPair currencyPair)
    {
        ExpressionSyntax ex = RateRules.CreateExpression(expression);
        RateRules.TryParse("", out RateRules rules);
    ...
    public bool Reevaluate()
    {
        ...
        SyntaxNode result = rewriter.Visit(expression);
        foreach (RateRulesErrors item in rewriter.Errors)
        {
            Errors.Add(item);
        }
        ...
        {
            foreach (RateRulesErrors item in calculate.Errors)
            {
                Errors.Add(item);
            }
            return false;
    ...
    private SyntaxNode _EvaluatedNode;
