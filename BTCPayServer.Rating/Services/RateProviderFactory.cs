...
{
    private class WrapperRateProvider : IRateProvider
    {
    ...
    private void InitExchanges()
    {
        ...
        _ = AddExchangeSharpProviders<ExchangeBinanceAPI>("binance");
        ...
        foreach (KeyValuePair<string, IRateProvider> provider in Providers.ToArray())
        {
            var prov = new BackgroundFetcherRateProvider(Providers[provider.Key])
            {
                RefreshRate = TimeSpan.FromMinutes(1.0),
                ValidatyTime = TimeSpan.FromMinutes(5.0)
            };
        ...
        foreach (AvailableRateProvider supportedExchange in GetCoinGeckoSupportedExchanges())
        {
    ...
    private IEnumerable<AvailableRateProvider> _AvailableRateProviders = null;
    public IEnumerable<AvailableRateProvider> GetSupportedExchanges()
    {
        if (_AvailableRateProviders == null)
        {
            ...
            foreach (AvailableRateProvider exchange in GetDirectlySupportedExchanges())
            {
                availableProviders.Add(exchange.Id, exchange);
            }
            foreach (AvailableRateProvider exchange in GetCoinGeckoSupportedExchanges())
            {
    ...
    {
        Providers.TryGetValue(exchangeName, out IRateProvider directProvider);
        directProvider = directProvider ?? NullRateProvider.Instance;

        var wrapper = new WrapperRateProvider(directProvider);
        PairRate[] value = await wrapper.GetRatesAsync(cancellationToken);
