...
public class CryptoMarketExchangeRateProvider : IRateProvider
{
    ...
    private readonly List<string> SupportedPairs = new List<string>()
        {
            "BTCARS",
            "BTCCLP",
            "BTCBRL"
    ...
    {
        HttpResponseMessage response = await _httpClient.GetAsync("https://api.exchange.cryptomkt.com/api/3/public/ticker/", cancellationToken);
        JObject jobj = await response.Content.ReadAsAsync<JObject>(cancellationToken);

        return (jobj ?? new JObject())
            .Properties()
