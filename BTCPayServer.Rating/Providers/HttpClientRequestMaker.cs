...
#nullable restore
    private class InternalHttpWebResponse : IHttpWebResponse
    {
        public InternalHttpWebResponse(HttpResponseMessage httpResponseMessage)
        {
            ...
            foreach (KeyValuePair<string, IEnumerable<string>> h in httpResponseMessage.Headers)
            {
                if (!headers.TryGetValue(h.Key, out List<string> list))
                {
            ...
            foreach (KeyValuePair<string, List<string>> item in headers)
            {
        ...
        {
            Headers.TryGetValue(name, out IReadOnlyList<string> list);
            return list ?? Empty;
        }
    }
