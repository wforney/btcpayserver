...
public class KrakenExchangeRateProvider : IRateProvider
{
    ...
    {
        _Helper = ExchangeAPI.GetExchangeAPIAsync<ExchangeKrakenAPI>().GetAwaiter().GetResult();
    }

    private readonly IExchangeAPI _Helper;
    ...
    private readonly ConcurrentDictionary<string, string> notFoundSymbols = new ConcurrentDictionary<string, string>(new Dictionary<string, string>()
        {
            {"ADAXBT","ADAXBT"},
            { "BSVUSD","BSVUSD"},
            { "QTUMEUR","QTUMEUR"},
    ...
    private DateTimeOffset? _LastSymbolUpdate = null;
    private readonly Dictionary<string, string> _TickerMapping = new Dictionary<string, string>()
        {
            { "XXDG", "DOGE" },
            { "XXBT", "BTC" },
            { "XBT", "BTC" },
    ...
    public async Task<PairRate[]> GetRatesAsync(CancellationToken cancellationToken)
    {
        ...
        {
            ExchangeTicker ticker = ConvertToExchangeTicker(symbol, apiTickers[symbol]);
            if (ticker != null)
            {
                try
                {
                    ...
                    if (CurrencyPair.TryParse(global, out CurrencyPair pair))
                    {
    ...
    {
        if (_LastSymbolUpdate is not null && DateTimeOffset.UtcNow - _LastSymbolUpdate.Value < TimeSpan.FromDays(0.5))
        {
            return _Symbols;
        }
        else
    ...
    private async Task<T> MakeJsonRequestAsync<T>(string url, string baseUrl = null, Dictionary<string, object> payload = null, string requestMethod = null, CancellationToken cancellationToken = default)
    {
        ...
        if (payload is not null)
        {
            _ = sb
                .Append("?")
                .Append(string.Join('&', payload.Select(kv => $"{kv.Key}={kv.Value}").OfType<object>().ToArray()));
        }
