...
internal class BackgroundFetcherRateJsonConverter : JsonConverter
{
...
public class BackgroundFetcherRateProvider : IRateProvider
{
    ...
    private TimeSpan _RefreshRate = TimeSpan.FromSeconds(30);
    /// <summary>
    ...
    public TimeSpan RefreshRate
    {
        ...
        set
        {
            TimeSpan diff = value - _RefreshRate;
            LatestFetch latest = _Latest;
            if (latest != null)
            {
    ...
    private TimeSpan _ValidatyTime = TimeSpan.FromMinutes(10);
    /// <summary>
    ...
    public TimeSpan ValidatyTime
    {
        ...
        set
        {
            TimeSpan diff = value - _ValidatyTime;
            LatestFetch latest = _Latest;
            if (latest != null)
            {
    ...
    private static readonly TimeSpan MaxBackoff = TimeSpan.FromMinutes(5.0);
    ...
    private async Task<LatestFetch> Fetch(CancellationToken cancellationToken)
    {
        ...
        LatestFetch previous = _Latest;
        var fetch = new LatestFetch();
        try
        {
            PairRate[] rates = await _Inner.GetRatesAsync(cancellationToken);
            fetch.Latest = rates;
