...
public class HitBTCRateProvider : IRateProvider
{
    ...
    {
        HttpResponseMessage response = await _httpClient.GetAsync("https://api.hitbtc.com/api/2/public/ticker", cancellationToken);
        JArray jarray = await response.Content.ReadAsAsync<JArray>(cancellationToken);
        return jarray
            .Children<JObject>()
            .Select(p =>
            {
                CurrencyPair.TryParse(p["symbol"].Value<string>(), out CurrencyPair currency);
                BidAsk bidask = CreateBidAsk(p);
